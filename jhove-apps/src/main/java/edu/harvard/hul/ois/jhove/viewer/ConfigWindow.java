...
public class ConfigWindow extends JDialog {
  ...
  static final Font _pathFont = new Font(SANS_SERIF, Font.PLAIN, 10);
  static final Font _infoFont = new Font(SANS_SERIF, Font.PLAIN, 12);
   ...
   * Constructor.
   *
   * @param configFile The file which was opened for configuration information, or null to start
   *     with a clean slate.
   * @param handler A ConfigHandler which has already processed the configuration file, or null if
   *     configFile is null.
   */
  ...
  private void doSave() {
    ...
    try {
      ...
      cw.writeFile(_modules, _handlers, _homeDir, _tempDir, _encoding, _bufferSize);
    } catch (IOException e) {
      JOptionPane.showMessageDialog(
          this, e.getMessage(), "Can't create config", JOptionPane.ERROR_MESSAGE);
    }
