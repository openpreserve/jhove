...
public class RepTreeRoot extends DefaultMutableTreeNode {
   ...
   * Constructor.
   *
   * @param info The RepInfo object whose contents are to be displayed.
  ...
  private static final String COUNTING_MODE_NTSC_NON_DROP_FRAME =
    ...
    if (arity == PropertyArity.SCALAR) {
      if (null == typ) {
        ...
        DefaultMutableTreeNode val =
            new DefaultMutableTreeNode(pProp.getName() + ": " + pValue.toString());
        return val;
      } else {
        ...
        switch (typ) {
          ...
          default:
            ...
            // Simple types: just use name plus string value.
            val = new DefaultMutableTreeNode(pProp.getName() + ": " + pValue.toString());
            return val;
        }
      }
    }
  ...
  private void snarfRepInfo() {
    ...
    if (module != null) {
      ...
      moduleNode.add(
          new DefaultMutableTreeNode("Date: " + _dateFmt.format(module.getDate()), false));
      add(moduleNode);
    }
        ...
        case INTEGER:
          {
            if (pVal instanceof int[]) {
              Integer[] vals =
                  Arrays.stream((int[]) pVal) // IntStream
        ...
        case LONG:
          {
            if (pVal instanceof long[]) {
              Long[] vals =
                  Arrays.stream((long[]) pVal) // IntStream
                      .boxed() // Stream<Integer>
                      .toArray(Long[]::new);
              addToNode(node, vals);
            } else {
  ...
  private void addListMembers(DefaultMutableTreeNode node, Property p) {
    ...
    l.forEach(item -> node.add(getDefaultMutableTreeNode(ptyp, item, canHaveChildren)));
  }
  ...
  private DefaultMutableTreeNode aesToNode(AESAudioMetadata aes) {
    ...
    if (in != AESAudioMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              BYTE_ORDER + (in == AESAudioMetadata.BIG_ENDIAN ? "BIG_ENDIAN" : "LITTLE_ENDIAN")));
    }
    ...
    if (!facelist.isEmpty()) {
      ...
      addAESTimeRange(timeline, facergn.getStartTime(), facergn.getDuration());
      region.add(timeline);
      ...
      if (nchan != AESAudioMetadata.NULL) {
        ...
        for (String loc : locs) {
          ...
          region.add(stream);
          stream.add(new DefaultMutableTreeNode("ChannelAssignment: " + loc, false));
        }
      }
      face.add(region);
      val.add(face);
    ...
    if (!flist.isEmpty()) {
      ...
      if (bitDepth != AESAudioMetadata.NULL
        ...
        if (bitDepth != AESAudioMetadata.NULL) {
          formatRegion.add(
              new DefaultMutableTreeNode("BitDepth: " + Integer.toString(bitDepth), false));
        }
        if (sampleRate != AESAudioMetadata.NILL) {
          formatRegion.add(
              new DefaultMutableTreeNode("SampleRate: " + Double.toString(sampleRate), false));
        }
        if (wordSize != AESAudioMetadata.NULL) {
          formatRegion.add(
              new DefaultMutableTreeNode("WordSize: " + Integer.toString(bitDepth), false));
        }
        if (bitRed != null) {
  ...
  private void addAESTimeRange(
    ...
    node.add(new DefaultMutableTreeNode(COUNTING_MODE_NTSC_NON_DROP_FRAME, false));
    node.add(new DefaultMutableTreeNode(HOURS + start.getHours(), false));
    node.add(new DefaultMutableTreeNode(MINUTES + start.getMinutes(), false));
    node.add(new DefaultMutableTreeNode(SECONDS + start.getSeconds(), false));
    node.add(new DefaultMutableTreeNode(FRAMES + start.getFrames(), false));

    // Do samples a bit more elaborately than is really necessary,
    // to maintain parallelism with the xml schema.
    DefaultMutableTreeNode snode = new DefaultMutableTreeNode(SAMPLES, true);
    double sr = start.getSampleRate();
    if (sr == 1.0) {
      sr = _sampleRate;
    }
    snode.add(new DefaultMutableTreeNode("SampleRate: S" + Integer.toString((int) sr), false));
    snode.add(new DefaultMutableTreeNode(NUMBER_OF_SAMPLES + start.getSamples(), false));
    node.add(snode);
    ...
    if (duration != null) {
      ...
      node.add(new DefaultMutableTreeNode(VIDEO_FIELD_FIELD_1));
      node.add(new DefaultMutableTreeNode(COUNTING_MODE_NTSC_NON_DROP_FRAME, false));
      node.add(new DefaultMutableTreeNode(HOURS + duration.getHours(), false));
      node.add(new DefaultMutableTreeNode(MINUTES + duration.getMinutes(), false));
      node.add(new DefaultMutableTreeNode(SECONDS + duration.getSeconds(), false));
      node.add(new DefaultMutableTreeNode(FRAMES + duration.getFrames(), false));
      ...
      snode.add(new DefaultMutableTreeNode("SamplesRate S" + Integer.toString((int) sr), false));
      snode.add(new DefaultMutableTreeNode(NUMBER_OF_SAMPLES + duration.getSamples(), false));
      node.add(snode);
  ...
  private DefaultMutableTreeNode nisoToNode(NisoImageMetadata niso) {
    ...
    if ((n = niso.getColorSpace()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "ColorSpace: "
                  + integerRepresentation(
                      n, NisoImageMetadata.COLORSPACE, NisoImageMetadata.COLORSPACE_INDEX),
              false));
    }
    ...
    if (iarray != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("YCbCrSubSampling"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    }
    if ((n = niso.getYCbCrPositioning()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "YCbCrPositioning: " + integerRepresentation(n, NisoImageMetadata.YCBCR_POSITIONING),
              false));
    }
    Rational[] rarray = niso.getYCbCrCoefficients();
    if (rarray != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("YCbCrCoefficients", true));
      val.add(nod);
    ...
    if (rarray != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ReferenceBlackWhite", true));
      val.add(nod);
      for (int i = 0; i < rarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(rarray[i].toString(), false));
      }
    ...
    if ((n = niso.getSegmentType()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "YSegmentType: " + integerRepresentation(n, NisoImageMetadata.SEGMENT_TYPE), false));
    }
    long[] larray = niso.getStripOffsets();
    if (larray != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("StripOffsets", true));
      val.add(nod);
      for (int i = 0; i < larray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Long.toString(larray[i]), false));
      }
    }
    ...
    if ((larray = niso.getStripByteCounts()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("StripByteCounts", true));
      val.add(nod);
      for (int i = 0; i < larray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Long.toString(larray[i]), false));
      }
    ...
    if ((larray = niso.getTileOffsets()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("TileOffsets", true));
      val.add(nod);
      for (int i = 0; i < larray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Long.toString(larray[i]), false));
      }
    ...
    if ((larray = niso.getTileByteCounts()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("TileByteCounts", true));
      val.add(nod);
      for (int i = 0; i < larray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Long.toString(larray[i]), false));
      }
    ...
    if ((n = niso.getPlanarConfiguration()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "PlanarConfiguration: "
                  + integerRepresentation(n, NisoImageMetadata.PLANAR_CONFIGURATION),
              false));
    }
    ...
    if ((n = niso.getChecksumMethod()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "ChecksumMethod: " + integerRepresentation(n, NisoImageMetadata.CHECKSUM_METHOD),
              false));
    }
    ...
    if ((n = niso.getOrientation()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "Orientation: " + integerRepresentation(n, NisoImageMetadata.ORIENTATION), false));
    }
    if ((n = niso.getDisplayOrientation()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "DisplayOrientation: "
                  + integerRepresentation(n, NisoImageMetadata.DISPLAY_ORIENTATION),
              false));
    }
    ...
    if (darray != null) {
      DefaultMutableTreeNode nod = new DefaultMutableTreeNode("SubjectDistance", true);
      val.add(nod);
      for (int i = 0; i < darray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Double.toString(darray[i]), false));
      }
    }
    ...
    if ((n = niso.getFlash()) != NisoImageMetadata.NULL) {
      ...
      val.add(
          new DefaultMutableTreeNode(
              "Flash: " + integerRepresentation(n & 1, NisoImageMetadata.FLASH_20), false));
    }
    ...
    if ((n = niso.getFlashReturn()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "FlashReturn: " + integerRepresentation(n, NisoImageMetadata.FLASH_RETURN), false));
    }
    if ((n = niso.getBackLight()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "BackLight: " + integerRepresentation(n, NisoImageMetadata.BACKLIGHT), false));
    }
    ...
    if ((n = niso.getSensor()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "Sensor: " + integerRepresentation(n, NisoImageMetadata.SENSOR), false));
    }
      ...
      val.add(
          new DefaultMutableTreeNode(
              "SamplingFrequencyPlane: "
                  + integerRepresentation(n, NisoImageMetadata.SAMPLING_FREQUENCY_PLANE),
              false));
    }
      ...
      val.add(
          new DefaultMutableTreeNode(
              "SamplingFrequencyUnit: "
                  + integerRepresentation(n, NisoImageMetadata.SAMPLING_FREQUENCY_UNIT),
              false));
    }
      ...
      val.add(
          new DefaultMutableTreeNode(
              "SourceXDimensionUnit: "
                  + integerRepresentation(n, NisoImageMetadata.SOURCE_DIMENSION_UNIT),
              false));
    }
    ...
    if ((iarray = niso.getBitsPerSample()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("BitsPerSample"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((iarray = niso.getExtraSamples()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ExtraSamples"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(
            new DefaultMutableTreeNode(
                integerRepresentation(iarray[i], NisoImageMetadata.EXTRA_SAMPLES), false));
      }
    }
    ...
    if ((iarray = niso.getColormapBitCodeValue()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ColormapBitCodeValue"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((iarray = niso.getColormapRedValue()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ColormapRedValue"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((iarray = niso.getColormapGreenValue()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ColormapGreenValue"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((iarray = niso.getColormapBlueValue()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("ColormapBlueValue"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((iarray = niso.getGrayResponseCurve()) != null) {
      DefaultMutableTreeNode nod = (new DefaultMutableTreeNode("GrayResponseCurve"));
      val.add(nod);
      for (int i = 0; i < iarray.length; i++) {
        nod.add(new DefaultMutableTreeNode(Integer.toString(iarray[i]), false));
      }
    ...
    if ((n = niso.getTargetType()) != NisoImageMetadata.NULL) {
      val.add(
          new DefaultMutableTreeNode(
              "TargetType: " + integerRepresentation(n, NisoImageMetadata.TARGET_TYPE), false));
    }
    ...
    if (sarray != null) {
      DefaultMutableTreeNode nod = new DefaultMutableTreeNode("ProcessingActions", true);
      val.add(nod);
  ...
  private DefaultMutableTreeNode getDefaultMutableTreeNode(
      PropertyType ptyp, Object item, boolean allowsChildren) {
