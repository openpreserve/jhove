...
public abstract class InfoWindow extends JFrame {
  ...
  private static final String[] encodings = {"UTF-8", "ISO-8859-1", "Cp1252", "MacRoman"};
  ...
  public InfoWindow(String title, App app, JhoveBase base) throws HeadlessException {
    ...
    _closeItem.setAccelerator(
        KeyStroke.getKeyStroke(
            KeyEvent.VK_W, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
    _closeItem.addActionListener(
        new ActionListener() {
          @Override
  ...
  /**
   * Sets the ActionListener for the "Save as" menu item. Subclasses need to call this with an
   * appropriate ActionListener in order to make the menu item functional.
   */
  ...
  /**
   * Puts up a dialog to save the file. If the user requests a file, deletes any old file with the
   * same name, creates the new file, and returns a PrintWriter to the file. The save dialog is
   * customized with two JComboBoxes. One lets the user select a character encoding, which is used
   * by the PrintWriter; the other lets the user choose an OutputHandler to control the output
   * format (e.g., text or HTML). The encodings shown in a JComboBox are UTF-8, ISO-8859-1, Cp1252,
   * MacRoman, and the default encoding for the locale (if different from the above), but the user
   * can type in other encodings. If an unknown encoding is selected, an error dialog will be
   * displayed. OutputHandlers other than the ones known to the application can't be specified (what
   * would the application do with them?)
   */
  protected PrintWriter doSaveDialog() {
    ...
    if (retval == JFileChooser.APPROVE_OPTION) {
      ...
      try {
        ...
        if (file.exists()) {
          int opt =
              JOptionPane.showConfirmDialog(
                  this,
                  "That file already exists. Replace?",
                  "Replace",
          ...
          if (!file.delete()) {
            JOptionPane.showMessageDialog(
                this, "Could not delete file", "File not deleted", JOptionPane.ERROR_MESSAGE);
            return null;
          }
        }
        file.createNewFile();
        ...
        JOptionPane.showMessageDialog(
            this, "Unknown encoding ", FILE_NOT_SAVED, JOptionPane.ERROR_MESSAGE);
        // Get rid of the file
        try {
          if (os != null) {
            os.close();
        ...
        JOptionPane.showMessageDialog(
            this, e.getMessage(), FILE_NOT_SAVED, JOptionPane.ERROR_MESSAGE);
        // Get rid of the file
        try {
          if (os != null) {
            os.close();
  ...
  /**
   * Sets up the OutputHandler from the JComboBox and returns it. Subclasses should call <code>
   * selectHandler</code> to obtain an OutputHandler with which to produce data.
   */
