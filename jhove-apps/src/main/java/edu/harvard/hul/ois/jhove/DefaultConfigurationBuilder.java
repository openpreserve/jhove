...
public class DefaultConfigurationBuilder {
  ...
  public DefaultConfigurationBuilder(File location) {
    if (location != null) {
      ...
      configFile = new File(JHOVE_DIR + FILE_SEP + "conf" + FILE_SEP + "jhove.conf");
    }
  }
  ...
  /**
   * We can't have a static method in an Interface and override it, so we have to get a bit ugly to
   * fake static inheritance. The advantage of this is that only the Modules with non-null default
   * config file parameters have to implement the defaultConfigParams static field.
   */
