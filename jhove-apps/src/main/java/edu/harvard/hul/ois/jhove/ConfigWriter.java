...
public class ConfigWriter {
  ...
  /**
   * Constructor. Creates a temporary file for writing and creates an OutputStreamWriter to write to
   * it. If there is already a file located by <code>file</code>, it will not be replaced or
   * overwritten until <code>writeFile</code> has successfully written out the temporary file.
   *
   ...
   *
   * <p>If the temporary file can't be written, or the configuration file can't be replaced, a
  ...
  public void writeFile(
    ...
    _confOut.println(" <jhoveHome>" + encodeContent(homeDir.getPath()) + "</jhoveHome>");

    // Write out the encoding
    if (encoding != null && encoding.length() > 0) {
      _confOut.println(" <defaultEncoding>" + encodeContent(encoding) + "</defaultEncoding>");
    }

    if (tempDir != null) {
      _confOut.println(" <tempDirectory>" + encodeContent(tempDir.getPath()) + "</tempDirectory>");
    }

    // Write the buffer size if not default
    if (bufferSize > 0) {
      _confOut.println(" <bufferSize>" + bufferSize + "</bufferSize>");
    }
    ...
    while (iter.hasNext()) {
      ...
      if (!"".equals(minfo.clas)) {
        ...
        _confOut.println(CLASS_TAG_START + encodeContent(minfo.clas) + CLASS_TAG_CLOSE);
        if (minfo.init != null && minfo.init.length() > 0) {
          _confOut.println("   <init>" + encodeContent(minfo.init) + "</init>");
        }
        /** tuple[2] and beyond are parameters */
        if (minfo.params != null) {
          for (int i = 0; i < minfo.params.length; i++) {
            _confOut.println("   <param>" + encodeContent(minfo.params[i]) + "</param>");
          }
        }
        _confOut.println(" </module>");
      }
    ...
    while (hiter.hasNext()) {
      ...
      if (handler.length() > 0) { // Don't write out blank handler names
        ...
        _confOut.println(CLASS_TAG_START + encodeContent(handler) + CLASS_TAG_CLOSE);
        _confOut.println(" </outputHandler>");
      }
    }
    ...
    if (_confFile.exists() && !_confFile.delete()) {
      if (_parent != null) {
        JOptionPane.showMessageDialog(
            _parent, "Can't replace old config file", "Error", JOptionPane.ERROR_MESSAGE);
      }
      _tempFile.delete();
    } else {
  ...
  /**
   * Encodes a content String in XML-clean form, converting characters to entities as necessary. The
   * null string will be converted to an empty string.
   */
