...
mkdir -p test-root/candidates/1.14
...
bash "$SCRIPT_DIR/baseline-jhove.sh" -j "${tempInstallLoc}" -c test-root/corpora -o test-root/candidates/1.14

if [ -d test-root/targets ]; then
	rm -rf test-root/targets
fi
mkdir -p test-root/targets/1.14

bash "$SCRIPT_DIR/create-1-14-target.sh"
bash "$SCRIPT_DIR/bbt-jhove.sh" -b test-root/targets/1.14/release -c ./test-root/corpora -j ./ -o test-root/candidates -k dev-test -i
exit $?
