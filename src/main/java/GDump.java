...
public class GDump extends Dump {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("00000006: LogicalScreenDescriptor: " + width
					+ "x" + height + " 0x" + leading(packedFields, 2)
					+ Integer.toHexString(packedFields) + " " + backgroundColor
					+ " " + aspectRatio);
			long os = 13;
			...
			for (int op = 0; (op = stream.readUnsignedByte()) != -1;) {
				if (op == 0x00) {
					...
					System.out.println(leading(os, 8) + os
							...
							+ imageTop + " " + imageWidth + "x" + imageHeight
							+ " 0x" + leading(packedFields, 2)
					...
					System.out.println(leading(os, 8) + os + ": ImageData: "
							+ size);
					...
					if (label == 0x01) {
						...
						System.out.println(leading(os, 8) + os
								+ ": PlainTextExtension: " + blockSize + " "
								+ gridLeft + "," + gridTop + " " + gridWidth
								+ "x" + gridHeight + " " + cellWidth + "x"
								+ cellHeight + " " + foreground + ","
								+ background);
						os += 15;
						os = subBlocks(stream, os);
					} else if (label == 0xf9) {
					...
					System.out.println(leading(os, 8) + os
							+ ": Unknown block: 0x" + leading(hex, 2) + hex);
					os++;
				}
			}
			stream.close();
	...
	private static long colorTable(DataInputStream stream, long os, int size)
		...
		for (int i = 0; i < n; i++) {
			...
			System.out.println(leading(os, 8) + os + ": " + leading(i, 3, ' ')
					+ i + ": " + leading(r, 3, ' ') + r + ","
					+ leading(g, 3, ' ') + g + "," + leading(b, 3, ' ') + b);
			os += 3;
		}
		return os;
	}
