...
public class JDump extends Dump {
			...
			while (!endOfImage) {
				if (!readingECS) {
					if (!haveCode) {
						...
						for (int i = 0; (code = stream.readUnsignedByte()) == 0xff; i++) {
					...
					System.out.println(leading(os, 8) + os + ": ECS" + nECS
							+ " " + length + " ...");
					os += length;
					nECS++;
					readingECS = false;
				...
				if (code == 0x01) {
					...
					if (id.equals("JFIF\0")) {
						...
						System.out.print(leading(os, 8) + os + ": APP0 " + "0x"
								+ leading(hex, 2) + hex);
						int n = length - 3;
						if (n > 0) {
							for (int i = 0; i < n; i++) {
					...
					if ((n == 1 || n == 2) && length >= 8) {
						...
						System.out.println(leading(os, 8) + os + ": APP" + n
								+ " " + length + " ...");
						for (int i = 2; i < length; i++) {
							stream.readUnsignedByte();
						}
					...
					System.out.print(leading(os, 8) + os + ": VER " + v + "."
							+ rev);
					int n = length - 4;
					if (n > 0) {
						int[] cap = new int[n];
	 ...
	 */
	private static int markerSegment(DataInputStream stream, boolean bigEndian)
			throws IOException {
