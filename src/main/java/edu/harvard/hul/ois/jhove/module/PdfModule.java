...
public class PdfModule extends ModuleBase {
	...
	private final static String[] compressionStrings = { "LZWDecode", /*
																	 * "FlateDecode"
																	 * ,
																	 */
			"RunLengthDecode", "DCTDecode", "CCITTFaxDecode" };
	...
	public PdfModule() {
		...
		doc.setIdentifier(new Identifier("http://partners.adobe.com/asn/"
				...
				+ "Specifications/PDFReference.pdf", IdentifierType.URL));
		...
		_signature.add(new ExternalSignature(".pdf", SignatureType.EXTENSION,
				SignatureUseType.OPTIONAL));
		_signature.add(new InternalSignature("%PDF-1.", SignatureType.MAGIC,
	...
	public final void parse(RandomAccessFile raf, RepInfo info)
		 ...
		 * subproperties
		 */
		_metadata = new Property("PDFMetadata", PropertyType.PROPERTY,
		...
		while (_startxref > 0) {
			...
			if (_startxref == _prevxref) {
				info.setMessage(new ErrorMessage(
						"Cross reference tables are broken", _parser
								.getOffset()));
		...
		metadataList.add(new Property("Objects", PropertyType.INTEGER,
				new Integer(_numObjects)));
		metadataList.add(new Property("FreeObjects", PropertyType.INTEGER,
		...
		if (_docCatalogList != null) {
			metadataList
					.add(new Property("DocumentCatalog", PropertyType.PROPERTY,
							PropertyArity.LIST, _docCatalogList));
		}
		if (_encryptList != null) {
			metadataList.add(new Property("Encryption", PropertyType.PROPERTY,
					PropertyArity.LIST, _encryptList));
		}
		if (_docInfoList != null) {
			metadataList.add(new Property("Info", PropertyType.PROPERTY,
		...
		if (_extStreamsList != null && !_extStreamsList.isEmpty()) {
			metadataList
					.add(new Property("ExternalStreams", PropertyType.PROPERTY,
							PropertyArity.LIST, _extStreamsList));
		}
		if (_filtersList != null && !_filtersList.isEmpty()) {
			metadataList.add(new Property("Filters", PropertyType.PROPERTY,
					PropertyArity.LIST, _filtersList));
		}
		if (_imagesList != null && !_imagesList.isEmpty()) {
			metadataList.add(new Property("Images", PropertyType.PROPERTY,
					PropertyArity.LIST, _imagesList));
		}
		if (_showFonts || _verbosity == Module.MAXIMUM_VERBOSITY) {
			try {
				...
				info.setMessage(new ErrorMessage(
						"unexpected error in parsing font property", e
								.toString()));
			}
		}
		if (_nFonts > maxFonts) {
	...
	protected boolean findLastTrailer(RepInfo info) throws IOException {
		...
		if (_eof < 0L) {
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage("No PDF trailer", _raf.length()));
			return false;
		}
	...
	protected boolean parseTrailer(RepInfo info, boolean prevOnly)
		...
		try {
			...
			if (xref instanceof Keyword) {
				_xrefIsStream = false;
				_parser.getNext(Numeric.class, invalidMsg); // first object
															// number
				_objCount = ((Numeric) _parser.getNext(Numeric.class,
						invalidMsg)).getIntegerValue();
				 ...
				 */
				PdfStream str = (PdfStream) _parser
						.readObjectDef((Numeric) xref);
			...
			if (trailer < 0L) {
				info.setWellFormed(false);
				info.setMessage(new ErrorMessage("No file trailer", _parser
						.getOffset()));
				return false;
			}
			...
			_encryptDictRef = (PdfIndirectObj) _trailerDict.get("Encrypt"); // This
																			// is
																			// at
																			// least
																			// v.
			...
			if (infoObj != null && !(infoObj instanceof PdfIndirectObj)) {
				throw new PdfInvalidException("Trailer dictionary Info key is "
						+ "not an indirect reference", _parser.getOffset());
			}
			...
			e.disparage(info);
			info.setMessage(new ErrorMessage(e.getMessage(), _parser
					.getOffset()));
		...
		while (_startxref > 0) {
			try {
				...
				try {
					...
					info.setWellFormed(false);
					info.setMessage(new ErrorMessage(
							"Malformed cross reference stream", _parser
									.getOffset()));
					return false;
				}
			} catch (PdfException e) {
	...
	protected boolean readDocCatalogDict(RepInfo info) throws IOException {
		...
		if (_docCatDictRef == null) {
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(nocat, 0));
			return false;
		}
		try {
		...
		try {
			...
			PdfObject pLayout = resolveIndirectObject(_docCatDict
					.get("PageLayout"));
			...
			PdfObject outlines = resolveIndirectObject(_docCatDict
					.get("Outlines"));
			...
			_pageLabelDict = (PdfDictionary) resolveIndirectObject(_docCatDict
					.get("PageLabels"));
			...
			try {
				PdfDictionary namesDict = (PdfDictionary) resolveIndirectObject(_docCatDict
						.get("Names"));
				if (namesDict != null) {
					PdfDictionary embeddedDict = (PdfDictionary) resolveIndirectObject(namesDict
							.get("EmbeddedFiles"));
					...
					PdfDictionary dDict = (PdfDictionary) resolveIndirectObject(namesDict
							.get("Dests"));
					if (dDict != null) {
			...
			try {
				_destsDict = (PdfDictionary) resolveIndirectObject(_docCatDict
						.get("Dests"));
			} catch (ClassCastException ce) {
	...
	protected boolean readEncryptDict(RepInfo info) throws IOException {
		...
		try {
			...
			Property p = new Property("SecurityHandler", PropertyType.STRING,
					filterText);
			_encryptList.add(p);
			...
			if (algorithm instanceof PdfSimpleObject) {
				...
				if (tok instanceof Numeric) {
					...
					if (_je != null && _je.getShowRawFlag()) {
						p = new Property("Algorithm", PropertyType.INTEGER,
								new Integer(algValue));
					} else {
						try {
							p = new Property("Algorithm", PropertyType.STRING,
			...
			if (length instanceof PdfSimpleObject) {
				...
				if (_je != null) {
					p = new Property("KeyLength", PropertyType.INTEGER,
							new Integer(keyLen));
					_encryptList.add(p);
				}
			}
			...
			if ("Standard".equals(filterText)) {
				...
				if (flagObj instanceof PdfSimpleObject) {
					...
					stdList.add(new Property("Revision", PropertyType.INTEGER,
							new Integer(rev)));
				}
	...
	protected boolean readDocInfoDict(RepInfo info) throws IOException {
		...
		try {
			...
			addStringProperty(_docInfoDict, _docInfoList, "Keywords",
					"Keywords");
			addStringProperty(_docInfoDict, _docInfoList, "Creator", "Creator");
			addStringProperty(_docInfoDict, _docInfoList, "Producer",
					"Producer");
			// CreationDate requires string-to-date conversion
			// ModDate does too
			addDateProperty(_docInfoDict, _docInfoList, "CreationDate",
					"CreationDate");
	...
	protected boolean readDocumentTree(RepInfo info) throws IOException {
		try {
			...
			info.setMessage(new ErrorMessage(e.getClass().getName(), _parser
					.getOffset()));
			info.setWellFormed(false);
			return false;
		}
	...
	protected boolean readXMPData(RepInfo info) {
		...
		try {
			PdfStream metadata = (PdfStream) resolveIndirectObject(_docCatDict
					.get("Metadata"));
			if (metadata == null) {
			...
			info.setMessage(new ErrorMessage(badMetadata, _parser.getOffset()));
			info.setValid(false);
			return false;
		}
		return true;
	...
	protected String extractFilters(Filter[] filters, PdfStream stream) {
		...
		if (filterStr != null && unique) {
			Property prop = new Property("FilterPipeline", PropertyType.STRING,
					filterStr);
	...
	protected void findImages(RepInfo info) throws IOException {
		...
		try {
			for (;;) {
				...
				if (rsrc != null) {
					PdfDictionary xo = (PdfDictionary) resolveIndirectObject(rsrc
							.get("XObject"));
					if (xo != null) {
						...
						while (iter.hasNext()) {
							...
							PdfObject xob = resolveIndirectObject((PdfObject) iter
									.next());
							...
							if (xobdict != null) {
								...
								if ("Image".equals(subtype.getStringValue())) {
									...
									PdfSimpleObject widObj = (PdfSimpleObject) xobdict
											.get("Width");
									niso.setImageWidth(widObj.getIntValue());
									PdfSimpleObject htObj = (PdfSimpleObject) xobdict
											.get("Height");
									...
									if (bpc != null) {
										...
										niso.setBitsPerSample(new int[] { bpc
												.getIntValue() });
									}
									...
									if (nam != null) {
										imgList.add(new Property("Name",
												PropertyType.STRING, nam
														.getStringValue()));
									}

									PdfSimpleObject id = (PdfSimpleObject) resolveIndirectObject(xobdict
											.get("ID"));
									if (id != null) {
										String idstr = toHex(id
												.getStringValue());
	...
	protected void findFonts(RepInfo info) throws IOException {
		...
		try {
			...
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage("Unexpected error in findFonts", e
					.toString(), _parser.getOffset()));
			return;
		}
	}
		...
		if (offset < 0) {
			...
			try {
				...
				if (streamObjIndex == _cachedStreamIndex) {
					...
					streamObj = resolveIndirectObject(getObject(streamObjIndex,
							recGuard - 1));
	...
	protected void addFontsProperty(List<Property> metadataList) {
		...
		if (_trueTypeFontsMap != null && !_trueTypeFontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty("TrueType", _trueTypeFontsMap, F_TT);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
				// Report an error here?
			}
		...
		if (_cid0FontsMap != null && !_cid0FontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty("CIDFontType0", _cid0FontsMap, F_CID0);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
				// TODO : Add exception handling
			}
		...
		if (_cid2FontsMap != null && !_cid2FontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty("CIDFontType2", _cid2FontsMap, F_CID2);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
				// TODO : Add exception handling
			}
		...
		if (fontTypesList.size() > 0) {
			metadataList.add(new Property("Fonts", PropertyType.PROPERTY,
					PropertyArity.LIST, fontTypesList));
		}
	}
	...
	protected void addPagesProperty(List<Property> metadataList, RepInfo info) {
		...
		try {
			...
			if (_showPages || _verbosity == Module.MAXIMUM_VERBOSITY) {
				Property prop = new Property("Pages", PropertyType.PROPERTY,
						PropertyArity.LIST, _pagesList);
	...
	protected Property buildPageProperty(PageObject page, int idx, RepInfo info)
		...
		try {
			...
			if (plProp == null || nominalNum[0] != idx + 1) {
				...
				pagePropList.add(new Property("Sequence", PropertyType.INTEGER,
						new Integer(idx + 1)));
		...
		try {
			...
			if (annots != null) {
				...
				for (int i = 0; i < contents.size(); i++) {
					PdfObject annot = resolveIndirectObject((PdfObject) contents
							.elementAt(i));
					if (annot instanceof PdfDictionary) {
						annotsList.add(buildAnnotProperty(
								(PdfDictionary) annot, info));
					} else {
		...
		try {
			...
			if (rot != null && rot.getIntValue() != 0) {
				pagePropList.add(new Property("Rotate", PropertyType.INTEGER,
			...
			if (uu != null) {
				pagePropList.add(new Property("UserUnit", PropertyType.DOUBLE,
			...
			if (thumb != null) {
				pagePropList.add(new Property("Thumb", PropertyType.BOOLEAN,
						Boolean.TRUE));
			}
			return new Property("Page", PropertyType.PROPERTY,
	...
	protected Property buildPageLabelProperty(PageObject page, int pageIndex,
		...
		try {
			...
			PdfDictionary pageLabelDict = (PdfDictionary) resolveIndirectObject(_pageLabelRoot
					.getPrevValue());
			...
			return new Property("Label", PropertyType.STRING,
					labelText.toString());
		} catch (Exception e) {
	...
	protected Property buildMeasureProperty(PdfDictionary meas) {
		...
		if (itemObj instanceof PdfSimpleObject) {
			plist.add(new Property("Subtype", PropertyType.STRING,
					((PdfSimpleObject) itemObj).getStringValue()));
		}
		itemObj = meas.get("R");
		if (itemObj instanceof PdfSimpleObject) {
			plist.add(new Property("Ratio", PropertyType.STRING,
					((PdfSimpleObject) itemObj).getStringValue()));
		}
		...
		return new Property("Measure", PropertyType.PROPERTY,
	...
	protected Property buildAnnotProperty(PdfDictionary annot, RepInfo info)
		...
		try {
			...
			propList.add(new Property("Subtype", PropertyType.STRING,
					((PdfSimpleObject) itemObj).getStringValue()));
			...
			if (itemObj != null) {
				propList.add(new Property("Contents", PropertyType.STRING,
						_encrypted ? ENCRYPTED : ((PdfSimpleObject) itemObj)
								.getStringValue()));
			}
			...
			propList.add(makeRectProperty(
					(PdfArray) resolveIndirectObject(itemObj), "Rect"));

			// Name comes from the NM entry and is optional
			itemObj = annot.get("NM");
			if (itemObj != null) {
				propList.add(new Property("Name", PropertyType.STRING,
						((PdfSimpleObject) itemObj).getStringValue()));
			}
			...
			if (itemObj != null) {
				Literal lastModLit = (Literal) ((PdfSimpleObject) itemObj)
						.getToken();
				Property dateProp;
				dateProp = new Property("LastModified", PropertyType.STRING,
						lastModLit.getValue());
			...
			if (itemObj != null) {
				...
				Property flagProp = (buildBitmaskProperty(flagValue, "Flags",
						PdfStrings.ANNOTATIONFLAGS, "No flags set"));
				if (flagProp != null) {
			...
			if (itemObj != null) {
				...
				PdfSimpleObject annType = (PdfSimpleObject) ((PdfDictionary) itemObj)
						.get("S");
				if (annType == null) {
			...
			if (itemObj instanceof PdfSimpleObject) {
				...
				propList.add(new Property("ReplyType", PropertyType.STRING,
						type));
			}
			...
			if (itemObj instanceof PdfArray) {
				...
				propList.add(new Property("CalloutLine", PropertyType.DOUBLE,
						PropertyArity.LIST, clList));
			}
	...
	protected void addDestination(PdfObject itemObj, String propName,
			List<Property> propList, RepInfo info) throws PdfException {
		try {
			...
			if (dest.isIndirect()) {
				...
				Integer destPg = (Integer) _pageSeqMap.get(new Integer(
						pageObjNum));
				if (destPg != null) {
	...
	protected Property buildFontProperty(String name, Map map, int fontType) {
		...
		while (fontIter.hasNext()) {
			...
			Property fProp = new Property("Font", PropertyType.PROPERTY,
	...
	protected Property buildCMapDictProperty(PdfStream encoding) {
		...
		Property prop = new Property("CMapDictionary", PropertyType.PROPERTY,
				PropertyArity.LIST, propList);
		Property subprop;
		...
		if (!cidList.isEmpty()) {
			subprop = new Property("CIDSystemInfos", PropertyType.PROPERTY,
	...
	protected Property buildCIDInfoProperty(PdfDictionary dict) {
		...
		Property prop = new Property("CIDSystemInfo", PropertyType.PROPERTY,
				PropertyArity.LIST, propList);
		Property subprop;
		...
		if (supp instanceof PdfSimpleObject) {
			try {
				...
				subprop = new Property("Supplement", PropertyType.INTEGER,
	...
	protected Property buildEncodingDictProperty(PdfDictionary encodingDict) {
		...
		Property diffsProp = new Property("Differences", PropertyType.BOOLEAN,
	...
	protected Property buildFontDescriptorProperty(PdfDictionary encodingDict) {
		...
		Property prop = new Property("FontDescriptor", PropertyType.PROPERTY,
				PropertyArity.LIST, propList);
		Property subprop;
		try {
		...
		try {
			...
			subprop = buildBitmaskProperty(flagValue, "Flags",
					PdfStrings.FONTDESCFLAGS, "No flags set");
			if (subprop != null) {
				propList.add(subprop);
			}
		} catch (Exception e) {
		...
		try {
			...
			if (bbox != null) {
				...
				subprop = new Property("FontBBox", PropertyType.INTEGER,
						PropertyArity.ARRAY, ibbox);
				propList.add(subprop);
			}
		} catch (Exception e) {
			// TODO : Add exception handling
	...
	protected Property buildViewPrefProperty(PdfDictionary prefDict) {
		...
		p = new Property("DisplayDocTitle", PropertyType.BOOLEAN,
				new Boolean(b));
		propList.add(p);
	...
	protected Property buildOutlinesProperty(PdfDictionary dict, RepInfo info)
		...
		Property prop = new Property("Outlines", PropertyType.PROPERTY,
	...
	protected Property buildOutlineItemProperty(PdfDictionary dict, RepInfo info)
		...
		try {
			Property prop = new Property("Item", PropertyType.PROPERTY,
					PropertyArity.LIST, itemList);
			PdfSimpleObject title = (PdfSimpleObject) resolveIndirectObject(dict
					.get("Title"));
			if (title == null) {
			...
			if (cnt != null
					&& (!(cnt instanceof PdfSimpleObject) || !(((PdfSimpleObject) cnt)
			...
			PdfDictionary child = (PdfDictionary) resolveIndirectObject(dict
					.get("First"));
			if (child != null) {
				...
				while (child != null) {
					...
					if (_visitedOutlineNodes.contains(onum)) {
						...
						Property p = buildOutlineItemProperty(
								(PdfDictionary) child, info);
						childList.add(p);
					}
					child = (PdfDictionary) resolveIndirectObject(child
							.get("Next"));
					if (child == null) {
						break;
					}
	...
	protected int resolveIndirectDest(PdfSimpleObject key) throws PdfException {
		if (_destNames != null) {
			Destination dest = new Destination(
					_destNames.get(key.getRawBytes()), this, true);
			// if (dest == null) {
			// return -1;
			// }
		...
		if (propObject instanceof PdfSimpleObject) {
			...
			if (tok instanceof Literal) {
				...
				if (propDate != null) {
					...
					throw new PdfInvalidException("Improperly formed date", 0);
				}
			}
		}
	}
