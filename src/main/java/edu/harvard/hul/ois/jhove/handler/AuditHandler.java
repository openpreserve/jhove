...
/**
 * JHOVE audit output handler, derived from the standard JHOVE XML handler. It
 * is expected that this class will be used as the parent for other, more
 * interesting output handlers. Subclasses should override the implementations
 * of the Impl methods, e.g., endDirectoryImpl ().
 * 
 * @see <a href="http://hul.harvard.edu/jhove/using.html#xml-hul">JHOVE XML
 *      output handler</a>
 */
public class AuditHandler extends XmlHandler {
	...
	private static final String NOTE = "This output handler is derived from the standard JHOVE XML output "
	...
	private static final String RIGHTS = "Copyright 2004-2005 by the President and Fellows of Harvard College. "
	...
	/**
	 * Callback indicating a directory is finished being processed. Prop the
	...
	/**
	 * Local extension to standard callback that determines whether or not to
	 * process the file.
	 * 
	 * @param filepath
	 *            File pathname
	...
	public void show(RepInfo info) {
		...
		/*
		 * If the file is not found, then no module is assigned in the RepInfo
		 * object.
		 */
	...
	public void showImpl(RepInfo info, AuditState state) {
		...
		/*
		 * Change the URI to a relative path by removing the home directory
		 * prefix.
		 */
	...
	public void showFooter() {
		...
		_writer.println("Total: " + nTotal + " (" + nValid + "," + nWellFormed
				+ ")");
		_writer.println("-->");
		...
		while (iter.hasNext()) {
			...
			_writer.println(directory + ": " + total + " (" + valid + ","
					+ wellFormed + ") + " + notProcessed + "," + notFound);
		...
		_writer.println("<!-- Elapsed time: " + hh + ":" + (mm > 9 ? "" : "0")
				+ mm + ":" + (ss > 9 ? "" : "0") + ss + " -->");
		_writer.flush();
	}
	...
	public void showHeader() {
		...
		/*
		 * Instantiate a state object and initialize with the values of the
	 ...
	 *
	 * Additional state information can be added to the AuditState object in the
	 * showHeaderImpl() method before it is pushed onto the stack.
	 */
	@Override
	public void startDirectory(String directory) {
		try {
			AuditState state = (AuditState) ((AuditState) _stateStack.peek())
					.clone(directory);
