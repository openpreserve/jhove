...
public class XmlModule extends ModuleBase {
	...
	/*
	 * According to RFC 3023, text/xml should be used for human-readable XML
	 * documents, and application/xml should be used for documents that aren't
	 * easily read by humans. Since that determination is beyond the scope of
	 * this project, we err on the side of pessimism and use application/xml as
	 * the primary MIME type. <code>MIMETYPE[2]</code> is only for XHTML.
	 */
	...
	/*
	 * Flag to control signature checking behavior. If true, checkSignatures
	 * insists on an XML document declaration; if false, it will parse the file
	 * if there is no document declaration.
	 */
	protected boolean _sigWantsDecl;

	/*
	 * Flag to indicate we're invoking the parser from checkSignatures. When
	 * true, it's up to checkSignatures to mark a signature as present.
	 */
	...
	public XmlModule() {
		...
		agent.setAddress("Massachusetts Institute of Technology, "
				...
				+ "32 Vassar Street, Room 32-G515, " + "Cambridge, MA 02139");
	...
	/**
	 * Sets the value of the doctype string, assumed to have been forced to
	 * upper case. This is set only when the HTML module invokes the XML module
	 ...
	 *            parameter. If the parameter starts with "schema", then the
	 *            part to the right of the equal sign identifies a URI with a
	 *            local path (URI, then semicolon, then path). If the first
	 *            character is 's' and the parameter isn't "schema", then
	 *            signature checking requires a document declaration, and the
	 *            rest of the URL is considered as follows. If the parameter
	 *            begins with 'b' or 'B', then the remainder of the parameter is
	 *            used as a base URL. Otherwise it is ignored, and there is no
	 *            base URL.
	 */
	@Override
	...
	/**
	 * Parse the content of a purported XML digital object and store the results
	 * in RepInfo.
	 * 
	 * This is designed to be called in two passes. On the first pass, a
	 * nonvalidating parse is done. If this succeeds, and the presence of DTD's
	 * or schemas is detected, then parse returns 1 so that it will be called
	 * again to do a validating parse. If there is nothing to validate, we
	 * consider it "valid."
	 *
	 * @param stream
	...
	public int parse(InputStream stream, RepInfo info, int parseIndex)
		...
		try {
			...
			info.setMessage(new ErrorMessage("File not found", ef.getMessage()
					.toString()));
			...
			info.setMessage(new ErrorMessage(e.getClass().getName() + ": "
			...
			if (msg == null) {
				...
				if (ee != null) {
					msg = "SAXException, cause = " + ee.getClass().getName();
				} else {
		...
		_propList.add(new Property("Parser", PropertyType.STRING, parser
				.getClass().getName()));
		...
		if (!schemaList.isEmpty()) {
			...
			List<Property> schemaPropList = new ArrayList<Property>(
					schemaList.size());
		...
		if (!uent.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				if (nameInCollection(name, attributeVals)) {
					...
					subPropList.add(new Property("Name", PropertyType.STRING,
							name));
		...
		if (!pi.isEmpty()) {
			...
			while (pii.hasNext()) {
				...
				subPropArr[1] = new Property("Data", PropertyType.STRING,
						pistr.data);
				piPropList
						.add(new Property("ProcessingInstruction",
								PropertyType.PROPERTY, PropertyArity.ARRAY,
								subPropArr));
			}
		...
		if (!notations.isEmpty()) {
			...
			if (!notProps.isEmpty()) {
				notationsProp = new Property("Notations",
						PropertyType.PROPERTY, PropertyArity.LIST, notProps);
			}
		}
	 ...
	 * information.
	 * 
	 * XML is a particularly messy case; in general, there's no even moderately
	 * good way to check "signatures" without parsing the whole file, since the
	 * document declaration is optional. We provide the user two choices, based
	 * on the "s" parameter. If 's' is the first character of the module
	 * parameter, then we look for an XML document declaration, and say there's
	 * no signature if it's missing. (This can reject well-formed XML files,
	 * though not valid ones.) Otherwise, if there's no document declaration, we
	...
	/*
	 * Converts an int to a 4-digit hex value, e.g., 003F or F10A. This is used
	...
	/**
	 * Add a mapping from a schema URI to a local file. The parameter is of the
	 * form schema=[URI];[path]
	 */
