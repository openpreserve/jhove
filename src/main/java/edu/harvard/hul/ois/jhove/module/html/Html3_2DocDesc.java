...
public class Html3_2DocDesc extends HtmlDocDesc {
	...
	/*
	 * Static initializer. A superclass is initialized before its subclass, so
	 * we can count on the static initializer of HtmlDocDesc to have run
	 * already.
	 * 
	 * It's time to start thinking about how to factor this code. Each element
	 * can be created separately, with the necessary arguments passed for each
	 * one. It would be a nice pattern if all elements had the same calling
	 * sequence, but realistically some are going to need extras such as special
	 * lists of names. The element functions (which will all be static) should
	 * be here if unique, or in the parent class if they can be used for more
	 * than one version of HTML. There should be a naming convention for the
	 * functions in the parent class indicating which names they can be used
	 * with.
	 */
	static {
		stSupportedElements = new HashMap(280);
		String[] fontMarkup = new String[] { "tt", "i", "b", "u", "strike",
				"big", "small", "sub", "sup" };
		String[] phraseMarkup = new String[] { "em", "strong", "dfn", "code",
				"samp", "kbd", "var", "cite" };
		String[] specialMarkup = new String[] { "a", "img", "applet", "font",
				"basefont", "br", "script", "map" };
		String[] formMarkup = new String[] { "input", "select", "textarea" };
		String[] listMarkup = new String[] { "ul", "ol", "dir", "menu" };

		/*
		 * textContent lists all the content types which are permitted in the
		 * markup elements. For a first cut, strings signify the name of the
		 * element which is permitted.
		 */
		...
		String[] blockMisc = new String[] { "p", "pre", "dl", "div", "center",
		...
		HtmlAttributeDesc halignAtt = new HtmlAttributeDesc("align",
				new String[] { "left", "center", "right" },
				HtmlAttributeDesc.IMPLIED);
		/*
		 * Caution -- some elements' align attributes have a different set of
		 * permitted values. Don't use halignAtt for these.
		 */
		...
		addOptionElement();
		addTableElement();
		HtmlAttributeDesc valignAtt = new HtmlAttributeDesc("valign",
	...
	/**
	 * Static initializers for each element. If elements are common to more than
	 * one HTML version, they should be moved into the superclass. Different
	 * initializers may have different argument lists.
	 */
	...
	private static void addAppletElement(HtmlAttributeDesc ialignAtt,
			List textContent) {
		String name = "applet";
		List atts = new ArrayList(9);
		addSimpleAttribute(atts, "codebase");
		atts.add(new HtmlAttributeDesc("code", null, HtmlAttributeDesc.REQUIRED));
		addSimpleAttribute(atts, "alt");
		addSimpleAttribute(atts, "name");
		atts.add(new HtmlAttributeDesc("width", null,
				HtmlAttributeDesc.REQUIRED));
		atts.add(new HtmlAttributeDesc("height", null,
				HtmlAttributeDesc.REQUIRED));
		atts.add(ialignAtt);
	...
	private static void addAreaElement() {
		...
		atts.add(new HtmlAttributeDesc("shape", new String[] { "rect",
				"circle", "poly" }, HtmlAttributeDesc.REQUIRED));
		addSimpleAttribute(atts, "coords");
		addSimpleAttribute(atts, "href");
		atts.add(new HtmlAttributeDesc("nohref", new String[] { "nohref" },
				HtmlAttributeDesc.IMPLIED));
		atts.add(new HtmlAttributeDesc("alt", null, HtmlAttributeDesc.REQUIRED));
	...
	private static void addBodyElement(List bodyContent) {
		String name = "body";
		HtmlTagDesc td = new HtmlTagDesc(name, true, true, bodyContent, null);
		td.setAttributes(new String[] { "bgcolor", "text", "link", "vlink",
				"alink", "background" });
		stSupportedElements.put(name, td);
	...
	private static void addDivElement(List bodyContent,
	...
	private static void addFormElement(List bodyContent) {
		...
		atts.add(new HtmlAttributeDesc("method", null, HtmlAttributeDesc.OTHER));
		atts.add(new HtmlAttributeDesc("enctype", null, HtmlAttributeDesc.OTHER));
	...
	private static void addHeadElement() {
		 ...
		 * are supposed to come in that order, ahead of anything else. For the
		 * moment, just toss them in with the rest.
		 */
	...
	private static void addInputElement(HtmlAttributeDesc ialignAtt) {
		...
		atts.add(new HtmlAttributeDesc("type", new String[] { "text",
				"password", "checkbox", "radio", "submit", "reset", "file",
				"hidden", "image" }, HtmlAttributeDesc.OTHER));
		addSimpleAttribute(atts, "name");
	...
	private static void addPElement(HtmlAttributeDesc halignAtt,
			List textContent) {
		final String name = "p";
		List atts = new ArrayList(1);
		atts.add(halignAtt);
	...
	private static void addPreElement(List textContent) {
		...
		removeStringsFromList(preContent, new String[] { "img", "big", "small",
				"sub", "sup", "font" });
	...
	private static void addTableElement() {
		...
		List atts = new ArrayList(5);
		atts.add(new HtmlAttributeDesc("align", new String[] { "left",
				"center", "right" }, HtmlAttributeDesc.IMPLIED));
		addSimpleAttribute(atts, "width");
		addSimpleAttribute(atts, "border");
	...
	private static void addTdElement(List bodyContent, List thtdAtts) {
		...
		HtmlTagDesc td = new HtmlTagDesc(name, true, false, bodyContent,
				thtdAtts);
		stSupportedElements.put(name, td);
	}
	...
	private static void addTitleElement() {
		/*
		 * I'm confused by the DTD for this one. Content consists only of
		 * PCDATA, but certain elements are specifically excluded from its
		 * content. This seems redundant.
		 */
		String name = "title";
	...
	private static void addTrElement(HtmlAttributeDesc halignAtt,
			HtmlAttributeDesc valignAtt) {
