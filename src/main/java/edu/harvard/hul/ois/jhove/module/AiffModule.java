...
/**
 * Module for identification and validation of AIFF files. Supports AIFF and
 * AIFF-C.
 * 
 * @author Gary McGath
 */
public class AiffModule extends ModuleBase {
	...
	/* Values for fileType */
	public final static int AIFFTYPE = 1, AIFCTYPE = 2;
	...
	private static final String[] MIMETYPE = { "audio/x-aiff",
			"application/aiff" };
	private static final String WELLFORMED = "Magic number: \"FORM\" at byte offset 0; \"AIFF\""
	...
	private static final String REPINFO = "Properties capturing the technical attributes of the audio "
			+ "from all chunks";
	...
	public AiffModule() {
		...
		sig = new ExternalSignature(".aifc", SignatureType.EXTENSION,
				SignatureUseType.OPTIONAL, "For AIFF-C profile");
		_signature.add(sig);
		...
		sig = new InternalSignature("AIFF", SignatureType.MAGIC,
				SignatureUseType.OPTIONAL, 8, "For AIFF profile");
		_signature.add(sig);

		sig = new InternalSignature("AIFC", SignatureType.MAGIC,
				SignatureUseType.OPTIONAL, 0, "For AIFF-C profile");
		_signature.add(sig);
	...
	public int parse(InputStream stream, RepInfo info, int parseIndex)
		...
		try {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (ch != sigByte[i]) {
					info.setMessage(new ErrorMessage(
							"Document does not start with AIFF FORM Chunk", 0));
			...
			info.setWellFormed(RepInfo.FALSE);
			info.setMessage(new ErrorMessage("Unexpected EOF", _nByte));
			return 0;
		}
		...
		_aesMetadata
				.setByteOrder(thisFileBigEndian ? AESAudioMetadata.BIG_ENDIAN
		...
		if (!_annotationList.isEmpty()) {
			_propList.add(new Property("Annotations", PropertyType.PROPERTY,
					PropertyArity.LIST, _annotationList));
		}
		if (!_midiList.isEmpty()) {
			_propList.add(new Property("MIDIData", PropertyType.PROPERTY,
					PropertyArity.LIST, _midiList));
		}
		if (!_saxelList.isEmpty()) {
			_propList.add(new Property("Saxels", PropertyType.PROPERTY,
					PropertyArity.LIST, _saxelList));
		}
	...
	/**
	 * Sets the endian-ness. <code>true</code> indicates big-endian, and
	 * <code>false</code> means little-endian. This is needed because chunk data
	 * can change the usual little-endian byte order to big-endian.
	 */
	...
	/**
	 * Adds an Annotation Property to the annotation list. This will get put
	 * into an Annotations Property.
	 */
	...
	/**
	 * Adds a Saxel Property to the saxel list. This will get put into a Saxels
	 * Property.
	 */
	...
	/**
	 * Adds a MIDI Property to the MIDI list. This will get put into a MIDIData
	 * Property.
	 */
	...
	protected void initParse() {
		...
		_metadata = new Property("AIFFMetadata", PropertyType.PROPERTY,
	...
	/**
	 * Reads a Pascal string. A Pascal string is one whose count is given in the
	 * first byte. The count is exclusive of the count byte itself. A Pascal
	 * string can have a maximum of 255 characters. If the count of a Pascal
	 * string is even (meaning the total number of bytes is odd), there will be
	...
	/**
	 * Converts a Macintosh-style timestamp (seconds since January 1, 1904) into
	 * a Java date. The timestamp is treated as a time in the default
	 * localization. Depending on that localization, there may be some variation
	 * in the exact hour of the date returned, e.g., due to daylight savings
	 * time.
	 * 
	 */
	 ...
	 * already been marked. The SSND chunk offset value must be added to the
	 * current byte offset for a correct value.
	 */
	...
	/**
	 * Reads the file type. Broken out from parse(). If it is not a valid file
	 * type, returns false.
	 */
