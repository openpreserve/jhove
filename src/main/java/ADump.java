			...
			while (!endOfFile) {
				...
				try {
					...
					if (ckID.equals("AESD")) {
						...
						alreadyRead = 24;
					} else if (ckID.equals("ANNO") || ckID.equals("AUTH")
							|| ckID.equals("(c) ") || ckID.equals("NAME")) {
						sbuf.setLength(0);
						...
						int numChannels = ModuleBase.readSignedShort(stream,
								ENDIAN);
						long numSampleFrames = ModuleBase.readUnsignedInt(
								stream, ENDIAN);
						...
						long offset = ModuleBase
								.readUnsignedInt(stream, ENDIAN);
