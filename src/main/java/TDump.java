...
public class TDump extends Dump {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((offset = readIFD(file, bigEndian, offset, nobyte, nosub)) > 0) {
				// Loop through to the offset position
			}
			file.close();
		} catch (Exception e) {
	 ...
	 *            Byte offset of IFD
	 * @param nobyte
	 *            If true, only display the first 8 bytes of data of type BYTE
	 * @param nosub
	 *            If true, do not parse subIFDs
	 */
	private static long readIFD(RandomAccessFile file, boolean bigEndian,
			long offset, boolean nobyte, boolean nosub) throws Exception {
		int nIFD = ++_nIFDs;
		List<Long> subIFDs = new ArrayList<Long>();
		...
		for (int i = 0; i < nEntries; i++) {
			...
			StringBuffer buffer = new StringBuffer(tag + " ("
					+ TiffTags.tagName(tag) + ") " + IFD.TYPE[type] + " "
					+ count);
			if (type == IFD.ASCII) {
				if (count > 4) {
