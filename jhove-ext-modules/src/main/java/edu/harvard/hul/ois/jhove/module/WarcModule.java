...
/**
 * JHOVE module for identifying, validating and characterizing WARC files. Ported from the JHOVE2
 * WARC module and based on the JWAT-tool, both created by nicl@kb.dk (nclarkekb@git).
 *
 * <p>This is a non-recursive validation. It only validates the WARC file format and WARC headers,
 * not the actual payload of the WARC records.
 *
 * @author jolf@kb.dk
 */
public class WarcModule extends ModuleBase {
  ...
  /**
   * Map of the WARC record versions and their count. Used for reporting the most seen version, as
   * the version for the WARC file.
   */
  private Map<String, Integer> versions;
  /**
   * List of Property elements for the records of the WARC-file. Each Property contains a map of all
   * properties for a given record.
   */
  ...
  private void setVendorAndSpecification() {
    ...
    Signature sig =
        new ExternalSignature(".warc", SignatureType.EXTENSION, SignatureUseType.OPTIONAL);
    _signature.add(sig);
    sig =
        new ExternalSignature(
            ".warc.gz", SignatureType.EXTENSION, SignatureUseType.OPTIONAL, "when compressed");
    _signature.add(sig);
  }
  ...
  protected void setReaderOptions(WarcReader reader) throws JhoveException {
    ...
    if (!reader.setPayloadDigestAlgorithm(payloadDigestAlgorithm)) {
      throw new JhoveException(
          MessageConstants.ERR_PAYLOAD_DIGEST_INVALID + payloadDigestAlgorithm);
    }
  ...
  /**
   * Parse WARC records. Parsing should be straight forward with all records accessible through the
   * same source.
   *
  ...
  /**
   * Process a WARC record. Does not characterize the record payload.
   *
  ...
  private void reportResults(WarcReader reader, RepInfo repInfo) {
    ...
    repInfo.setProperty(
        new Property("Records", PropertyType.PROPERTY, PropertyArity.LIST, recordProperties));
