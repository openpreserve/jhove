...
/**
 * JHOVE module for identifying, validating and characterizing GZIP files. Ported from the JHOVE2
 * GZIP module, created by lbihanic, selghissassi, nicl
 *
 * <p>JHOVE2 GZip module. This module parses and validates GZip files in compliance with <a
 * href="http://www.ietf.org/rfc/rfc1952.txt">RFC 1952</a> (GZIP file format specification version
 * 4.3) and supports multiple member GZIP files.
 *
 * <p>This is a non-recursive validation. It only validates the GZIP file format, not the actual
 * content within the WARC records.
 *
 * @author jolf@kb.dk
 */
public class GzipModule extends ModuleBase {
  /*------------ MODULE DEFINITIONS ---------------*/
  private static final Agent KB_AGENT =
      new Builder("Royal Library of Denmark", AgentType.STANDARD)
  ...
  private static final String RIGHTS =
      "Copyright 2015 by The Royal Library of Denmark. "
          + "Released under the GNU Lesser General Public License.";

  private static final String EXTENSION = ".gz";

  /**
   * List of Property elements for the entry of the GZIP-file. Each Property contains a map of all
   * properties for a given entry.
   */
  ...
  private void setVendorAndSpecification() {
    ...
    Signature sig =
        new ExternalSignature(EXTENSION, SignatureType.EXTENSION, SignatureUseType.OPTIONAL);
    _signature.add(sig);
  }
  ...
  /** Resets parameter settings. Returns to a default state without any parameters. */
  @Override
  ...
  public void checkSignatures(File file, InputStream stream, RepInfo info) throws IOException {
    ...
    boolean checkIsGzip =
        GzipReader.isGzipped(
            new ByteCountingPushBackInputStream(stream, GzipReader.DEFAULT_INPUT_BUFFER_SIZE));
    if (checkIsGzip) {
  ...
  /**
   * Parses GZIP entries. Parsing should be straight forward with all records accessible through the
   * same source.
   *
  ...
  /**
   * Processes a GZIP entry. Extracts all the properties of the entry into a map, and puts this map
   * on the list.
   *
  ...
  protected void processEntry(GzipEntry entry) throws EOFException, IOException {
    ...
    Property p =
        new Property("Record", PropertyType.STRING, PropertyArity.MAP, properties.getProperties());
  ...
  private void reportResults(GzipReader reader, RepInfo repInfo) {
    ...
    repInfo.setProperty(
        new Property("Records", PropertyType.PROPERTY, PropertyArity.LIST, entryProperties));
