 ...
 *
 * <p>This class is a wrapper for the information available in an WARC record. Since the WARC reader
 * is not persistent its data must be moved to a simpler data class which can be persisted instead.
 *
 * <p>Note: Some populate methods currently do not include any functionality. However they are
 * included for backwards compatibility in case the ISO standard changes and extra properties are
 * required.
 *
 * @author nicl
 */
public class WarcRecordData {
  ...
  /**
   * Constructs an object using the data in the <code>WarcRecord</code> object.
   *
