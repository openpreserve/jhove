...
public class IhdrChunk extends PNGChunk {
  ...
  private static final int COLOR_TRUE = 2; // RGB
  private static final int COLOR_INDEXED = 3; // Palette
  ...
  private static final int[][] allowedBitDepths = {
  ...
  /**
   * The IHDR chunk contains image information in a fixed format. I don't think the spec says it
   * can't have extra bytes which would just be padding.
   */
  @Override
  public void processChunk(RepInfo info) throws Exception {
    ...
    if (ctErr || colorType == 1 || colorType == 5 || colorType > 6) {
      ErrorMessage msg = new ErrorMessage("Invalid color type " + colorType);
      info.setMessage(msg);
      info.setWellFormed(false);
      badChunk = true;
    } else {
      if (!colorAndDepthOK(colorType, bitDepth)) {
        ErrorMessage msg =
            new ErrorMessage(
                MessageConstants.PNG_GDM_26,
                String.format(MessageConstants.PNG_GDM_26_SUB.getMessage(), colorType, bitDepth));
        info.setMessage(msg);
        info.setWellFormed(false);
        badChunk = true;
      }
      _propList.add(new Property("ColorType", PropertyType.STRING, colorTypeNames[colorType]));
    }
    ...
    _propList.add(new Property("Compression", PropertyType.STRING, compressionStr));

    _propList.add(new Property("Filter type", PropertyType.INTEGER, Integer.valueOf(filter)));
    ...
    _propList.add(new Property("Interlace", PropertyType.STRING, interlaceStr));
    if (badChunk) {
