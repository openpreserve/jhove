 ...
 *
 * @author Gary McGath
 *     <p>An earlier PNG module was submitted by Gian Uberto Lauri. A few bits of code are copied
 *     from it. It validates files but doesn't do much metadata extraction. I noticed it only after
 *     making a significant start on my own version. -- GDM
 */
public class PngModule extends ModuleBase {
  ...
  private static final String RIGHTS =
      "Copyright 2016 by Gary McGath. " + "Released under the GNU Lesser General Public License.";
  ...
  public PngModule() {
    super(
        ...
        false);
    Signature sig =
        new InternalSignature(_sigBytes, SignatureType.MAGIC, SignatureUseType.MANDATORY, 0, "");
    _signature.add(sig);
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    _metadata = new Property("PNGMetadata", PropertyType.PROPERTY, PropertyArity.LIST, _propList);
    _nisoData = new NisoImageMetadata();
    Property nisoProp = new Property(NISO_IMAGE_MD, PropertyType.NISOIMAGEMETADATA, _nisoData);
    ...
    try {
      ...
      msg = new ErrorMessage(String.format(MessageConstants.PNG_GDM_69.getMessage(), _nByte));
      info.setMessage(msg);
      info.setWellFormed(false);
      return 0;
    } catch (Exception e) {
    ...
    if (!_keywordPropList.isEmpty()) {
      _propList.add(
          new Property("Keywords", PropertyType.PROPERTY, PropertyArity.LIST, _keywordPropList));
    }

    // Add the splat property list if it isn't empty.
    if (!_spltList.isEmpty()) {
      _propList.add(
          new Property("Suggested palettes", PropertyType.PROPERTY, PropertyArity.LIST, _spltList));
    }
  ...
  /**
   * This lets the module skip over the remainder of a chunk, not including the name, length,and
   * CRC. It updates the CRC.
   */
  ...
  /**
   * Add a keyword and value. Creating arbitrary properties on the fly doesn't go well with JHOVE's
   * approach, so we make each property a Map, with keys Keyword, Value, and optionally Language.
   */
  ...
  public void addSplt(String name, int sampleDepth, int numSamples) {
    List<Property> props = new ArrayList<>();
    Property prop =
        new Property("Suggested palette", PropertyType.PROPERTY, PropertyArity.LIST, props);
    props.add(new Property("Name", PropertyType.STRING, name));
    props.add(new Property("Sample depth", PropertyType.INTEGER, Integer.valueOf(sampleDepth)));
    props.add(new Property("Number of samples", PropertyType.INTEGER, Integer.valueOf(numSamples)));
    _spltList.add(prop);
  }
