...
public class SpltChunk extends PNGChunk {
  ...
  /**
   * Process the chunk. We add a property for the suggested palette by adding it to the module's
   * list of sPLT's.
   */
  @Override
  public void processChunk(RepInfo info) throws Exception {
    ...
    if (sampleDepth != 8 && sampleDepth != 16) {
      ErrorMessage msg =
          new ErrorMessage(
              MessageConstants.PNG_GDM_46.getMessage(),
              String.format(MessageConstants.PNG_GDM_46_SUB.getMessage(), sampleDepth));
      info.setMessage(msg);
      info.setWellFormed(false);
      throw new PNGException(MessageConstants.PNG_GDM_44);
    }
