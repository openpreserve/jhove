...
public abstract class ModuleBase implements Module {
	 ...
	 *            valid content
	 * @param repInfoNote
	 *            Note pertaining to RepInfo (may be null)
	 * @param note
	...
	protected ModuleBase(String name, String release, int[] date,
			String[] format, String coverage, String[] mimeType,
			String wellFormedNote, String validityNote, String repInfoNote,
			String note, String rights, boolean isRandomAccess) {
		...
		if (_signature.size() > 0) {
			...
			while (iter.hasNext()) {
				...
				if (sig instanceof InternalSignature) {
					...
					if (!match && isig.getUse()
							.equals(SignatureUseType.MANDATORY)) {
	...
	@SuppressWarnings("unused")
	@Override
	public void checkSignatures(File file, RandomAccessFile raf, RepInfo info)
			throws IOException {
		info.setFormat(_format[0]);
		info.setMimeType(_mimeType[0]);
		info.setModule(this);
	...
	protected static void setChecksums(Checksummer ckSummer, RepInfo info) {
		if (ckSummer != null) {
			info.setChecksum(
					new Checksum(ckSummer.getCRC32(), ChecksumType.CRC32));
