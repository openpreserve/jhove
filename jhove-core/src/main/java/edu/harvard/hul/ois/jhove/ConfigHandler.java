...
public class ConfigHandler extends org.xml.sax.helpers.DefaultHandler {
  ...
  /**
   * The list of handlers. Each element in the List is an array of two Strings representing the
   * class and the initialization string.
   */
  private List<String[]> _handler;

  /**
   * The list of handler parameters. Each element in the List is a List of Strings (which may be
   * empty but not null) representing parameters to be passed to the module. List elements are in
  ...
  /**
   * The list of modules. Each element in the List is an array of two Strings representing the class
   * and the initialization string.
   */
  protected List<ModuleInfo> _module;

  /**
   * The list of module parameters. Each element in the List is a List of Strings (which may be
   * empty but not null) representing parameters to be passed to the module. List elements are in
  ...
  /**
   * Returns the List of module parameters specified by the config file. Each element of the List is
   * a List (possibly empty) of Strings whose elements are parameters to pass to the module. The
   * values returned by <code>getModuleParams()</code> are in one-to-one correspondence with those
   * return by <code>getModule()</code>.
   */
   ...
   * handler. The values returned by <code>getHandlerParams()</code> are in one-to-one
   * correspondence with those return by <code>getHandler()</code>.
   */
  ...
  /**
   * Returns the MIX schema version specified by the config file. Acceptable values are "0.2" and
   * "1.0" and "2.0".
   */
  ...
  /**
   * Returns the number of bytes to examine when looking for an indefinitely positioned signature,
   * or checking the first sigBytes bytes of a file in lieu of a signature.
   */
  ...
  /**
   * Returns the name of the desired log level. This should be the name of one of the predefined
   * values of java.util.logging.Level, e.g., "WARNING", "INFO", "ALL". The default level is SEVERE.
   */
  ...
  /**
   * EntityResolver designed to locate the config schema. It tries to find it as a local resource.
   *
   * <p>It appears that not all SAX implementations will actually call this function for schema
