 ...
 * Validation Environment Copyright 2004-2005 by JSTOR and the President and Fellows of Harvard
 * College
 *
 * <p>********************************************************************
 */
...
public class AESAudioMetadata {
  ...
  /**
   * Public interface to the nested FaceRegion object. Instances of this should be created only by
   * appropriate methods, but can be accessed through the public methods of this interface.
   */
  ...
  class FaceImpl implements Face {
    ...
    /** Constructor. Initially the duration is set to null, indicating unknown value. */
    public FaceImpl() {
    ...
    /** Returns the starting time. Will be zero if not explicitly specified. */
    @Override
    public TimeDesc getStartTime() {
      return _startTime;
    }

    /** Returns the duration. May be null if the duration is unspecified. */
    @Override
    public TimeDesc getDuration() {
      return _duration;
    }
  ...
  class FaceRegionImpl implements FaceRegion {
    ...
    /** Returns the channel map locations. The array length will equal the number of channels. */
    @Override
    public String[] getMapLocations() {
  ...
  /**
   * Gets the list of Format Regions. Since one is created automatically on initialization, it's
   * possible that the list will contain a Format Region with only default values. This should be
   * checked with isEmpty ().
   */
  ...
  /**
   * Returns the names of the map locations. The returned value is an array whose length equals the
   * number of channels and whose elements correspond to channels 0, 1, etc.
   */
  public String[] getMapLocations() {
  ...
  /**
   * Returns the use (role of the document). The value returned is an array of two strings, the
   * useType and the otherType.
   */
  ...
  public void setBitrateReduction(
      ...
      String dataRate,
      String dataRateMode) {
    _curFormatRegion.setBitrateReduction(
        codecName,
        codecNameVersion,
  ...
  /**
   * Set the application-specific data. For present purposes, we assume this is representable as a
   * text string.
   */
  ...
  /**
   * Sets the direction. This must be one of the values FORWARD, REVERSE, A_WIND, B_WIND, C_WIND,
   * D_WIND, FRONT, BACK. FORWARD may be the only one that makes sense for digital formats.
   */
  public void setDirection(String direction) {
  ...
  /**
   * Sets the array of channel map locations. The length of the array must equal the number of
   * channels.
   */
  public void setMapLocations(String[] locations) {
  ...
  /**
   * Sets the primary identifier type. If the primary identifier type is OTHER, use
   * setOtherPrimaryIdentifierType instead.
   */
  ...
  /**
   * Sets the role of the document. Permitted values are ORIGINAL_MASTER, PRESERVATION_MASTER,
   * PRODUCTION_MASTER, SERVICE, PREVIEW, or OTHER. If useType is "OTHER", then otherType is
   * significant. Since OTHER is the only meaningful value for a digital document, the code assumes
   * this will always be the case and uses otherType.
   */
  ...
  /**
   * Adds a FormatRegion object to a FormatSize list. The most recently added FormatRegion object
   * will be filled in by setBitDepth, setSampleRate, and setWordSize.
   */
