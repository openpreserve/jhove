...
/**
 * An InputStream layered on top of a RandomAccessFile. This is useful for a Module which has
 * requirements that force it to use a RandomAccessFile, but is usually accessed sequentially.
 *
 * <p>An RAFInputStream maintains its own position information in the file, so multiple
 * RAFInputStreams in the same file will work without interference. However, this class is not
 * thread-safe.
 *
 * @author Gary McGath
 */
public class RAFInputStream extends InputStream {
   ...
   *
   * <p>The stream starts at the current position of the RandomAccessFile.
   *
   ...
   *
   * @param raf the file on which the stream is to be based.
   * @param bufferSize the buffer size to be used. If less than or equal to 0, the default buffer
   *     size is used.
   */
  ...
  /**
   * Seeks to the next offset and fills the file buffer. Sets <code>eof</code> to <code>true</code>
   * if the offset to read is beyond the end of file.
   */
   ...
   * stream without blocking. A single read or skip of this many bytes will not block, but may read
   * or skip fewer bytes.
   *
   * @return an estimate of the number of bytes that can be read (or skipped over) from this input
   *     stream without blocking, or <code>0</code> when it reaches the end of the input stream.
   */
  @Override
  ...
  /**
   * Returns the current position in the file. What is reported is the position of the byte in the
   * file which was last extracted from the buffer.
   */
  public long getFilePos() throws IOException {
    return _raf.getFilePointer() - (fileBufBytes - fileBufPos);
  }
}
