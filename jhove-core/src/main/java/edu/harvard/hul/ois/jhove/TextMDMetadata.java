...
/**
 * Encapsulation of the textMD metadata for text files. See http://www.loc.gov/standards/textMd for
 * more information.
 *
...
public class TextMDMetadata {
  ...
  public static final String[] BYTE_ORDER = {"big", "little", "middle"};
  ...
  public static final String[] LINEBREAK = {"CR", "LF", "CR/LF"};
  ...
  /**
   * charset Usage: The character set employed by the text. Controlled vocab using IANA names for
   * character sets. Attributes: none. Contains: none. Contained by: character_info.
   */
  private String charset;

  /**
   * byte_order Usage: Byte order, primarily useful for cases where it's not clear just by
   * specifying an IANA character set. Uses enumerated values of big, little, and middle' endian.
   * Attributes: none. Contains: none. Contained by: character_info.
   */
  private int byte_order = NILL;

  /**
   * byte_size Usage: The size of an individual byte within the expressed as a number of bits (as
   * integer). This does not necessarily equal the character size, as a character may have more than
   * one, or a variable number of bytes per character. Attributes: none. Contains: none. Contained
   * by: character_info.
   */
  private String byte_size;

  /**
   * character_size Usage: The size of an individual character within the character set as a number
   * of bytes of the size expressed in the byte_size. In the case of variable encodings, such as
   * UTF-8 for Unicode, the character_size element should state "variable" and also identify the
   * specific variable character set encoding in the encoding attribute. Attributes: encoding.
   * Contains: none. Contained by: character_info.
   */
  private String character_size;

  /**
   * linebreak Usage: How line breaks are represented in current file (which may differ from how
   * they were originally encoded). Either carriage return, line feed, or carriage return/line feed.
   * Attributes: none. Contains: none. Contained by: character_info.
   */
  private int linebreak = NILL;

  /**
   * language Usage: Language(s) used in work. Use ISO 639-2 codes, which are enumerated in the
   * schema as valid text values. Attributes: none. Contains: none. Contained by: textMD.
   */
  private String language;

  /**
   * markup_basis Usage: The metalanguage used to create the markup language, such as SGML, XML,
   * GML, etc. Attributes: version. Contains: none. Contained by: textMD.
   */
  private String markup_basis;
  /**
   * version Usage: Used to record the version number (as a string) for a given piece of software, a
   * markup language, or a schema version.
   */
  private String markup_basis_version;

  /**
   * markup_language Usage: Markup language employed on the text (i.e., the specific schema or dtd).
   * May be a URI for schema or dtd, but not mandatory. Attributes: version. Contains: none.
   * Contained by: textMD.
   */
  ...
  /**
   * Transform a given charset in the "authorized" list given in the textMD schema enumeration. From
   * the schema documentation on charset
   * (http://www.loc.gov/standards/textMD/elementSet/index.html#element_charset). The character set
   * employed by the text. Controlled vocab using IANA names for character sets:
   * http://www.iana.org/assignments/character-sets. The problem arises because the java Charset
   * uses the (preferred MIME name) where textMD uses the Name ...
   *
