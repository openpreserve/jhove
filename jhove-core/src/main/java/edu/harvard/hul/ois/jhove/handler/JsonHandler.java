...
public class JsonHandler extends HandlerBase {
  ...
  private static final String RIGHTS =
      "Version 1.0 release by Open Preservation Foundation. "
  ...
  public void show(App app) {
    ...
    for (String handlerKey : _je.getHandlerMap().keySet()) {
      ...
      oHandlersBuilder.add(
          Json.createObjectBuilder()
              .add("outputHandler", handler.getName())
              .add("release", handler.getRelease()));
    }
    ...
    if (module != null) {
      infoBuilder.add(
          ...
          Json.createObjectBuilder()
              ...
              .add("release", module.getRelease())
              .add("date", date.format(module.getDate())));
    }
  ...
  protected JsonObjectBuilder showChecksum(Checksum checksum) {
    return Json.createObjectBuilder()
        .add("checksum", checksum.getValue())
  ...
  protected JsonObjectBuilder showSignature(Signature signature) {
    ...
    if (SignatureType.MAGIC.equals(signature.getType())
      ...
      sigBuilder.add(
          "offset", "0x" + Integer.toHexString(((InternalSignature) signature).getOffset()));
    }
  ...
  protected JsonObjectBuilder showScalarProperty(Property property) {
    ...
    switch (propType) {
      ...
      case AESAUDIOMETADATA:
        propBuilder.add(
            property.getName(), showAESAudioMetadata((AESAudioMetadata) property.getValue()));
        break;
      case NISOIMAGEMETADATA:
        propBuilder.add(
            property.getName(), showNisoImageMetadata((NisoImageMetadata) property.getValue()));
        break;
      case TEXTMDMETADATA:
        propBuilder.add(
            property.getName(), showTextMDMetadata((TextMDMetadata) property.getValue()));
        break;
      case SHORT:
      ...
      if (rarray != null) {
        if (bMix10) {
          ...
          yccBuilder.add(
              ...
              Json.createObjectBuilder()
                  .add("mix:lumaRed", showRational(rarray[0]))
  ...
  protected JsonObjectBuilder showAESAudioMetadata(AESAudioMetadata aes) {
    ...
    if (in != AESAudioMetadata.NULL) {
      aesBuilder.add(
          "aes:byteOrder", (in == AESAudioMetadata.BIG_ENDIAN ? "BIG_ENDIAN" : "LITTLE_ENDIAN"));
    }
    ...
    if (facelist != null && !facelist.isEmpty()) {
      ...
      for (int ch = 0; ch < nchan; ch++) {
        ...
        streamsBuilder.add(
            Json.createObjectBuilder().add("aes:channelNum", ch).add("aes:mapLocation", locs[ch]));
      }
        ...
        if (bitRed != null) {
          formatRegionBuilder.add(
              ...
              Json.createObjectBuilder()
                  ...
                  .add("aes:codecCreatorApplicationVersion", bitRed[3])
    ...
    timerangeBuilder.add(
        ...
        Json.createObjectBuilder()
            ...
            .add(
                "tcf:samples",
                Json.createObjectBuilder()
                    .add("tcf:sampleRate", "S" + Integer.toString((int) sr))
                    .add("tcf:numberOfSamples", start.getSamples()))
            .add(
                "tcf:filmFraming",
                Json.createObjectBuilder()
                    .add("tcf:framing", "NOT_APPLICABLE")
