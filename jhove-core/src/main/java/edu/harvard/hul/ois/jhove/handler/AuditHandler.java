...
/**
 * JHOVE audit output handler, derived from the standard JHOVE XML handler. It is expected that this
 * class will be used as the parent for other, more interesting output handlers. Subclasses should
 * override the implementations of the Impl methods, e.g. endDirectoryImpl().
 */
public class AuditHandler extends XmlHandler {
  ...
  /**
   * Callback indicating a directory is finished being processed. Pop the state stack and place the
   * current directory file count into the directory hash.
   */
  @Override
  ...
  /**
   * Local extension to the standard callback that outputs the information contained in a RepInfo
   * object.
   *
   * @param info Object representation information
   */
  public void showImpl(RepInfo info) {
    ...
    switch (info.getWellFormed()) {
      case RepInfo.TRUE:
        if (info.getValid() == RepInfo.TRUE) status = "valid";
        else status = "well-formed";
        break;
  ...
  /**
   * Local extension to the standard callback that does the final output. This should be in a
   * suitable format for including multiple files between the header and the footer, and the XML of
   * the header and footer must balance out.
   */
  ...
  /**
   * Local extension to the standard callback that does the initial output. This should be in a
   * suitable format for including multiple files between the header and the footer, and the XML of
   * the header and footer must balance out.
   *
   ...
   *
   * <p>Additional state information can be added to the AuditState object in the showHeaderImpl()
   * method before it is pushed onto the stack.
   */
  @Override
  public void startDirectory(String directory) {
    try {
      AuditState state = (AuditState) _stateStack.peek().clone(directory);
