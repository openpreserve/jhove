...
/**
 * This class encapsulates XMP metadata within a file. It makes use of an InputStream as a data
 * source.
 *
 * <p>This differs from normal XML handling in that it's necessary to process the xpacket processing
 * instruction in order to determine the encoding of the XML. the processingInstruction function
 * looks for xpacket, and throws a special SAXException if it's necessary to change encoding.
 *
 * <p>We don't actually extract any information from the XMP, but simply check it for
 * well-formedness. By convention, XMPHandler should be invoked on an XMPSource (TBW), which
 * provides the ability to recapture the InputStream from which the XMP was obtained and put it into
 * a property once it's verified here.
 */
public class XMPHandler extends org.xml.sax.helpers.DefaultHandler {
  ...
  /**
   * Returns true if no violations of PDF/A compliance have been found, false if a problem was
   * detected.
   */
  ...
  public void endElement(String namespaceURI, String localName, String rawName) {
    if (xmpBasicSchema.equals(namespaceURI)) {
      ...
      if ("Bag".equals(rawName) || "Seq".equals(rawName) || "Alt".equals(rawName)) {}
    }
  }

  /**
   * Catch a fatal error. This is put here because the default behavior is to report a "fatal error"
   * to standard output, which is harmless but scary.
   */
  @Override
