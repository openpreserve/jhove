...
/**
 * This class encapsulates representation information, as defined by ISO/IEC 14721, about a content
 * stream.
 *
 * @see <a href="http://wwwclassic.ccsds.org/documents/pdf/CCSDS-650.0-B-1.pdf">ISO/IEC 14721
 *     (PDF)</a>
 */
public class RepInfo implements Cloneable {
  ...
  /** Validity flag. A ternary variable which can have a value of TRUE, FALSE, or UNDETERMINED. */
  private int _valid;

  /** Values for _valid */
  public static final int TRUE = 1, FALSE = 0, UNDETERMINED = -1;
  ...
  /**
   * Well-formed flag. A ternary variable which can have a value of TRUE, FALSE, or UNDETERMINED.
   */
   ...
   *
   * <p>By default, urlFlag is <code>false</code>.
   *
   * @param uri Object file pathname or URI
  ...
  /**
   * Clones the RepInfo one level deep, making fresh copies of the checksum, message, profile,
   * signature match, and property fields. The external RepInfo (if any) is not cloned, but is
   * attached directly to the clone.
   */
  @Override
  public Object clone() {
    RepInfo newri;
  ...
  /**
   * Copies all the information out of the parameter object. This is a "shallow" copy; it is assumed
   * that the parameter object is a temporary one that will not be further modified.
   */
  ...
  /**
   * Returns the creation date stored in this object. A creation date is not automatically
   * generated, but must be explicitly stored.
   */
  ...
  /**
   * Returns the last modified date stored in this object. A date is not automatically generated,
   * but must be explicitly stored.
   */
  ...
  /**
   * Returns the Property map stored in this object. The Property map contains key-value pairs whose
   * key is a <code>String</code> and whose value is a <code>Property</code>.
   */
  ...
  /** Returns a flag which, if <code>true</code>, indicates the object is a URL. */
  ...
  /**
   * Returns the list of matching signatures. JhoveBase will make this value persistent across
   * module invocations for a given document, so the list returned will reflect all modules that
   * have looked at the document so far.
   */
   ...
   *
   * @param wellFormed Boolean argument that maps to an integer value: true maps to TRUE, and false
   *     to FALSE.
   */
  ...
  /**
   * Set the wellFormed flag. Setting wellFormed to false forces the consistent and valid flags to
   * be false as well.
   */
   ...
   *
   * @param valid Boolean argument that maps to an integer value: true maps to TRUE, and false to
   *     FALSE.
   */
   ...
   * Set the validity flag
   *
   * @param valid Permitted values are TRUE, FALSE, AND UNDETERMINED. The effect of using other
   *     values is undefined.
   */
  ...
  /**
   * Adds the name of a module, signifying that the document signature matched the module's
   * requirements. JhoveBase will make this value persistent across module invocations for a given
   * document.
   */
  ...
  /**
   * Adds a list of module names, signifying that the document signature matched the module's
   * requirements. Any previous list is lost. JhoveBase will make this value persistent across
  ...
  /**
   * Output the information in this object. The format and destination of the output are determined
   * by the OutputHandler.
   */
