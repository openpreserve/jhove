 ...
 *
 * <p>More than one JhoveBase may be instantiated and process files in concurrent threads. Any one
...
public class JhoveBase {
   ...
   * Class constructor.
   *
   * <p>Instantiates a <code>JhoveBase</code> object.
   *
   ...
   *
   * <p>This method parses the configuration file and initialises the JHOVE engine based on the
   * values parsed.
   *
   * <p>Version 1.11 would create the configuration file if not found. Version 1.12 changes this
  ...
  public void init(String configFile, String saxClass) throws JhoveException {
    ...
    for (int i = 0; i < modList.size(); i++) {
      ...
      try {
        ...
        throw new JhoveException(CoreMessageConstants.EXC_MODL_INST_FAIL + modInfo.clas, e);
      }
    }
    ...
    for (int i = 0; i < hanList.size(); i++) {
      ...
      try {
        ...
        throw new JhoveException(CoreMessageConstants.EXC_HNDL_INST_FAIL + tuple[0], e);
      }
    }
  ...
  /**
   * Sets a callback object for tracking progress. By default, the callback is <code>null</code>.
   */
  ...
  /**
   * Processes a file or directory, or outputs information. If <code>dirFileOrUri</code> is null,
   * Does one of the following:
   *
   * <ul>
   *   <li>If module is non-null, provides information about the module.
   *   <li>Otherwise if <code>aboutHandler</code> is non-null, provides information about that
   *       handler.
  ...
  /**
   * Returns <code>false</code> if processing should be aborted. Calls itself recursively for
   * directories.
   */
   ...
   * by calling <code>abort</code>, in which case it will delete the temporary file and return
   * <code>null</code>.
   */
  ...
  /**
   * Aborts an activity. This simply sets a flag; whether anything is aborted depends on what
  ...
  /**
   * Processes the file. Returns <code>false</code> if aborted, or if the module is incapable of
  ...
  /**
   * Returns the maximum number of bytes to check, for modules that look for an indefinitely
   * positioned signature or check the first sigBytes bytes in lieu of a signature.
   */
  ...
  /**
   * Returns the directory designated for saving files. This is simply the directory most recently
   * set by <code>setSaveDirectory</code>.
   */
  ...
  /**
   * Returns <code>true</code> if raw output is requested. Raw output means numeric rather than
   ...
   * buffer size.
   *
   * <p>Any non-negative value less than 1024 will result in a buffer size of 1024.
   */
  ...
  /**
   * Sets the value to be returned by <code>getShowRawFlag</code>, which determines if only raw
   ...
   * subdirectory named by CONFIG_DIR under the home directory, and to be named <code>jhove.conf
   * </code>. Returns <code>null</code> if no such file is found.
   */
  ...
  /**
   * Returns the value of the property <code>edu.harvard.hul.ois.jhove.saxClass</code>, which should
   * be the name of the main SAX class. Returns <code>null</code> if no such property has been set
   * up.
   */
