...
/**
 * Immutable class that reads a properties file containing JHOVE release details. The version
 * number, build date and format are kept up to date by Maven, which filters the properties as part
...
public final class ReleaseDetails {
  private static final String APPLICATION_PROPERTIES_PATH =
      "org/openpreservation/jhove/jhove.properties";
  ...
  public boolean equals(Object obj) {
    ...
    } else if (!this.buildDate.equals(other.buildDate)) return false;
  ...
  private static ReleaseDetails fromPropertyResource(final String propertyResourceName) {
    Properties props = new Properties();
    try (InputStream is =
        ReleaseDetails.class.getClassLoader().getResourceAsStream(propertyResourceName)) {
      if (is == null) {
        throw new IllegalStateException(
            CoreMessageConstants.ERR_APP_PROP_MISS + propertyResourceName);
      }
      try {
        props.load(is);
      } catch (IOException e) {
        is.close();
        throw new IllegalStateException(
            CoreMessageConstants.ERR_APP_PROP_MISS + propertyResourceName, e);
      }
      is.close();
    } catch (IOException e) {
