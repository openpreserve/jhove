...
public class GDump extends Dump {
  ...
  public static void main(String[] args) {
    ...
    try (FileInputStream file = new FileInputStream(args[0]);
      ...
      System.out.println(
          "00000006: LogicalScreenDescriptor: "
              + width
              + "x"
              + height
              + " 0x"
      ...
      if (globalColorTable) {
        System.out.println(leading(os, 8) + os + ": GlobalColorTable:");
      ...
      for (int op = 0; (op = stream.readUnsignedByte()) != -1; ) {
        if (op == 0x00) {
          ...
          int imageLeft = ModuleBase.readUnsignedShort(stream, bigEndian, null);
          int imageTop = ModuleBase.readUnsignedShort(stream, bigEndian, null);
          int imageWidth = ModuleBase.readUnsignedShort(stream, bigEndian, null);
          int imageHeight = ModuleBase.readUnsignedShort(stream, bigEndian, null);
          ...
          if (localColorTable) {
            System.out.println(leading(os, 8) + os + ": LocalColorTable:");
            os = colorTable(stream, os, localColorTableSize);
          }

          int size = stream.readUnsignedByte();
          System.out.println(leading(os, 8) + os + ": ImageData: " + size);
          ...
          if (label == 0x01) {
            ...
            int gridLeft = ModuleBase.readUnsignedShort(stream, bigEndian, null);
            int gridTop = ModuleBase.readUnsignedShort(stream, bigEndian, null);
            int gridWidth = ModuleBase.readUnsignedShort(stream, bigEndian, null);
            int gridHeight = ModuleBase.readUnsignedShort(stream, bigEndian, null);
            ...
            int blockSize = stream.readUnsignedByte();
            packedFields = stream.readUnsignedByte();
            int delayTime = ModuleBase.readUnsignedShort(stream, bigEndian, null);
            ...
            System.out.println(leading(os, 8) + os + ": CommentExtension: \"" + "\"");
            os += 2;
            os = subBlocks(stream, os);
          } else if (label == 0xff) {
            int blockSize = stream.readUnsignedByte();
          ...
          System.out.println(leading(os, 8) + os + ": Trailer: 0x3b");
          os++;
          break;
        } else {
          String hex = Integer.toHexString(op);
          System.out.println(leading(os, 8) + os + ": Unknown block: 0x" + leading(hex, 2) + hex);
          os++;
        }
      }
      stream.close();
