...
public class GifModule extends ModuleBase {
  ...
  public GifModule() {
    ...
    Document doc =
        new Document(
            ...
            DocumentType.REPORT);
    Builder builder =
        new Agent.Builder("Compuserve Interactive Services Inc.", AgentType.COMMERCIAL)
            .address("5000 Arlington Centre Blvd., Columbus, OS 43220")
    ...
    doc.setIdentifier(
        new Identifier("http://www.w3.org/Graphics/GIF/spec-gif87.txt", IdentifierType.URL));
    ...
    doc.setDate("1987-06-15");
    doc.setIdentifier(
        new Identifier("http://www.w3.org/Graphics/GIF/spec-gif89a.txt", IdentifierType.URL));
    _specification.add(doc);

    Signature sig =
        new InternalSignature("GIF", SignatureType.MAGIC, SignatureUseType.MANDATORY, 0);
    _signature.add(sig);
    sig =
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    Property _blocks =
        new Property("Blocks", PropertyType.PROPERTY, PropertyArity.LIST, _blocksList);
  ...
  protected boolean readLSD(RepInfo info) throws IOException {
    ...
    propVec.add(
        new Property("BackgroundColorIndex", PropertyType.INTEGER, new Integer(bgColorIndex)));
    ...
    propVec.add(
        new Property("PixelAspectRatio", PropertyType.SHORT, new Short((short) pixAspectRatio)));
    propVec.add(
        addByteProperty(
    ...
    propVec.add(
        addByteProperty(
            "GlobalColorTableSortFlag", sortFlag ? 1 : 0, GifStrings.COLOR_TABLE_SORT_FLAG));
    propVec.add(
        new Property(
    ...
    if (_globalColorTableFlag) {
      ...
      _blocksList.add(
          new Property("GlobalColorTable", PropertyType.SHORT, PropertyArity.ARRAY, gctArray));
    }
    return true;
  }
  ...
  protected boolean readBlock(RepInfo info) throws IOException {
    ...
    try {
      ...
      info.setWellFormed(RepInfo.FALSE);
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_9, _nByte));
      return false;
    }
    try {
      switch (type) {
        ...
        default:
          info.setWellFormed(RepInfo.FALSE);
          info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_2, "Type = " + type, _nByte));
          return false;
      }
    } catch (EOFException e) {
  ...
  protected boolean readExtBlock(RepInfo info) throws IOException {
    ...
    switch (subtype) {
      ...
      default:
        info.setWellFormed(RepInfo.FALSE);
        info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_3, "Type = " + subtype, _nByte));
        return false;
    }
  }
  ...
  protected boolean readImage(RepInfo info) throws IOException {
    ...
    propVec.add(
        addByteProperty(
            "LocalColorTableFlag", localColorTableFlag, GifStrings.LOCAL_COLOR_TABLE_FLAG));
    int sortFlag = (packedFields & 0X20) >> 5;
    propVec.add(
        addByteProperty("LocalColorTableSortFlag", sortFlag, GifStrings.COLOR_TABLE_SORT_FLAG));
    int localColorTableSize = 0;
    int rawLocalColorTableSize = packedFields & 0X7;
    propVec.add(
        new Property(
            "LocalColorTableSize", PropertyType.SHORT, new Short((short) rawLocalColorTableSize)));
  ...
  protected boolean readAppExtension(RepInfo info) throws IOException {
    int blockSize = readUnsignedByte(_dstream, this);
    if (blockSize != 11) {
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_1, _nByte));
      info.setWellFormed(RepInfo.FALSE);
      return false;
    }
    ...
    propVec.add(
        new Property(
            "ApplicationAuthenticationCode", PropertyType.SHORT, PropertyArity.ARRAY, appAuth));

    int appDataSize = 0;
    // We are interested in the application extension for XMP.
    if (("XMP Data".equals(appIdent.toString())
            || (debug_appIdentCaseInsens && "xmp data".equalsIgnoreCase(appIdent.toString())))
        && appAuth[0] == (short) 'X'
        && appAuth[1] == (short) 'M'
        && appAuth[2] == (short) 'P') {
      appDataSize = readXMP();
    ...
    propVec.add(
        new Property("ApplicationDataSize", PropertyType.INTEGER, new Integer(appDataSize)));
  ...
  protected boolean readPlainTextExtension(RepInfo info) throws IOException {
    ...
    int blockSize = readUnsignedByte(_dstream, this);
    if (blockSize != 12) {
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_7, _nByte));
      info.setWellFormed(RepInfo.FALSE);
      return false;
    }
    ...
    if (!_globalColorTableFlag) {
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_8, _nByte));
      info.setValid(false);
    }
    ...
    propVec.add(
        new Property("CharacterCellWidth", PropertyType.SHORT, new Short((short) charCWidth)));

    int charCHeight = readUnsignedByte(_dstream, this);
    propVec.add(
        new Property("CharacterCellHeight", PropertyType.SHORT, new Short((short) charCHeight)));

    int textFgIdx = readUnsignedByte(_dstream, this);
    propVec.add(
        new Property("TextForegroundColorIndex", PropertyType.SHORT, new Short((short) textFgIdx)));

    int textBgIdx = readUnsignedByte(_dstream, this);
    propVec.add(
        new Property("TextBackgroundColorIndex", PropertyType.SHORT, new Short((short) textBgIdx)));
  ...
  protected boolean readGraphicsCtlBlock(RepInfo info) throws IOException {
    ...
    if (++_gceCounter > 1) {
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_5, _nByte));
      info.setWellFormed(RepInfo.FALSE);
    }
    int blockSize = readUnsignedByte(_dstream, this);
    if (blockSize != 4) {
      info.setMessage(new ErrorMessage(MessageConstants.GIF_HUL_6, _nByte));
      info.setWellFormed(RepInfo.FALSE);
      return false;
    }
    int packedFields = readUnsignedByte(_dstream, this);
    ...
    propVec.add(
        addByteProperty("TransparencyFlag", transparencyFlag, GifStrings.GCE_TRANSPARENCY_FLAG));
    ...
    propVec.add(
        new Property("TransparentColorIndex", PropertyType.SHORT, new Short((short) transIndex)));
