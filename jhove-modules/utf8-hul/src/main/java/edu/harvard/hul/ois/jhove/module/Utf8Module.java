...
public class Utf8Module extends ModuleBase {
  ...
  private static final String WELLFORMED =
      "An UTF-8 object is well-formed "
  ...
  private static final String RIGHTS =
      "Copyright 2003-2011 by JSTOR and "
  ...
  public Utf8Module() {
    ...
    Agent agent =
        new Agent.Builder("The Unicode Consortium", AgentType.NONPROFIT)
            .web("http://www.unicode.org/versions/Unicode7.0.0/")
            .address("Mountain View, California")
            .build();
    doc.setAuthor(agent);
    agent =
        new Agent.Builder("Addison-Wesley", AgentType.COMMERCIAL)
  ...
  public final int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    while (!eof) {
      try {
        ...
        if (0xc0 <= b[0] && b[0] <= 0xdf) {
          ...
          nBytes = 4;
        } else if ((0x80 <= b[0] && b[0] <= 0xbf) || (0xf8 <= b[0] && b[0] <= 0xff)) {
          ErrorMessage error =
              new ErrorMessage(
                  MessageConstants.UTF8_HUL_2,
                  "Value = " + ((char) b[0]) + " (0x" + Integer.toHexString(b[0]) + ")",
                  this._nByte);
        ...
        for (int i = 1; i < nBytes; i++) {
          ...
          if (0x80 > b[i] || b[i] > 0xbf) {
            String subMessage =
                "Value = " + ((char) b[i]) + " (0x" + Integer.toHexString(b[i]) + ")";
        ...
        if (nBytes == 1) {
          ...
          ch = ((b[0] & 0x07) << 18) + ((b[1] & 0x3f) << 12) + ((b[2] & 0x3f) << 6) + (b[3] & 0x3f);
        }
    ...
    this._textMD.setByte_order(
        this._bigEndian ? TextMDMetadata.BYTE_ORDER_BIG : TextMDMetadata.BYTE_ORDER_LITTLE);
    ...
    info.setProperty(
        new Property("UTF8Metadata", PropertyType.PROPERTY, PropertyArity.LIST, metadataList));

    Property property = new Property("Characters", PropertyType.LONG, new Long(nChar));
    ...
    if (!propArray.isEmpty()) {
      property =
          new Property(LineEnding.PROP_NAME, PropertyType.STRING, PropertyArity.LIST, propArray);
      metadataList.add(property);
    }
    ...
    if (!this.usedCtrlChars.isEmpty()) {
      ...
      property =
          new Property(ControlChar.PROP_NAME, PropertyType.STRING, PropertyArity.LIST, propList);
      metadataList.add(property);
    }
    ...
    if (this._withTextMD) {
      property =
          new Property(
              "TextMDMetadata", PropertyType.TEXTMDMETADATA, PropertyArity.SCALAR, this._textMD);
      metadataList.add(property);
    }
  ...
  /**
   * Check if the digital object conforms to this Module's internal signature information. Try to
   * read the BOM if it's present, and check the beginning of the file.
   *
  ...
  protected boolean checkMark(int byt, RepInfo info) {
    ...
    if (this._nByte == 3) {
      ...
      if (this.initialBytes[0] == 0xEF
          && this.initialBytes[1] == 0xBB
