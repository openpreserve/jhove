...
public class ChunkHeader {
   ...
   *
   * <p>If {@code _chunkId} is non-null it's assumed to have already been read.
   */
      ...
      if (ch < LOWEST_PRINTABLE_ASCII || ch > HIGHEST_PRINTABLE_ASCII) {
        _repInfo.setMessage(
            new ErrorMessage(
                ...
                String.format(MessageConstants.IFF_HUL_1_SUB.getMessage(), ch),
                _module.getNByte() - 1));
    ...
    if (spacePrecedesPrintableCharacters) {
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              MessageConstants.IFF_HUL_2.getId(),
              String.format(MessageConstants.IFF_HUL_2.getMessage(), _chunkId));
      _repInfo.setMessage(new ErrorMessage(message, _module.getNByte() - Chunk.ID_LENGTH));
