...
public class AiffModule extends ModuleBase {
  ...
  /* Values for fileType */
  public static final int AIFFTYPE = 1, AIFCTYPE = 2;
  ...
  private static final String REPINFO =
      "Properties capturing the technical attributes of the audio " + "from all chunks";
  ...
  public AiffModule() {
    ...
    Document doc =
        new Document(
            ...
            DocumentType.REPORT);
    Builder builder =
        new Agent.Builder("Apple Computer, Inc.", AgentType.COMMERCIAL)
    ...
    Signature sig =
        new ExternalSignature("AIFF", SignatureType.FILETYPE, SignatureUseType.OPTIONAL);
    _signature.add(sig);
    ...
    sig =
        new ExternalSignature(
            ".aifc", SignatureType.EXTENSION, SignatureUseType.OPTIONAL, "For AIFF-C profile");
    _signature.add(sig);
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    try {
      ...
      info.setWellFormed(RepInfo.FALSE);
      info.setMessage(new ErrorMessage(MessageConstants.AIFF_HUL_8, _nByte));
      return 0;
    }

    if (!commonChunkSeen) {
      info.setWellFormed(RepInfo.FALSE);
      info.setMessage(new ErrorMessage(MessageConstants.AIFF_HUL_2));
    }
    if (fileType == AIFCTYPE && !formatVersionChunkSeen) {
      info.setWellFormed(RepInfo.FALSE);
      info.setMessage(new ErrorMessage(MessageConstants.AIFF_HUL_3));
    }
    ...
    _aesMetadata.setByteOrder(
        thisFileBigEndian ? AESAudioMetadata.BIG_ENDIAN : AESAudioMetadata.LITTLE_ENDIAN);
    ...
    if (!_annotationList.isEmpty()) {
      _propList.add(
          new Property("Annotations", PropertyType.PROPERTY, PropertyArity.LIST, _annotationList));
    }
    if (!_midiList.isEmpty()) {
      _propList.add(new Property("MIDIData", PropertyType.PROPERTY, PropertyArity.LIST, _midiList));
    }
    if (!_saxelList.isEmpty()) {
      _propList.add(new Property("Saxels", PropertyType.PROPERTY, PropertyArity.LIST, _saxelList));
    }
  ...
  /**
   * Sets the endian-ness. <code>true</code> indicates big-endian, and <code>false</code> means
   * little-endian. This is needed because chunk data can change the usual little-endian byte order
   * to big-endian.
   */
  ...
  /**
   * Adds an Annotation Property to the annotation list. This will get put into an Annotations
   * Property.
   */
  ...
  protected void initParse() {
    ...
    _metadata = new Property("AIFFMetadata", PropertyType.PROPERTY, PropertyArity.LIST, _propList);
  ...
  /**
   * Reads a Pascal string. A Pascal string is one whose count is given in the first byte. The count
   * is exclusive of the count byte itself. A Pascal string can have a maximum of 255 characters. If
   * the count of a Pascal string is even (meaning the total number of bytes is odd), there will be
   * a pad byte to skip, so that the next item can start on an even boundary.
   *
   * <p>We assume the string will be in ASCII or Macintosh encoding.
   */
  ...
  /**
   * Converts a Macintosh-style timestamp (seconds since January 1, 1904) into a Java date. The
   * timestamp is treated as a time in the default localization. Depending on that localization,
   * there may be some variation in the exact hour of the date returned, e.g., due to daylight
   * savings time.
   */
   ...
   * The SSND chunk offset value must be added to the current byte offset for a correct value.
   */
  ...
  protected boolean readFileType(RepInfo info) throws IOException {
    ...
    if ("AIFF".equals(typ)) {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.AIFF_HUL_4, _nByte));
      info.setWellFormed(RepInfo.FALSE);
      return false;
    }
  }
  ...
  protected boolean readChunk(RepInfo info) throws IOException {
    ...
    if ("FVER".equals(id)) {
      ...
      info.setMessage(new InfoMessage(MessageConstants.AIFF_HUL_9, id, _nByte));
    }
    if (chunk != null) {
  ...
  protected void dupChunkError(RepInfo info, String chunkName) {
    ...
    info.setMessage(
        new ErrorMessage(JhoveMessages.getMessageInstance(mess.getId(), message), _nByte));
    info.setValid(false);
  }
}
