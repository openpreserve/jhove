      ...
      while (!endOfFile) {
        ...
        try {
          ...
          if ("AESD".equals(ckID)) {
            ...
            int numChannels = ModuleBase.readSignedShort(stream, ENDIAN);
            long numSampleFrames = ModuleBase.readUnsignedInt(stream, ENDIAN);
            int sampleSize = ModuleBase.readSignedShort(stream, ENDIAN);
            ...
            if (aiff_c) {
              ...
              int count = ModuleBase.readUnsignedByte(stream, null);
              alreadyRead = 23;
              sbuf.setLength(0);
            ...
            int numComments = ModuleBase.readUnsignedShort(stream, ENDIAN, null);
            System.out.print(": " + numComments);
            alreadyRead = 2;
          } else if ("FVER".equals(ckID)) {
            long timestamp = ModuleBase.readUnsignedInt(stream, ENDIAN, null);
            ...
            int numMarkers = ModuleBase.readUnsignedShort(stream, ENDIAN);
            System.out.print(": " + numMarkers);
            alreadyRead = 2;
          } else if ("SSND".equals(ckID)) {
            long offset = ModuleBase.readUnsignedInt(stream, ENDIAN);
            long blockSize = ModuleBase.readUnsignedInt(stream, ENDIAN);
