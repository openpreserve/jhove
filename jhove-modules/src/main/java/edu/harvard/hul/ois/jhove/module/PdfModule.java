 ...
 * JSTOR and the President and Fellows of Harvard College This program is free
...
public class PdfModule extends ModuleBase {
	...
	private static final String DICT_KEY_MODIFED_DATE = "ModDate";
	private static final String DICT_KEY_TRAPPED = "Trapped";
	private static final String DICT_KEY_XOBJ_SUBTYPE = "Subtype";
	private static final String DICT_KEY_FONT_SUBTYPE = "Subtype";
	...
	private static final String PROP_NAME_FILTER = "Filter";
	...
	private static final String PROP_NAME_MODIFIED_DATE = DICT_KEY_MODIFED_DATE;
	...
	private static final String PROP_NAME_PAGES = "Pages";
	...
	/** Font type selectors. */
	public final static int F_TYPE0 = 1, F_TYPE1 = 2, F_TT = 3, F_TYPE3 = 4, F_MM1 = 5, F_CID0 = 6, F_CID2 = 7;
	...
	/*
	 * The maximum number of fonts that will be reported before we just give up
	 * and report a stub to avoid running out of memory.
	 */
	...
	protected Map<Integer, PdfObject> _type1FontsMap; // Map of type 1 font
														// dictionaries
	protected Map<Integer, PdfObject> _mmFontsMap; // Map of multi master font
													// dictionaries
	protected Map<Integer, PdfObject> _type3FontsMap; // Map of type 3 font
														// dictionaries
	protected Map<Integer, PdfObject> _trueTypeFontsMap; // Map of TrueType font
															// dictionaries
	protected Map<Integer, PdfObject> _cid0FontsMap; // Map of CIDFont/Type1
														// dictionaries
	protected Map<Integer, PdfObject> _cid2FontsMap; // Map of CIDFont/TrueType
														// dictionaries

	protected Map<Integer, Integer> _pageSeqMap; // Map associating page object
													// dicts with sequence
													// numbers
	...
	/*
	 * These three variables track whether a message has been posted notifying
	 * the user of omitted information.
	 */
	...
	/*
	 * These are the message texts to post in case of omitted information.
	 */
	private final static String fontsSkippedString = "Fonts exist, but are not displayed; to display "
			+ "remove param value of f from the config file";
	private final static String outlinesSkippedString = "Outlines exist, but are not displayed; to display "
			+ "remove param value of o from the config file";
	private final static String annotationsSkippedString = "Annotations exist, but are not displayed; to display "
			+ "remove param value of a from the config file";
	private final static String pagesSkippedString = "Page information is not displayed; to display "
			+ "remove param value of p from the config file";

	/* Warning messages. */
	protected final static String outlinesRecursiveString = "Outlines contain recursive references.";

	/* Name-to-value array pairs for NISO metadata */
	private final static String[] compressionStrings = { "LZWDecode", /* "FlateDecode", */ "RunLengthDecode",
			"DCTDecode", "CCITTFaxDecode" };
	private final static int[] compressionValues = { 5, /* 8, */ 32773, 6, 2 };
	/*
	 * The value of 2 (CCITTFaxDecode) is a placeholder; additional checking of
	 * the K parameter is needed to determine the real value if that's returned.
	 */

	private final static String[] colorSpaceStrings = { "Lab", "DeviceRGB", "DeviceCMYK", "DeviceGray", "Indexed" };
	...
	public PdfModule() {
		...
		Document doc = new Document("PDF Reference: Adobe Portable " + "Document Format, Version 1.4",
				DocumentType.BOOK);
		...
		doc.setIdentifier(new Identifier(
				"http://partners.adobe.com/asn/" + "acrobat/docs/File_Format_" + "Specifications/PDFReference.pdf",
				IdentifierType.URL));
		...
		doc.setIdentifier(new Identifier("http://partners.adobe.com/public/developer/en/pdf/PDFReference15_v6.pdf",
				IdentifierType.URL));
		_specification.add(doc);
		...
		doc.setIdentifier(new Identifier("http://partners.adobe.com/public/developer/en/pdf/PDFReference16.pdf",
				IdentifierType.URL));
		_specification.add(doc);
		...
		doc = new Document("Graphic technology -- Prepress " + "digital data exchange -- Use of PDF -- "
				+ "Part 4: Complete exchange using CMYK and " + "spot colour printing data using "
				+ "PDF 1.4 (PDF/X-1a)", DocumentType.STANDARD);
		doc.setPublisher(isoAgent);
		...
		doc = new Document("Graphic technology -- Prepress " + "digital data exchange -- Use of PDF -- "
				+ "Part 6: Complete exchange suitable for " + "colour-managed workflows using " + "PDF 1.4 (PDF/X-3)",
				DocumentType.STANDARD);
		doc.setPublisher(isoAgent);
		...
		_signature.add(new ExternalSignature(".pdf", SignatureType.EXTENSION, SignatureUseType.OPTIONAL));
		_signature.add(new InternalSignature("%PDF-1.", SignatureType.MAGIC, SignatureUseType.MANDATORY, 0));

		doc = new Document("Document management -- Electronic " + "document file format for long-term "
				+ "preservation -- Part 1: Use of PDF (PDF/A)", DocumentType.RFC);
		doc.setPublisher(isoAgent);
		doc.setDate("2003-11-30");
		doc.setIdentifier(new Identifier("ISO/CD 19005-1", IdentifierType.ISO));
		doc.setIdentifier(
				new Identifier("http://www.aiim.org/documents/standards/ISO_19005-1_(E).doc", IdentifierType.URL));
		_specification.add(doc);
	...
	/******************************************************************
	 * PUBLIC INSTANCE METHODS. Parsing methods.
	 ******************************************************************/
	 ...
	 *            parameter. If the parameter contains the indicated characters,
	 *            then the specified information is omitted; otherwise, it is
	 *            included. (This is the reverse of the behavior prior to beta
	 *            3.) These characters may be provided as separate parameters,
	...
	/**
	 * Parse a file and stores descriptive information. A RandomAccessFile must
	 * be used to represent the object.
	 *
	 * @param raf
	 *            A PDF file
	 * @param info
	 *            A clean RepInfo object, which will be modified to hold the
	...
	public final void parse(RandomAccessFile raf, RepInfo info) throws IOException {
		...
		/*
		 * We construct a big whopping property, which contains up to 11
		 * subproperties
		 */
		_metadata = new Property("PDFMetadata", PropertyType.PROPERTY, PropertyArity.LIST, metadataList);

		if (_raf.length() > 10000000000L) { // that's 10^10
			_pdfACompliant = false; // doesn't meet size limit in Appendix C of
									// PDF spec
		}
		if (!parseHeader(info)) {
			return;
		...
		/*
		 * Walk through the linked trailer and cross reference sections.
		 */
		_prevxref = -1;
		...
		while (_startxref > 0) {
			...
			if (_xrefIsStream) {
				/*
				 * If we have an xref stream, readXRefInfo dealt with all the
			...
			if (_startxref == _prevxref) {
				info.setMessage(new ErrorMessage(MessageConstants.ERR_XREF_TABLES_BROKEN, _parser.getOffset()));
		...
		metadataList.add(new Property(PROP_NAME_OBJECTS, PropertyType.INTEGER, new Integer(_numObjects)));
		metadataList.add(new Property(PROP_NAME_FREE_OBJECTS, PropertyType.INTEGER, new Integer(_numFreeObjects)));
		metadataList.add(new Property(PROP_NAME_INC_UPDATES, PropertyType.INTEGER, new Integer(_numTrailers)));
		if (_docCatalogList != null) {
			metadataList.add(
					new Property(PROP_NAME_DOC_CATALOG, PropertyType.PROPERTY, PropertyArity.LIST, _docCatalogList));
		}
		if (_encryptList != null) {
			metadataList
					.add(new Property(PROP_NAME_ENCRYPTION, PropertyType.PROPERTY, PropertyArity.LIST, _encryptList));
		}
		if (_docInfoList != null) {
			metadataList.add(new Property(PROP_NAME_INFO, PropertyType.PROPERTY, PropertyArity.LIST, _docInfoList));
		}
		if (_idProperty != null) {
			metadataList.add(_idProperty);
		}
		if (_extStreamsList != null && !_extStreamsList.isEmpty()) {
			metadataList.add(new Property(PROP_NAME_EXTERNAL_STREAMS, PropertyType.PROPERTY, PropertyArity.LIST,
					_extStreamsList));
		}
		if (_filtersList != null && !_filtersList.isEmpty()) {
			metadataList.add(new Property(PROP_NAME_FILTERS, PropertyType.PROPERTY, PropertyArity.LIST, _filtersList));
		}
		if (_imagesList != null && !_imagesList.isEmpty()) {
			metadataList.add(new Property(PROP_NAME_IMAGES, PropertyType.PROPERTY, PropertyArity.LIST, _imagesList));
		}
	...
	/**
	 * Returns true if the module hasn't detected any violations of PDF/A
	 * compliance. This must return true, but is not sufficient by itself, to
	 * establish compliance. The <code>AProfile</code> profiler makes the final
	 * determination.
	 */
	...
	/**
	 * Initialize the module. This is called at the start of parse restore the
	 * module to its initial state.
	 */
	@Override
	...
	private long lastEOFOffset(RandomAccessFile raf) throws IOException {
		...
		for (int i = buf.length - 4; i >= 1; i--) {
			if (buf[i] == '%') {
				if ((buf[i - 1] == '%') && (buf[i + 1] == 'E') && (buf[i + 2] == 'O') && (buf[i + 3] == 'F')) {
					eofpos = offset + i - 1;
					break;
				}
			}
	...
	private long lastStartXrefOffset(RandomAccessFile raf, long eofOffset) throws IOException {
		...
		for (int i = buf.length - 9; i >= 0; i--) {
			if (buf[i] == 's') {
				if ((buf[i + 1] == 't') && (buf[i + 2] == 'a') && (buf[i + 3] == 'r') && (buf[i + 4] == 't')
						&& (buf[i + 5] == 'x') && (buf[i + 6] == 'r') && (buf[i + 7] == 'e') && (buf[i + 8] == 'f')) {
					xrefpos = offset + i;
					break;
				}
			}
	...
	protected boolean findLastTrailer(RepInfo info) throws IOException {
		/*
		 * Parse file trailer. Technically, this should be the last thing in the
		 * file, but we follow the Acrobat convention of looking in the last
		 * 1024 bytes. Since incremental updates may add multiple EOF comments,
		 * make sure that we use the last one in the file.
		 */
		...
		if (_startxref < 0L) {
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(MessageConstants.ERR_STARTXREF_MISSING, _parser.getOffset()));
			return false;
		}
		return true;
	}
	...
	/*
	 * Parse a "trailer" (which is not necessarily the last thing in the file,
	 * as trailers can be linked.)
	 */
	protected boolean parseTrailer(RepInfo info, boolean prevOnly) throws IOException {
		...
		try {
			...
			/*
			 * The next object may be either the keyword "xref", signifying a
			 * classic cross-reference table, or a stream object, signifying the
			 * new-style cross-reference stream.
			 */
			Token xref = _parser.getNext();
			if (xref instanceof Keyword) {
				_xrefIsStream = false;
				_parser.getNext(Numeric.class, MessageConstants.ERR_XREF_TABLE_INVALID); // first
																							// object
																							// number
				_objCount = ((Numeric) _parser.getNext(Numeric.class, MessageConstants.ERR_XREF_TABLE_INVALID))
						.getIntegerValue();
				...
				/*
				 * We don't need to see a trailer dictionary. Move along, move
				 * along.
				 */
				return true;
			}
			...
			if (trailer < 0L) {
				info.setWellFormed(false);
				info.setMessage(new ErrorMessage(MessageConstants.ERR_FILE_TRAILER_MISSING, _parser.getOffset()));
				return false;
			}
			...
			if (_docCatDictRef == null) {
				throw new PdfInvalidException(MessageConstants.ERR_TRAILER_DICT_ROOT_MISSING, _parser.getOffset());
			}
			_encryptDictRef = (PdfIndirectObj) _trailerDict.get(DICT_KEY_ENCRYPT); // This
																					// is
																					// at
																					// least
																					// v.
			...
			if (obj != null) {
				if (obj instanceof PdfArray) {
					...
					try {
						...
						_idProperty = new Property("ID", PropertyType.STRING, PropertyArity.ARRAY, id);
					} catch (Exception e) {
						throw new PdfInvalidException(MessageConstants.ERR_TRAILER_ID_INVALID);
					}
				} else {
			...
			if (obj != null) {
				 ...
				 */
				_logger.warning(MessageConstants.WRN_HYBRID_XREF_NOT_IMPLEMENTED);
			}
		} catch (PdfException e) {

			e.disparage(info);
			info.setMessage(new ErrorMessage(e.getMessage(), _parser.getOffset()));
	...
	/*
	 * Parses the cross-reference streams. This is called from readXRefInfo if
	 * there is no cross-reference table. I still need to deal with hybrid
	 * cases. All linked cross-reference streams are handled here.
	 */
	protected boolean readXRefStreams(RepInfo info) throws IOException {
		...
		while (_startxref > 0) {
			try {
				_parser.seek(_startxref);
				PdfStream pstream = (PdfStream) _parser.readObjectDef();
				...
				if (sObjNum < 0 || sObjNum >= no) {
					throw new PdfMalformedException(MessageConstants.ERR_XREF_STRM_OBJ_NUM_INVALID,
							_parser.getOffset());
				}
				...
				try {
					while (xstream.readNextObject()) {
						...
						if (xstream.isObjCompressed()) {
							...
							_xref2[objNum] = new int[] { xstream.getContentStreamObjNum(),
					...
					info.setWellFormed(false);
					info.setMessage(new ErrorMessage(MessageConstants.ERR_XREF_STRM_MALFORMED, _parser.getOffset()));
					return false;
				}
			} catch (PdfException e) {
	...
	/*
	 * Parses the cross-reference table. This is called from readXRefInfo if
	 * there is a cross-reference table.
	 */
				...
				while ((token = _parser.getNext()) != null) {
					...
					for (int i = 0; i < _objCount; i++) {
						...
						long offset = ((Numeric) _parser.getNext(Numeric.class,
								MessageConstants.ERR_XREF_TABLE_MALFORMED)).getLongValue();
						...
						if ("n".equals(keyval)) {
							...
							throw new PdfMalformedException(MessageConstants.ERR_XREF_TABLE_OPERATOR_ILLEGAL,
									_parser.getOffset());
						}
					}
				}
	...
	protected boolean readDocCatalogDict(RepInfo info) throws IOException {
		...
		if (_docCatDictRef == null) {
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(MessageConstants.ERR_DOC_CAT_DICT_MISSING, 0));
			return false;
		}
		try {
			_docCatDict = (PdfDictionary) resolveIndirectObject(_docCatDictRef);
		} catch (Exception e) {
			_logger.warning(MessageConstants.WRN_CAT_NON_DICT_TO_DICT);
			e.printStackTrace();
		}
		if (_docCatDict == null) {
		...
		try {
			...
			p = new Property(PROP_NAME_PAGE_LAYOUT, PropertyType.STRING, pLayoutText);
			_docCatalogList.add(p);
			...
			if (lang != null && lang instanceof PdfSimpleObject) {
				...
				p = new Property(PROP_NAME_LANG, PropertyType.STRING, _encrypted ? ENCRYPTED : langText);
				_docCatalogList.add(p);
			}
			...
			_pageLabelDict = (PdfDictionary) resolveIndirectObject(_docCatDict.get(DICT_KEY_PAGE_LABELS));
			...
			if (vers instanceof PdfSimpleObject) {
				...
				try {
					...
					if (ver != infoVer) {
						info.setMessage(
								new InfoMessage(MessageConstants.INF_HEADER_CAT_VER_MISMATCH_1 + versString
					...
					throw new PdfInvalidException(MessageConstants.ERR_DOC_CAT_VERSION_INVALID);
				}
			}
			...
			try {
				PdfDictionary namesDict = (PdfDictionary) resolveIndirectObject(_docCatDict.get(DICT_KEY_NAMES));
				if (namesDict != null) {
				...
				_logger.info(MessageConstants.INF_NAMES_DICT_CLASS_CAST);
				throw new PdfInvalidException(MessageConstants.ERR_NAMES_DICT_INVALID);
			} catch (Exception e) {
				_logger.info(MessageConstants.INF_NAMES_DICT_EXCEP + e.getClass().getName());
			...
			try {
				_destsDict = (PdfDictionary) resolveIndirectObject(_docCatDict.get(DICT_KEY_DESTS));
			} catch (ClassCastException ce) {
				_logger.info(MessageConstants.INF_NAMES_DICT_CLASS_CAST);
			...
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(e.toString(), _parser.getOffset()));
			return false;
		}
		return true;
	}
	...
	protected boolean readEncryptDict(RepInfo info) throws IOException {
		...
		try {
			...
			PdfDictionary dict = (PdfDictionary) resolveIndirectObject(_encryptDictRef);
			_encryptDict = dict;
			...
			Property p = new Property(PROP_NAME_SECURITY_HANDLER, PropertyType.STRING, filterText);
			_encryptList.add(p);
			...
			if (algorithm instanceof PdfSimpleObject) {
				...
				if (tok instanceof Numeric) {
					...
					if (_je != null && _je.getShowRawFlag()) {
						p = new Property(PROP_NAME_ALGORITHM, PropertyType.INTEGER, new Integer(algValue));
					} else {
						try {
							p = new Property(PROP_NAME_ALGORITHM, PropertyType.STRING, PdfStrings.ALGORITHM[algValue]);
						} catch (Exception e) {
							throw new PdfInvalidException(MessageConstants.ERR_ENCRYPT_DICT_ALG_INVALID,
									_parser.getOffset());
						}
					}
					if (p != null) {
			...
			if (length instanceof PdfSimpleObject) {
				...
				if (_je != null) {
					p = new Property(DICT_KEY_KEY_LENGTH, PropertyType.INTEGER, new Integer(keyLen));
					_encryptList.add(p);
				}
			}
			...
			if ("Standard".equals(filterText)) {
				...
				if (flagObj instanceof PdfSimpleObject) {
					...
					stdList.add(new Property(PROP_NAME_REVISION, PropertyType.INTEGER, new Integer(rev)));
				}
				PdfObject oObj = dict.get("O");
				if (oObj != null) {
					if (oObj instanceof PdfSimpleObject) {
						stdList.add(new Property(PROP_NAME_OWNER_STRING, PropertyType.STRING,
				...
				if (uObj != null) {
					if (uObj instanceof PdfSimpleObject) {
						stdList.add(new Property(PROP_NAME_USER_STRING, PropertyType.STRING,
								toHex(((PdfSimpleObject) uObj).getRawBytes())));
					}
				}
				_encryptList.add(new Property(PROP_NAME_STANDARD_SECURITY_HANDLER, PropertyType.PROPERTY,
						PropertyArity.LIST, stdList));
			}
	...
	protected boolean readDocInfoDict(RepInfo info) throws IOException {
		...
		try {
			_docInfoDict = (PdfDictionary) resolveIndirectObject(_docInfoDictRef);
			...
			addDateProperty(_docInfoDict, _docInfoList, DICT_KEY_MODIFED_DATE, PROP_NAME_MODIFIED_DATE);
			...
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(MessageConstants.ERR_UNEXPECTED_EXCEPTION + e.getClass().getName()));
		}
		return true;
	}
	...
	protected boolean readXMPData(RepInfo info) {
		try {
			...
			info.setMessage(new ErrorMessage(MessageConstants.ERR_XMP_INVALID, _parser.getOffset()));
			info.setValid(false);
			return false;
		}
		return true;
			...
			for (;;) {
				...
				if (streams != null) {
					...
					while (streamIter.hasNext()) {
						...
						if (specStr != null) {
							Property prop = new Property(PROP_NAME_FILE, PropertyType.STRING, specStr);
	...
	/**
	 * Locates the filters in the content stream dictionaries and generate a
	 * list of unique pipelines.
	 * 
	 * @return <code>false</code> if the filter structure is defective.
	 */
	...
	/**
	 * Finds the filters in a stream or array object which is the value of a
	 * stream's Filter key, and put them in _filtersList if a duplicate isn't
	 * there already. If the name is "Crypt", appends a colon and the name if
	 * available. Returns the filter string whether it's added or not, or null
	 * if there are no filters.
	 */
	protected String extractFilters(Filter[] filters, PdfStream stream) {
		/*
		 * Concatenate the names into a string of names separated by spaces.
		 */
		...
		if (filterStr != null && unique) {
			Property prop = new Property(PROP_NAME_FILTER_PIPELINE, PropertyType.STRING, filterStr);
	...
	protected void findImages(RepInfo info) throws IOException {
		...
		try {
			for (;;) {
				...
				if (rsrc != null) {
					PdfDictionary xo = (PdfDictionary) resolveIndirectObject(rsrc.get(RESOURCE_NAME_XOBJECT));
					if (xo != null) {
						Iterator<PdfObject> iter = xo.iterator();
						while (iter.hasNext()) {
							// Get an XObject and check if it's an image.
							_logger.info(MessageConstants.INF_IMAGE_GET);
							...
							if (xobdict != null) {
								...
								if (XOBJ_SUBTYPE_IMAGE.equals(subtype.getStringValue())) {
									...
									_logger.info(MessageConstants.INF_IMAGE_XOBJ);
									List<Property> imgList = new ArrayList<Property>(10);
									Property prop = new Property(PROP_NAME_IMAGE, PropertyType.PROPERTY,
											PropertyArity.LIST, imgList);
									NisoImageMetadata niso = new NisoImageMetadata();
									imgList.add(new Property(PROP_NAME_NISO_IMAGE_MD, PropertyType.NISOIMAGEMETADATA,
											niso));
									niso.setMimeType(MIME_TYPE);
									PdfSimpleObject widObj = (PdfSimpleObject) xobdict.get(DICT_KEY_WIDTH);
									niso.setImageWidth(widObj.getIntValue());
									PdfSimpleObject htObj = (PdfSimpleObject) xobdict.get(DICT_KEY_HEIGHT);
									...
									if (filt != null) {
										...
										// property.
										int nisoFilt = nameToNiso(filt, compressionStrings, compressionValues);
										if (nisoFilt >= 0) {
											/*
											 ...
											 * value.
											 */
											PdfObject parms = xobdict.get(DICT_KEY_DECODE_PARAMS);
											if (parms != null) {
												PdfSimpleObject kobj = null;
												if (parms instanceof PdfDictionary) {
													kobj = (PdfSimpleObject) ((PdfDictionary) parms).get(DICT_KEY_K);
												}
												/*
												 * Note that the DecodeParms
												 * value may also be an array of
												 * dictionaries. We are not
											...
											imgList.add(new Property(PROP_NAME_FILTER, PropertyType.STRING, filt));
										}
									} else {
									...
									if (colorSpc != null) {
										...
										if (colorName != null) {
											int nisoSpace = nameToNiso(colorName, colorSpaceStrings, colorSpaceValues);
											if (nisoSpace >= 0) {
												niso.setColorSpace(nisoSpace);
											} else {
												imgList.add(new Property(PROP_NAME_COLOR_SPACE, PropertyType.STRING,
														colorName));
											}
										}
									}

									PdfSimpleObject bpc = (PdfSimpleObject) xobdict.get(DICT_KEY_BITS_PER_COMPONENT);
									if (bpc != null) {
									...
									PdfSimpleObject intent = (PdfSimpleObject) xobdict.get(DICT_KEY_INTENT);
									if (intent != null) {
										imgList.add(new Property(PROP_NAME_INTENT, PropertyType.STRING,
												intent.getStringValue()));
									}

									PdfSimpleObject imgmsk = (PdfSimpleObject) xobdict.get(DICT_KEY_IMAGE_MASK);
									if (imgmsk != null) {
										boolean b = imgmsk.isTrue();
										imgList.add(new Property(PROP_NAME_IMAGE_MASK, PropertyType.BOOLEAN,
												new Boolean(b)));
									}
									...
									if (dcd != null) {
										...
										imgList.add(new Property(PROP_NAME_DECODE, PropertyType.INTEGER,
												PropertyArity.LIST, dcdlst));
									}

									PdfSimpleObject intrp = (PdfSimpleObject) xobdict.get(DICT_KEY_INTERPOLATE);
									if (intrp != null) {
										boolean b = intrp.isTrue();
										imgList.add(new Property(PROP_NAME_INTERPOLATE, PropertyType.BOOLEAN,
												new Boolean(b)));
									}

									PdfSimpleObject nam = (PdfSimpleObject) xobdict.get(DICT_KEY_NAME);
									if (nam != null) {
										imgList.add(new Property(PROP_NAME_NAME, PropertyType.STRING,
												nam.getStringValue()));
									}
									...
									if (id != null) {
										...
										imgList.add(new Property(PROP_NAME_ID, PropertyType.STRING, idstr));
									}
	...
	/*
	 * Convert a Filter name to a NISO compression scheme value. If the name is
	 * unknown to NISO, return -1.
	 */
	...
	protected void findFonts(RepInfo info) throws IOException {
		...
		try {
				...
				if (fonts != null) {
					...
					while (fontIter.hasNext()) {
						...
						if (font instanceof PdfDictionary) {
							...
							info.setWellFormed(false);
							info.setMessage(
									new ErrorMessage(MessageConstants.ERR_PAGE_FONT_DICT_MISSING, _parser.getOffset()));
							return;
						}
						...
						if (!_skippedFontsReported && !_showFonts && _verbosity != Module.MAXIMUM_VERBOSITY) {
			...
			info.setWellFormed(false);
			info.setMessage(new ErrorMessage(MessageConstants.ERR_FIND_FONTS_ERR, e.toString(), _parser.getOffset()));
			return;
		}
	}

	/**
	 * Add the font to the appropriate map, and return its subtype. If we've
	 * exceeded the maximum number of fonts, then ignore it.
	 */
	protected String addFontToMap(PdfDictionary font) {
		...
		try {
			PdfSimpleObject subtype = (PdfSimpleObject) font.get(DICT_KEY_FONT_SUBTYPE);
	...
	// sufficient?
	/*
	 * protected void addObject (int objectNum, int genNum, Object obj) { long
	 * key = ((long) objectNum << 32) + ((long) genNum & 0XFFFFFFFF);
	 * _objects.put (new Long (key), obj); }
	 */

	/**
	 * If the argument is an indirect object reference, returns the object it
	 * resolves to, otherwise returns the object itself. In particular, calling
	 * with null will return null.
	 */
	public PdfObject resolveIndirectObject(PdfObject indObj) throws PdfException, IOException {
		if (indObj instanceof PdfIndirectObj) {
			int objIndex = ((PdfIndirectObj) indObj).getObjNumber();
			/*
			 * Here we need to allow for the possibility that the object is
			 * compressed in an object stream. That means creating a new
			 * structure (call it _xref2) that contains the stream object number
			 * and offset whenever _xref[objIndex] is negative. _xref2 will have
			 * to contain the content stream object number (which will itself
			 * have to be resolved) and the offset into the object stream.
			 */
	...
	/**
	 * Returns an object of a given number. This may involve recursion into
	 * object streams, in which case it calls itself.
	 * 
	 * @param objIndex
	 *            The object number to look up
	 * @param recGuard
	 *            The maximum permitted number of recursion levels; no
	 *            particular value is required, but 30 or more should avoid
	 *            false exceptions.
	 */
	protected PdfObject getObject(int objIndex, int recGuard) throws PdfException, IOException {
		...
		if (offset < 0) {
			/*
			 * The object is located in an object stream. Need to get the object
			 * stream first. Be cautious dealing with _cachedStreamIndex and
			 * _cachedObjectStream; these can be modified by a recursive call to
			 * getObject.
			 */
			try {
				...
				if (streamObjIndex == _cachedStreamIndex) {
					...
					streamObj = resolveIndirectObject(getObject(streamObjIndex, recGuard - 1));
	...
	/**
	 * Get a font map. The map returned is determined by the selector. Any other
	 * value returns null.
	 */
	...
	/**
	 * Return a List of all the font maps. Together, these contain all the fonts
	 * and subfonts in the document. Some of the maps may be null.
	 */
	...
	/**
	 * Returns a NameTreeNode for the EmbeddedFiles entry of the Names
	 * dictionary. Returns null if there isn't one.
	 */
	...
	/**
	 * Add the various font lists as a fonts property. Note: only add the
	 * "Fonts" property if there are, in fact, fonts defined.
	 */
	protected void addFontsProperty(List<Property> metadataList) {
		...
		if (_trueTypeFontsMap != null && !_trueTypeFontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty(PROP_NAME_FONT_TRUE_TYPE, _trueTypeFontsMap, F_TT);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
				// Report an error here?
			}
		...
		if (_cid0FontsMap != null && !_cid0FontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty(PROP_NAME_FONT_CID_TYPE0, _cid0FontsMap, F_CID0);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
			}
		}
		if (_cid2FontsMap != null && !_cid2FontsMap.isEmpty()) {
			try {
				fontp = buildFontProperty(PROP_NAME_FONT_CID_TYPE2, _cid2FontsMap, F_CID2);
				fontTypesList.add(fontp);
			} catch (ClassCastException e) {
			}
		}
		if (fontTypesList.size() > 0) {
			metadataList.add(new Property(PROP_NAME_FONTS, PropertyType.PROPERTY, PropertyArity.LIST, fontTypesList));
		}
	}
	...
	protected void addPagesProperty(List<Property> metadataList, RepInfo info) {
		...
		try {
			...
			for (;;) {
				...
				_pageSeqMap.put(new Integer(page.getDict().getObjNumber()), new Integer(pageIndex + 1));
			}
			_docTreeRoot.startWalk();
			for (;;) {
				PageObject page = _docTreeRoot.nextPageObject();
			...
			if (_showPages || _verbosity == Module.MAXIMUM_VERBOSITY) {
				Property prop = new Property(PROP_NAME_PAGES, PropertyType.PROPERTY, PropertyArity.LIST, _pagesList);
	...
	protected Property buildPageProperty(PageObject page, int idx, RepInfo info) throws PdfException {
		...
		try {
			...
			if (plProp == null || nominalNum[0] != idx + 1) {
				...
				pagePropList.add(new Property(PROP_NAME_SEQUENCE, PropertyType.INTEGER, new Integer(idx + 1)));
		...
		try {
			...
			if (annots != null) {
				...
				for (int i = 0; i < contents.size(); i++) {
					PdfObject annot = resolveIndirectObject(contents.elementAt(i));
				...
				if (!annotsList.isEmpty()) {
					if (_showAnnotations || _verbosity == Module.MAXIMUM_VERBOSITY) {
						Property annotProp = new Property(PROP_NAME_ANNOTATIONS, PropertyType.PROPERTY,
								PropertyArity.LIST, annotsList);
		...
		try {
			...
			if (rot != null && rot.getIntValue() != 0) {
				pagePropList.add(new Property(PROP_NAME_ROTATE, PropertyType.INTEGER, new Integer(rot.getIntValue())));
			}
			...
			if (vp != null) {
				...
				while (iter.hasNext()) {
					PdfDictionary vpd = (PdfDictionary) resolveIndirectObject(iter.next());
					PdfObject vpdbb = vpd.get("BBox");
					List<Property> vpPropList = new ArrayList<Property>();
					vpPropList.add(makeRectProperty((PdfArray) resolveIndirectObject(vpdbb), "BBox"));
	...
	/*
	 * Build a subproperty of a subproperty for page labels. The nomNumRef
	 * argument is a crock for returning the nominal number; element 0 of the
	 * array is replaced by the nominal number of the page.
	 */
	protected Property buildPageLabelProperty(PageObject page, int pageIndex, int[] nomNumRef) throws PdfException {
		...
		try {
			...
			PdfDictionary pageLabelDict = (PdfDictionary) resolveIndirectObject(_pageLabelRoot.getPrevValue());
			StringBuffer labelText = new StringBuffer();
			PdfSimpleObject prefixObj = (PdfSimpleObject) pageLabelDict.get("P");
			if (prefixObj != null) {
				labelText.append(prefixObj.getStringValue());
			}
			PdfSimpleObject firstPageObj = (PdfSimpleObject) pageLabelDict.get("St");
			int nominalPage;
			if (firstPageObj != null) {
				nominalPage = pageIndex - curFirstPage + firstPageObj.getIntValue();
			} else {
		...
		try {
			...
			if (itemObj != null) {
				propList.add(new Property("Contents", PropertyType.STRING,
						_encrypted ? ENCRYPTED : ((PdfSimpleObject) itemObj).getStringValue()));
			}

			// Rectangle is required, and must be in the rectangle format
			itemObj = annot.get("Rect");
			propList.add(makeRectProperty((PdfArray) resolveIndirectObject(itemObj), "Rect"));
			...
			if (itemObj != null) {
				...
				Property dateProp;
				dateProp = new Property("LastModified", PropertyType.STRING, lastModLit.getValue());
			...
			if (itemObj != null) {
				...
				Property flagProp = (buildBitmaskProperty(flagValue, "Flags", PdfStrings.ANNOTATIONFLAGS,
						"No flags set"));
				if (flagProp != null) {
			...
			if (itemObj != null) {
				...
				PdfSimpleObject annType = (PdfSimpleObject) ((PdfDictionary) itemObj).get("S");
				if (annType == null) {
				...
				if ("GoTo".equals(annType.getStringValue())) {
					...
					if (destObj != null) {
						addDestination(destObj, "ActionDest", propList, info);
					}
				}
			}
	...
	/*
	 * Given a PdfObject that stands for a Destination, add a representative
	...
	protected void addDestination(PdfObject itemObj, String propName, List<Property> propList, RepInfo info) {
		try {
			...
			if (dest.isIndirect()) {
				...
				if (!_encrypted) {
					int pageObjNum = resolveIndirectDest(dest.getIndirectDest());
					if (pageObjNum == -1) {
			...
			info.setMessage(new ErrorMessage(msg, _parser.getOffset()));
			info.setValid(false);
		}
	}

	/*
	 * Build up a property for one of the kinds of fonts in the file.
	 */
	...
	protected List<Property> oneFontPropList(PdfDictionary dict, int fontType) {
		...
		Property prop;
		if (fontType == F_TYPE1 || fontType == F_TYPE3 || fontType == F_MM1 || fontType == F_TT) {
		...
		if (fontType == F_TYPE1 || fontType == F_TT || fontType == F_MM1 || fontType == F_TYPE3) {
		...
		if (fontType == F_TYPE3) {
			...
			try {
				if (bboxObj instanceof PdfArray) {
					fontPropList.add(makeRectProperty((PdfArray) bboxObj, "FontBBox"));
				}
			} catch (Exception e) {
			}
		...
		if (fontType == F_TYPE1 || fontType == F_TT || fontType == F_MM1 || fontType == F_CID0 || fontType == F_CID2) {
			...
			if (descriptorObj instanceof PdfDictionary) {
				prop = buildFontDescriptorProperty((PdfDictionary) descriptorObj);
				fontPropList.add(prop);
			}
		}
		...
		if (fontType == F_TYPE0 || fontType == F_TYPE1 || fontType == F_TT || fontType == F_MM1
				|| fontType == F_TYPE3) {
	...
	/*
	 * Code for CMapProperty for Type 0 fonts, based on the Encoding entry,
	 * broken out of buildFontProperty.
	 */
	...
	/*
	 * Code for CIDInfoProperty for CIDFontType0 and CIDFontType2 conts.
	 */
	protected Property buildCIDInfoProperty(PdfDictionary dict) {
		...
		if (reg instanceof PdfSimpleObject) {
			try {
				...
				subprop = new Property("Registry", PropertyType.STRING, _encrypted ? ENCRYPTED : regText);
				propList.add(subprop);
			} catch (Exception e) {
			}
		}
	...
	/*
	 * Code for EncodingDictionary Property for type 1, 3, TrueType, and MM
	 * fonts. This is based on a dictionary entry with the same name as the one
	 * for buildCMapDictProperty, but different information. Included properties
	 * are BaseEncoding and Differences.
	 */
	protected Property buildEncodingDictProperty(PdfDictionary encodingDict) {
		...
		Property diffsProp = new Property("Differences", PropertyType.BOOLEAN, new Boolean(diffs != null));
	...
	/*
	 * Separated-out code for FontDescriptor property. This is a list of six
	 * Properies: FontName, Flags, FontBBox, FontFile, FontFile2, and FontFile3.
	 */
	protected Property buildFontDescriptorProperty(PdfDictionary encodingDict) {
		...
		try {
			...
			subprop = buildBitmaskProperty(flagValue, "Flags", PdfStrings.FONTDESCFLAGS, "No flags set");
			if (subprop != null) {
				propList.add(subprop);
			}
		} catch (Exception e) {
	...
	/*
	 * Return TRUE if the string is a font subset string, which begins with six
	 * uppercase letters and then a plus sign
	 */
	...
	/*
	 * Create the "Outlines" property from the Outlines item in the catalog
	 * dictionary. As a side effect, we set the actionsExist flag if any Actions
	 * are found. Because we check destinations, this can't be called till the
	 * page tree is built.
	 * 
	 * Outlines can be recursive, according to Adobe people, so we have to track
	 * visited nodes.
	 */
	...
	/*
	 * Create an item property within the outlines hierarchy. If an Outline item
	 * property has children, then there is a list property called "Children"
	 * with elements called "Item". It calls itself recursively to walk down the
	 * outline.
	 */
	protected Property buildOutlineItemProperty(PdfDictionary dict, RepInfo info) throws PdfException {
		...
		try {
			...
			PdfSimpleObject title = (PdfSimpleObject) resolveIndirectObject(dict.get("Title"));
			if (title == null) {
				throw new PdfInvalidException(MessageConstants.ERR_OUTLINE_DICT_ITEM_INVALID);
			}
			itemList.add(new Property("Title", PropertyType.STRING, _encrypted ? ENCRYPTED : title.getStringValue()));
			...
			PdfDictionary child = (PdfDictionary) resolveIndirectObject(dict.get("First"));
			if (child != null) {
				...
				while (child != null) {
					...
					child = (PdfDictionary) resolveIndirectObject(child.get("Next"));
					if (child == null) {
						break;
					}
					// Check if this object is its own sibling. (It really does
	...
	/*
	 * This is separated out from readDocCatalogDict, where it would otherwise
	 * make sense, because we can't build the outlines property till we have a
	 * page tree to locate destinations.
	 */
	...
	/*
	 * Given a PdfSimpleObject representing a key, look up the Destination which
	 * it references. There are two completely different ways this can be done,
	 * though any given PDF file is supposed to implement only one. If
	 * _destsDict is non-null, we look the string up there, and may find either
	 * a dictionary or an array. Otherwise if _destNames is non-null, it's a
	 * NameTreeNode which contains the mapping. In either case, the destination
	 * could be external, in which case we just return a string saying so. (The
	 * implementation of Destinations in PDF is a prime example of design by
	 * stone soup.) We return the page sequence number for the referenced page.
	...
	protected int resolveIndirectDest(PdfSimpleObject key) throws PdfException {
		if (_destNames != null) {
			Destination dest = new Destination(_destNames.get(key.getRawBytes()), this, true);
			// if (dest == null) {
			// return -1;
			// }
	...
	protected Property buildUserPermProperty(int flags, String[] flagStrs) {
		return buildBitmaskProperty(flags, "UserAccess", flagStrs, "No permissions");
	}

	/**
	 * Add a string proprerty, based on a dictionary entry with a string value,
	 * to a specified List.
	 */
	...
	/**
	 * Add a date proprerty, based on a dictionary entry with a string value, to
	 * a specified List.
	 */
	protected void addDateProperty(PdfDictionary dict, List<Property> propList, String key, String propName)
			throws PdfException {
		if (_encrypted) {
	...
	/*
	 * General function for adding a property with a 32-bit value, with an array
	 * of Strings to interpret the value as a bitmask.
	 */
	...
	/*
	 * Take a PdfArray which is supposed to conform to the rectangle description
	 * (i.e., it's an array of 4 numbers) and create a Property which is an
	 * array of 4 integers.
	 */
