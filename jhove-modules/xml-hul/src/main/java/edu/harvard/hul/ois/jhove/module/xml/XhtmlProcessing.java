...
/**
 * Methods for processing XHTML elements in an XML document. These are closely related in
 * functionality to corresponding methods in the XML module.
 *
 * @author Gary McGath
...
public final class XhtmlProcessing {
  ...
  protected static void processElement(
      String localName, String qualifiedName, Attributes atts, HtmlMetadata mdata) {
  ...
  /**
   * Process metadata from an A element. Only elements with an HREF attribute are of interest here.
   * We ignore links to anchors.
   */
  ...
  /**
   * Processes metadata from the ABBR element. This will require the PCdata as well as the
   * attributes, so we deposit the Property temporarily in the Metadata.
   */
  ...
  /**
   * Processes metadata from the TITLE element. This will require PCData, so we deposit the Property
   * temporarily in the Metadata.
   */
  private static void processTitle(HtmlMetadata mdata, Attributes atts) {
    Property p =
        new Property("title", PropertyType.STRING, ""); // store property with placeholder value
    mdata.setPropUnderConstruction(p);
  }

  /**
   * Processes metadata from the CITE element. This will require PCData, so we deposit the Property
   * temporarily in the Metadata.
   */
  private static void processCite(HtmlMetadata mdata, Attributes atts) {
    Property p =
        new Property("cite", PropertyType.STRING, ""); // store property with placeholder value
    mdata.setPropUnderConstruction(p);
  }
}
