...
public class XmlModule extends ModuleBase {
  ...
  private static final String RIGHTS =
      "Copyright 2004-2007 by JSTOR and "
  ...
  public XmlModule() {
    ...
    Document doc =
        new Document(
            "Extensible Markup Language (XML) 1.0 " + "(Third Edition)", DocumentType.REPORT);
    ...
    Signature sig =
        new ExternalSignature(".xml", SignatureType.EXTENSION, SignatureUseType.OPTIONAL);
    _signature.add(sig);
    _localSchemas = new HashMap<>();
  }

  /**
   * Sets the value of the doctype string, assumed to have been forced to upper case. This is set
   * only when the HTML module invokes the XML module for an XHTML document.
   */
   ...
   *     starts with "schema", then the part to the right of the equal sign identifies a URI with a
   *     local path (URI, then semicolon, then path). If the first character is 's' and the
   *     parameter isn't "schema", then signature checking requires a document declaration, and the
   *     rest of the URL is considered as follows. If the parameter begins with 'b' or 'B', then the
   *     remainder of the parameter is used as a base URL. Otherwise it is ignored, and there is no
   *     base URL.
   */
  @Override
   ...
   *
   * <p>This is designed to be called in two passes. On the first pass, a nonvalidating parse is
   * done. If this succeeds, and the presence of DTD's or schemas is detected, then parse returns 1
   * so that it will be called again to do a validating parse. If there is nothing to validate, we
   * consider it "valid."
   *
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) {
    ...
    _metadata = new Property("XMLMetadata", PropertyType.PROPERTY, PropertyArity.LIST, _propList);
    ...
    if (parseIndex > 0) {
      try {
        ...
        try {
          ...
          info.setMessage(new InfoMessage(MessageConstants.XML_HUL_9));
        }
      }
    }
    try {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.XML_HUL_10, ef.getMessage().toString()));
      ...
      String mess = e.getMessage().toString() + "Line = " + line + ", Column = " + col;
      ...
      String subMess =
          (ee != null)
              ? MessageFormat.format(
                  MessageConstants.XML_HUL_12.getMessage(), ee.getClass().getName())
    ...
    _propList.add(new Property("Parser", PropertyType.STRING, parser.getClass().getName()));
    ...
    if (!schemaList.isEmpty()) {
      ...
      while (iter.hasNext()) {
        ...
        schItemProps[0] = new Property("NamespaceURI", PropertyType.STRING, schItems.namespaceURI);
        schItemProps[1] = new Property("SchemaLocation", PropertyType.STRING, schItems.location);
        schemaPropList.add(
            new Property("Schema", PropertyType.PROPERTY, PropertyArity.ARRAY, schItemProps));
      }
      // Now put the list into a Property, which goes into
      // the metadata.
      Property prop =
          new Property("Schemas", PropertyType.PROPERTY, PropertyArity.LIST, schemaPropList);
      _propList.add(prop);
    }
    ...
    if (!ns.isEmpty()) {
      ...
      while (iter.hasNext()) {
        ...
        supPropArr[0] = new Property("Prefix", PropertyType.STRING, key);
        supPropArr[1] = new Property("URI", PropertyType.STRING, val);
        Property onens =
            new Property("Namespace", PropertyType.PROPERTY, PropertyArity.ARRAY, supPropArr);
        nsList.add(onens);
    ...
    if (!refs.isEmpty()) {
      ...
      charRefsProp =
          new Property("CharacterReferences", PropertyType.STRING, PropertyArity.LIST, refList);
    ...
    if (!uent.isEmpty()) {
      ...
      while (iter.hasNext()) {
        ...
        if (nameInCollection(name, attributeVals)) {
          ...
          subPropList.add(new Property("Type", PropertyType.STRING, "External unparsed"));
          ...
          subPropList.add(new Property("NotationName", PropertyType.STRING, entarr[3]));

          entProps.add(
              new Property("Entity", PropertyType.PROPERTY, PropertyArity.LIST, subPropList));
        }
      }
    }
    ...
    if (!pi.isEmpty()) {
      ...
      while (pii.hasNext()) {
        ...
        subPropArr[0] = new Property("Target", PropertyType.STRING, pistr.target);
        subPropArr[1] = new Property("Data", PropertyType.STRING, pistr.data);
        piPropList.add(
            new Property(
                "ProcessingInstruction", PropertyType.PROPERTY, PropertyArity.ARRAY, subPropArr));
      }
      procInstProp =
          new Property(
              "ProcessingInstructions", PropertyType.PROPERTY, PropertyArity.LIST, piPropList);
    }
    ...
    if (!notations.isEmpty()) {
      ...
      while (iter.hasNext()) {
        ...
        if (nameInCollection(notName, piTargets)
          ...
          subPropList.add(new Property("Name", PropertyType.STRING, notName));
          if (notArray[1] != null) {
            subPropList.add(new Property("PublicID", PropertyType.STRING, notArray[1]));
          }
          if (notArray[2] != null) {
            subPropList.add(new Property("SystemID", PropertyType.STRING, notArray[2]));
          }
          notProps.add(
              new Property("Notation", PropertyType.PROPERTY, PropertyArity.LIST, subPropList));
        }
      }
      // Recheck emptiness in case only unprocessed notations were found
      if (!notProps.isEmpty()) {
        notationsProp =
            new Property("Notations", PropertyType.PROPERTY, PropertyArity.LIST, notProps);
      }
    }
    ...
    if (_withTextMD) {
      ...
      Property property =
          new Property(
              "TextMDMetadata", PropertyType.TEXTMDMETADATA, PropertyArity.SCALAR, _textMD);
   ...
   *
   * <p>XML is a particularly messy case; in general, there's no even moderately good way to check
   * "signatures" without parsing the whole file, since the document declaration is optional. We
   * provide the user two choices, based on the "s" parameter. If 's' is the first character of the
   * module parameter, then we look for an XML document declaration, and say there's no signature if
   * it's missing. (This can reject well-formed XML files, though not valid ones.) Otherwise, if
   * there's no document declaration, we parse the whole file.
   *
  ...
  /**
   * Add a mapping from a schema URI to a local file. The parameter is of the form
   * schema=[URI];[path]
   */
