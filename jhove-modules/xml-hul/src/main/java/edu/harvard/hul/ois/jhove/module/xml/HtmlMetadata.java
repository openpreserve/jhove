 ...
 * Validation Environment Copyright 2004-2009 by JSTOR and the President and Fellows of Harvard
 * College ********************************************************************
 */
...
/**
 * Repository for an HTML document's metadata. Also hold some state information, so that properties
 * involving tags, attributes and pcdata can be constructed.
 *
 * @author Gary McGath
 */
public class HtmlMetadata {
  ...
  /**
   * Adds a String to the Entities property. This property is a SortedSet, so duplicates are not
   * added, and the resulting set can be iterated in alphabetical order.
   */
  ...
  public Property toProperty(TextMDMetadata _textMD) {
    ...
    Property val =
        new Property("HTMLMetadata", PropertyType.PROPERTY, PropertyArity.LIST, propList);
    if (_lang != null) {
      propList.add(new Property("PrimaryLanguage", PropertyType.STRING, _lang));
      if (_textMD != null) {
    ...
    if (_languages != null) {
      propList.add(
          new Property("OtherLanguages", PropertyType.STRING, PropertyArity.SET, _languages));
    }
    if (_title != null) {
      propList.add(new Property("Title", PropertyType.STRING, _title));
    }
    if (_meta != null) {
      // We're guaranteed that if _meta isn't null, it's non-empty.
      propList.add(new Property("MetaTags", PropertyType.PROPERTY, PropertyArity.LIST, _meta));
    }
    if (_frames != null) {
      propList.add(new Property("Frames", PropertyType.PROPERTY, PropertyArity.LIST, _frames));
    }
    if (_links != null) {
      propList.add(new Property("Links", PropertyType.STRING, PropertyArity.LIST, _links));
    }
    if (_scripts != null) {
      propList.add(new Property("Scripts", PropertyType.STRING, PropertyArity.LIST, _scripts));
    }
    if (_images != null) {
      propList.add(new Property("Images", PropertyType.PROPERTY, PropertyArity.LIST, _images));
    }
    if (_citations != null) {
      propList.add(new Property("Citations", PropertyType.STRING, PropertyArity.LIST, _citations));
    }
    if (_defs != null) {
      propList.add(new Property("DefinedTerms", PropertyType.STRING, PropertyArity.LIST, _defs));
    }
    if (_abbrs != null) {
      propList.add(
          new Property("Abbreviations", PropertyType.PROPERTY, PropertyArity.LIST, _abbrs));
    }
    if (_entities != null) {
      propList.add(new Property("Entities", PropertyType.STRING, PropertyArity.SET, _entities));
    }
    if (utf8BM != null) {
  ...
  /**
   * Sets a "property under construction". This is generally called when an XML element is found,
   * and the PCDATA must be incorporated into the property.
   */
  ...
  /**
   * Adds PCDATA text to the property under construction. This may not all be provided in one lump,
   * so it has to allow for multiple chunks.
   */
  public void addToPropUnderConstruction(char[] ch, int start, int length) {
    if (_propUnderConstruction != null) {
      ...
      if ("abbr".equals(name)) {
        ...
        Property abProp = new Property("abbr", PropertyType.STRING, argStr);
        ...
        _propUnderConstruction = new Property(name, PropertyType.STRING, (String) val + argStr);
      }
    }
  }
