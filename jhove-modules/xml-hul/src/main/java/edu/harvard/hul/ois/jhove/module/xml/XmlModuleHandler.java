...
public class XmlModuleHandler extends DefaultHandler {
  ...
  /**
   * Sets the value of the XHTML flag. Special properties are extracted if this is an XHTML
   * document.
   */
  ...
  /**
   * Looks for the first element encountered. Stores its name as the value to be returned by
   * getRoot, qualified name by preference, local name if the qualified name isn't available.
   */
  @Override
  public void startElement(
      String namespaceURI, String localName, String qualifiedName, Attributes atts) {
    ...
    if (_xhtmlFlag) {
      ...
      XhtmlProcessing.processElement(localName, qualifiedName, atts, _htmlMetadata);
    }
  }
  ...
  /**
   * Processes PCData characters. This does things only in connection with properties under
   * construction in HTML metadata.
   */
  @Override
  public void characters(char[] ch, int start, int length) {
    if (_htmlMetadata != null && _htmlMetadata.getPropUnderConstruction() != null) {
  ...
  /**
   * Handles a processing instruction. Adds it to the list that will be returned by <code>
   * getProcessingInstructions</code>. Each element of the list is an array of two Strings. Element
   * 0 of the array is the target, and element 1 is the data.
   */
  @Override
  ...
  /**
   * Puts all notations into the notation list. A list entry is a String[3], consisting of name,
  ...
  /**
   * Overrides standard resolveEntity. First looks for DTD and entity files that are stored as
   * resources, and uses those if available. (Faster and more reliable than grabbing them over the
   * Net.) If that fails, calls the superclass's resolveEntity. Regardless, it then looks for
   * anything that appears to be a DTD and puts it in the DTD URI field. If the superclass's attempt
   * to resolve the entity results in an IOException, we just ignore it.
   */
  @Override
  ...
  /**
   * Picks up unparsed entity declarations, after calling the superclass's unparsedEntityDecl, and
   * puts their information into the unparsed entity declaration list as an array of four strings: [
   * name, publicId, systemId, notationName]. Null values are converted into empty strings.
   */
  @Override
    ...
    JhoveMessage message =
        JhoveMessages.getMessageInstance(
            ...
            MessageFormat.format(
                MessageConstants.XML_HUL_1.getMessage(), e.getMessage().toString()));
    _messages.add(new InfoMessage(message));
  }

  /**
   * Processes a parsing exception. An ill-formed piece of XML will get a fatalError (I think), so
   * we can assume that any error here indicates only invalidity.
   */
  @Override
    ...
    if (_nErrors == MAXERRORS) {
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              MessageFormat.format(
                  MessageConstants.XML_HUL_2.getMessage(), Integer.valueOf(MAXERRORS)));
      _messages.add(new InfoMessage(message));
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              MessageFormat.format(
                  MessageConstants.XML_HUL_1.getMessage(), e.getMessage().toString()));
      _messages.add(new ErrorMessage(message, "Line = " + line + ", Column = " + col));
    }
    ++_nErrors;
  }
  ...
  /**
   * Returns the list of schemas. The elements of the list are Strings, giving the URI's for the
   * schemas.
   */
  ...
  /**
   * Returns the list of unparsed entities. The elements of the list are arrays of four Strings,
   * giving the name, public ID, system ID and notation name respectively.
   */
  ...
  /**
   * Returns the List of processing instructions. Each element is an array of two strings, giving
   * the target and data respectively.
   */
  ...
  /**
   * Returns the validity state. If <code>error</code> has been called, the return value will be
   * <code>false</code>.
   */
  ...
  /**
   * Returns <code>true</code> if we have seen an element or a processing instruction, which implies
   * that we've seen an XML declaration.
   */
