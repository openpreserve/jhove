...
/**
 * Base class for all profiles under TIFF/FX. All TIFF/FX profiles should call
 * TiffFXBase.satisfiesClass to establish that common requirements are met.
 *
 * @author Gary McGath
 */
public abstract class TiffFXBase extends TiffProfile {
  ...
  public static final int GLOBALPARAMETERSIFD = 400, STRIPROWCOUNTS = 559;
  ...
  /**
   * Test for common requirements of all Tiff/FX profiles. Subclasses should call <code>
   * satisfiesClass()</code> from their <code>satisfiesThisProfile()</code> method to avoid
   * redundant code. If this method returns <code>false</code>, the IFD does not meet the
   * requirements of any TIFF/FX profile. Calling this also guarantees that the image length, image
   * width, bits per sample, X resolution (sampling frequency), Y resolution and page number values
   * are non-null objects.
   */
  ...
  /**
   * Convert a units/cm value to a units/inch value. For expected values, we use a table lookup to
   * avoid rounding problems. If a value isn't in the table, we do a rounded conversion.
   */
