 ...
 *
 * <p>This doesn't go into the _profiles list of TiffIFD, but rather is one of two (or more?)
 * profiles that must be checked to determine if the file meets the Exif profile. It should be
 * called only for the "thumbnail" IFD, which is the second top-level IFD.
 *
 * @author Gary McGath
 * @see TiffProfileExif
 */
public class TiffProfileExifThumb extends TiffProfile {
  ...
  /**
   * Compression scheme of the main IFD. We need to check our compression against the main IFD's
   * compression.
   */
  int mainCompression;
  ...
  /**
   * Returns true if the IFD satisfies the requirements of a thumbnail IFD for an Exif profile. See
   * the Exif specification for details.
   */
  @Override
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    return !(mainCompression == 1 && tifd.getNisoImageMetadata().getCompressionScheme() != 1);
  }
}
