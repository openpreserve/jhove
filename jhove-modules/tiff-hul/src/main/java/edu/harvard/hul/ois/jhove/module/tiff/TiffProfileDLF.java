...
public abstract class TiffProfileDLF extends TiffProfile {
  ...
  /**
   * Returns true if the IFD satisfies the requirements that are common to the bilevel, grayscale,
   * and color DLF profiles. The subclasses should call super(ifd) first, then do additional
   * checking if it returns true. details.
   */
  @Override
  ...
  /**
   * Checks for minimum X and Y resolution. All of the DLF profiles have similar tests for
   * XResolution and YResolution. In all cases the values depend on the ResolutionUnit, which must
   * be either 2 or 3.
   *
   * @param tifd The TiffIFD from which to extract the tags.
   * @param minUnit2Res The minimum XResolution and YResolution when ResolutionUnit is 2
   * @param minUnit3Res The minimum XResolution and YResolution when ResolutionUnit is 3
   */
