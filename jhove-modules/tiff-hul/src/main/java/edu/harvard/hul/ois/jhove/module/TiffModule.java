 ...
 * Validation Environment Copyright 2003-2012 by JSTOR and the President and Fellows of Harvard
 * College
 *
...
public class TiffModule extends ModuleBase {
  ...
  /**
   * * These profiles are not reported anymore (SLA, 2004-01-06) "Adobe PageMaker 6.0; Adobe
   * Photoshop 'Advanced TIFF'; " + *
   */
  private static final String[] MIMETYPE = {"image/tiff", "image/tiff-fx", "image/ief"};

  private static final String WELLFORMED =
      "A TIFF file is well-formed if "
  ...
  public TiffModule() {
    ...
    doc.setIdentifier(
        new Identifier(
            "http://partners.adobe.com/asn/" + "tech/tiff/specification.jsp", IdentifierType.URL));
    ...
    doc =
        new Document(
            "ISO/DIS 12639:2003, Graphic technology -- "
    ...
    doc =
        new Document(
            ...
            DocumentType.REPORT);
    agent =
        new Agent.Builder("Digital Library Federation", AgentType.NONPROFIT)
            .address("1755 Massachusetts Ave., NW, Suite 500, " + "Washington, DC 20036")
            .telephone("+1 (202) 939-4761")
            .fax("+1 (202) 939-4765")
            .email("dlf@clir.org")
    ...
    ident =
        new Identifier(
            "http://partners.adobe.com/asn/developer/pdfs/tn/TIFFPM6.pdf", IdentifierType.URL);
    doc.setIdentifier(ident);
    _specification.add(doc);
    ...
    doc =
        new Document(
            "TIFF-F Revised Specification: The " + "Spirit of TIFF Class F", DocumentType.REPORT);
    ...
    doc =
        new Document(
            "Tag Image File Format (TIFF) -- F Profile " + "for Facsimile", DocumentType.RFC);
    Agent ietfAgent =
        new Agent.Builder("IETF", AgentType.STANDARD).web("http://www.ietf.org").build();
    doc.setPublisher(ietfAgent);
    doc.setDate("1998-03");
    ident = new Identifier("RFC 2306", IdentifierType.RFC);
    doc.setIdentifier(ident);
    ident =
        new Identifier("http://hul.harvard.edu/jhove/references.html#rfc2306", IdentifierType.URL);
    doc.setIdentifier(ident);
    _specification.add(doc);
    ...
    doc =
        new Document(
            "A File Format for the Exchange of " + "Images in the Internet", DocumentType.RFC);
    doc.setPublisher(ietfAgent);
    doc.setDate("1992-04");
    ...
    doc =
        new Document(
            "GeoTIFF Format Specification: " + "GeoTIFF Revision 1.0", DocumentType.REPORT);
    ...
    ident =
        new Identifier(
            "http://remotesensing.org/geotiff/spec/" + "geotiffhome.html", IdentifierType.URL);
    doc.setIdentifier(ident);
    _specification.add(doc);
    ...
    sig =
        new ExternalSignature(
            ".tfx", SignatureType.EXTENSION, SignatureUseType.OPTIONAL, "For TIFF-FX");
    _signature.add(sig);

    sig =
        new ExternalSignature(
            "TFX ", SignatureType.FILETYPE, SignatureUseType.OPTIONAL, "For TIFF-FX");
    _signature.add(sig);
  ...
  public final void parse(RandomAccessFile raf, RepInfo info) throws IOException {
    ...
    try {
      ...
      if (ch0 != ch1 || (ch0 != 0X49 && ch0 != 0X4D)) {
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_20.getMessage(), (char) ch0, (char) ch1);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_20.getId(), mess);
      ...
      tiffMetadata[0] =
          new Property(
              "ByteOrder", PropertyType.STRING, _bigEndian ? "big-endian" : "little-endian");
      ...
      if (magic != 42) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_21.getId(), mess);
      ...
      ifdsList.add(new Property("Number", PropertyType.INTEGER, new Integer(ifds.size())));

      /* Build the IFD property list, for each of the IFDs. */
      ListIterator<IFD> iter = ifds.listIterator();
      while (iter.hasNext()) {
        IFD ifd = iter.next();
        ifdsList.add(ifd.getProperty(_je != null ? _je.getShowRawFlag() : false));
      ...
      info.setProperty(
          new Property("TIFFMetadata", PropertyType.PROPERTY, PropertyArity.ARRAY, tiffMetadata));
      ...
      if (inHeader) {
        ...
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_68.getMessage(), e.getClass().getName());
  ...
  public final List<IFD> exifParse(RandomAccessFile raf, RepInfo info) throws IOException {
    ...
    try {
      ...
      if (ch0 != ch1 || (ch0 != 0X49 && ch0 != 0X4D)) {
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_22.getMessage(), (char) ch0, (char) ch1);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_22.getId(), mess);
        throw new TiffException(message, 0);
      }
      _bigEndian = (ch0 == 0X4D);
      ...
      if (magic != 42) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_23.getId(), mess);
        throw new TiffException(message, 2);
      }
      ...
      else info.setMessage(new InfoMessage(e.getMessage(), e.getOffset()));
      return ifds;
    } catch (IOException e) {
      JhoveMessage msg;
      if (inHeader) {
        ...
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_69.getMessage(), e.getClass().getName());
  ...
  /**
   * Build list of profiles to check. Profile checking is, for the most part, done per IFD rather
   * than per file. Exif profile checking is an exception, since it requires the coordination of
   * multiple IFD's. Hence, the Exif profiles aren't added to the list, but treated elsewhere.
   */
  ...
  /**
   * Go through all the IFD's, calling checkIFDValidity on each one that is a standard IFD. (Private
   * IFD's have different requirements, and for the moment aren't checked here.) If any of them are
   * invalid, set info's valid field to false. Validity problems are non-fatal, and more information
   * is better, so we keep going with all IFDs even if we find problems.
   */
  ...
  protected void checkValidity(TiffIFD ifd, RepInfo info) throws TiffException {
    ...
    boolean tilesDefined =
        (tileWidth != NisoImageMetadata.NULL
      ...
      if (len != stripByteCounts.length) {
        String mess =
            MessageFormat.format(
                MessageConstants.TIFF_HUL_28.getMessage(), len, stripByteCounts.length);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_28.getId(), mess);
    ...
    if (tilesDefined) {
      ...
      if (tileWidth % 16 > 0) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_34.getId(), mess);
        reportInvalid(message, info);
      }
      if (tileLength % 16 > 0) {
        String mess = MessageFormat.format(MessageConstants.TIFF_HUL_35.getMessage(), tileLength);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_35.getId(), mess);
        reportInvalid(message, info);
      }
      ...
      if (planarConfiguration == 2) {
        ...
        if (tileOffsets != null && tileOffsets.length < spp_tpi) {
          String mess =
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_36.getMessage(), tileOffsets.length, spp_tpi);
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_36.getId(), mess);
          reportInvalid(message, info);
        }
        if (tileByteCounts != null && tileByteCounts.length < spp_tpi) {
          String mess =
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_37.getMessage(), tileByteCounts.length, spp_tpi);
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_37.getId(), mess);
          reportInvalid(message, info);
        }
      } else {
        if (tileOffsets != null && tileOffsets.length < tilesPerImage) {
          String mess =
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_38.getMessage(), tileOffsets.length, tilesPerImage);
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_38.getId(), mess);
          reportInvalid(message, info);
        }
        if (tileByteCounts != null && tileByteCounts.length < tilesPerImage) {
          String mess =
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_39.getMessage(), tileByteCounts.length, tilesPerImage);
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_39.getId(), mess);
          reportInvalid(message, info);
        }
      }
    }
    ...
    if ((photometricInterpretation == 4 && (newSubfileType & 4) == 0)
      ...
      reportInvalid(MessageConstants.TIFF_HUL_40, info);
    }
    ...
    if ((photometricInterpretation == 0
        ...
        && samplesPerPixel < 1) {
      String mess =
          MessageFormat.format(MessageConstants.TIFF_HUL_42.getMessage(), samplesPerPixel);
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_42.getId(), mess);
      reportInvalid(message, info);
    }
    if ((photometricInterpretation == 2
        ...
        && samplesPerPixel < 3) {
      String mess =
          MessageFormat.format(MessageConstants.TIFF_HUL_43.getMessage(), samplesPerPixel);
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_43.getId(), mess);
      reportInvalid(message, info);
    }
    ...
    if (photometricInterpretation == 3) {
      ...
      if (colormapBitCodeValue == null
          || colormapRedValue == null
          || colormapGreenValue == null
          || colormapBlueValue == null) {
        reportInvalid(MessageConstants.TIFF_HUL_44, info);
      }
      if (samplesPerPixel != 1) {
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_45.getMessage(), samplesPerPixel);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_45.getId(), mess);
        reportInvalid(message, info);
      }
      ...
      if (colormapBitCodeValue.length < len) {
        String mess =
            MessageFormat.format(
                MessageConstants.TIFF_HUL_46.getMessage(), colormapBitCodeValue.length, len);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_46.getId(), mess);
        reportInvalid(message, info);
      }
    }
    ...
    if (niso.getCompressionScheme() == 6 && ifd.getJPEGProc() == IFD.NULL) {
      reportInvalid(MessageConstants.TIFF_HUL_49, info);
    }
    ...
    if (photometricInterpretation == 8 || photometricInterpretation == 9) {
      ...
      if (in != 1 && in != 3) {
        String mess =
            MessageFormat.format(MessageConstants.TIFF_HUL_50.getMessage(), samplesPerPixel, len);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_50.getId(), mess);
        reportInvalid(message, info);
      }
    ...
    if (ifd.getClipPath() != null && ifd.getXClipPathUnits() == IFD.NULL) {
      reportInvalid(MessageConstants.TIFF_HUL_52, info);
    }
    ...
    if (dateTime != null) {
      if (dateTime.length() != 19) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_53.getId(), mess);
        reportInvalid(message, info);
        return;
      }
      if (dateTime.charAt(4) != ':'
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_54.getId(), mess);
        reportInvalid(message, info);
        return;
      }
      try {
        ...
        if (yyyy < 0
          ...
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_55.getId(), mess);
          reportInvalid(message, info);
        }
      } catch (Exception e) {
        String mess = MessageFormat.format(MessageConstants.TIFF_HUL_56.getMessage(), dateTime);
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_56.getId(), mess);
        reportInvalid(message, info);
      }
    }
  }
  ...
  protected List<IFD> parseIFDs(long offset, RepInfo info, boolean suppressErrors, int ifdType)
    ...
    while (next != 0L) {
      if ((next & 1) != 0) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_59.getId(), mess);
  ...
  protected IFD parseIFDChain(
      long next, RepInfo info, int type, List<IFD> list, boolean suppressErrors)
      throws TiffException {
    IFD ifd = null;
    switch (type) {
      case IFD.EXIF:
    ...
    if (ifd instanceof TiffIFD) {
      ...
      if ((offset = tifd.getInteroperabilityIFD()) != IFD.NULL) {
        IFD io = parseIFDChain(offset, info, IFD.INTEROPERABILITY, list, suppressErrors);
        tifd.setTheInteroperabilityIFD((InteroperabilityIFD) io);
      }
      if ((offset = tifd.getGlobalParametersIFD()) != IFD.NULL) {
        IFD io = parseIFDChain(offset, info, IFD.GLOBALPARAMETERS, list, suppressErrors);
  ...
  /**
   * Initializes the state of the module for parsing. This overrides the superclass method to reset
   * all the profile flags.
   */
  @Override
  ...
  /**
   * Return the index into _mimeType which should be used for the MIME type property. This must be
   * called after all the profiles have been checked. An index of 0 is dominant; if any profiles
   * return 0 from their getMimeClass method, or if conflicting values are returned by different
