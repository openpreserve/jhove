 ...
 *
 * <p>Image data content is not checked for profile conformance. Only tags are checked.
 *
 * @author Gary McGath
 */
public class TiffProfileFXM extends TiffFXBase {
  ...
  /**
   * Returns true if the IFD satisfies the requirements of a TIFF/FX M profile. See the TIFF/FX
   * specification for details.
   *
   * <p>Proper validation should check if the subIFDs are appropriate to the M profile layer scheme.
   * However, the existing design of the TIFF module has almost no understanding of IFD hierarchies.
   * This could be an enhancement for a future release.
   */
  @Override
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    if (!satisfiesCompression(tifd, new int[] {3, 4, 7, 9, 10})) {
      return false;
    ...
    if (!satisfiesSamplesPerPixel(tifd, new int[] {1, 3, 4})) {
      return false;
    }
    if (!satisfiesResolutionUnit(tifd, new int[] {2, 3, NisoImageMetadata.NULL})) {
      return false;
    }
    if (!satisfiesPhotometricInterpretation(tifd, new int[] {0, 1, 2, 5, 10})) {
      return false;
      // NOTE: The March 2003 draft allows only 0, 2 and
      // 10.  Watch for change.
    }
    if (!satisfiesFillOrder(tifd, new int[] {1, 2})) {
      return false;
    }
    ...
    if (tifd.getStripRowCounts() != null && niso.getRowsPerStrip() != NisoImageMetadata.NULL) {
      return false;
    }
