...
public final class TiffProfilePagemaker6 extends TiffProfile {
  ...
  /**
   * Returns true if the IFD satisfies the requirements of the profile. See the PageMaker
  ...
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    if (imageLength == NisoImageMetadata.NULL || niso.getImageWidth() == NisoImageMetadata.NULL) {
      return false;
    }
    ...
    if (!satisfiesCompression(tifd, new int[] {1, 2, 5, 32773, 32895, 32896})) {
      return false;
    }
    ...
    } else if (inkSet == 1 && spp != 4) {
      /* Only check for RGB, not hi-fi/multi-ink. */
      return false;
    }
    ...
    if (bps != null) {
      if (pi == 0 || pi == 1 || pi == 3) {
        for (int i = 0; i < bps.length; i++) {
          if (bps[i] != 1 && bps[i] != 2 && bps[i] != 4 && bps[i] != 8) {
            return false;
          }
        }
      } else {
    ...
    if (inkSet == 2 && (tifd.getInkNames() == null || tifd.getNumberOfInks() == IFD.NULL)) {
      return false;
    }
