...
/**
 * Profile checker for TIFF/EP. For TIFF/EP, no default values may be assumed. At the moment, we
 * have no way to determine which values were defaulted, so defaults are shown even if the file
 * satisfies the EP profile.
 *
 * <p>This class also serves as the base class for DNG, which is defined as a restricted subset of
 * TIFF/EP.
 */
public class TiffProfileEP extends TiffProfile {
  ...
  /**
   * Returns true if the IFD satisfies the requirements of a TIFF/EP profile. See the TIFF/EP
  ...
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    if (niso.getImageWidth() == NisoImageMetadata.NULL
        ...
        || (niso.getScannerModelName() == null && niso.getScannerModelNumber() == null)
    ...
    if (!satisfiesOrientation(tifd, new int[] {NisoImageMetadata.NULL, 1, 3, 6, 8, 9})) {
      return false;
    }
    ...
    if (pInterpretation == 6
        && (niso.getYCbCrCoefficients() == null
    ...
    if (compression != NisoImageMetadata.NULL
        && !(compression == 1 || compression == 7 || compression > 32767)) {
      return false;
    }
    return true;
  }
