...
public abstract class IFD {
      ...
      for (int i = 0; i < nFields; i++) {
        ...
        if (!DEBUG_ALLOW_OUT_OF_SEQUENCE && tag < prevTag) {
          ...
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_2.getId(), mess);
        ...
        if (type < BYTE || type > IFD) {
          String subMess =
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_3_SUB.getMessage(), type, Integer.valueOf(tag));
          _info.setMessage(
              new ErrorMessage(MessageConstants.TIFF_HUL_3, subMess, _offset + 4 + 12 * i));
        } else {
          ...
          if (calcValueSize(type, count) > 4) {
            ...
            if ((value & 1) != 0) {
              final String mess =
                  MessageFormat.format(MessageConstants.TIFF_HUL_4.getMessage(), value);
              JhoveMessage message =
                  JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_4.getId(), mess);
  ...
  /**
   * Sets flag indicating whether this is the "thumbnail" IFD. The second IFD in the top-level chain
   * is assumed to be the Thumbnail IFD.
   *
   ...
   * @param value
   * @param labels
   * @param rawOutput: true if string output is wanted, false returns a long property
  ...
  protected Property addBitmaskProperty(
    ...
    if (!rawOutput) {
      ...
      try {
        ...
        _errors.add(
            MessageFormat.format(
                MessageConstants.TIFF_HUL_66.getMessage(), name, Long.valueOf(value)));
      }
   ...
   * @param value
   * @param labels
   * @param rawOutput: true if integer output is wanted, false returns a string property
   ...
   * @param labels
   * @param index
   * @param rawOutput: true if int output is wanted, false returns a string property
   * @return property representing an integer value
   */
  protected Property addIntegerProperty(
    ...
    if (!rawOutput) {
      ...
      for (int i = 0; i < value.length; i++) {
        try {
          ...
          _errors.add(
              MessageFormat.format(
                  MessageConstants.TIFF_HUL_66.getMessage(), name, Long.valueOf(value[i])));
        }
      }
  ...
  protected Property propertyHeader(String type, List entries) {
    ...
    array[2] = new Property("Entries", PropertyType.PROPERTY, PropertyArity.LIST, entries);

    return new Property("IFD", PropertyType.PROPERTY, PropertyArity.ARRAY, array);
  }

  /**
   * Reads a string value from the TIFF file. If there are non-ASCII characters, they're escaped as
   * %XX
   *
  ...
  /** Returns <code>true</code> if file is big-endian, <code>false</code> if little-endian. */
  ...
  protected static void checkCount(int tag, long count, int minCount) throws TiffException {
    if (count < minCount) {
      String mess =
          MessageFormat.format(
              MessageConstants.TIFF_HUL_6.getMessage(),
              tag,
              Integer.valueOf(minCount),
              Long.valueOf(count));
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_6.getId(), mess);
   ...
   * @param count Tag entry count
   */
  protected static void checkCountArray(int tag, long count) throws TiffException {
  ...
  protected static void checkType(int tag, int type, int expected) throws TiffException {
    ...
    if ((type == BYTE || type == SHORT || type == LONG || type == IFD)
        && (expected == BYTE || expected == SHORT || expected == LONG || expected == IFD)) {
      return; // it's OK
    }
    if (type != expected) {
      String mess =
          MessageFormat.format(
              MessageConstants.TIFF_HUL_7.getMessage(),
              tag,
              Integer.valueOf(expected),
              Integer.valueOf(type));
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_7.getId(), mess);
      throw new TiffException(message);
    }
  }
  ...
  protected static void checkType(int tag, int type, int type1, int type2) throws TiffException {
    if (type != type1 && type != type2) {
      String mess =
          MessageFormat.format(
              MessageConstants.TIFF_HUL_8.getMessage(),
              tag,
              Integer.valueOf(type1),
              Integer.valueOf(type2),
              Integer.valueOf(type));
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_8.getId(), mess);
      throw new TiffException(message);
    }
  }
