...
public class InteroperabilityIFD extends IFD {
  ...
  public Property getProperty(boolean rawOutput) {
    ...
    entries.add(new Property("Index", PropertyType.STRING, _interoperabilityIndex));
      ...
      if (tag == INTEROPERABILITYINDEX) {
        ...
        String subMessage =
            MessageFormat.format(
                MessageConstants.TIFF_HUL_15_SUB.getMessage(), Integer.valueOf(tag));
        _info.setMessage(new ErrorMessage(MessageConstants.TIFF_HUL_15, subMessage, value));
      }
    } catch (IOException e) {
      String mess =
          MessageFormat.format(MessageConstants.TIFF_HUL_16.getMessage(), Integer.valueOf(tag));
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_16.getId(), mess);
