...
public class GlobalParametersIFD extends IFD {
  ...
  public Property getProperty(boolean rawOutput) {
    ...
    if (_profileType != NULL) {
      entries.add(addIntegerProperty("ProfileType", _profileType, PROFILETYPE_L, rawOutput));
    }
    if (_faxProfile != NULL) {
      entries.add(addIntegerProperty("FaxProfile", _faxProfile, FAXPROFILE_L, rawOutput));
    }
    if (_codingMethods != NULL) {
      entries.add(addBitmaskProperty("CodingMethods", _codingMethods, CODINGMETHODS_L, rawOutput));
    }
    if (_versionYear != null) {
      entries.add(new Property("VersionYear", PropertyType.STRING, _versionYear));
    }
    if (_modeNumber != NULL) {
      entries.add(new Property("ModeNumber", PropertyType.INTEGER, new Integer(_modeNumber)));
    }
  ...
  public void lookupTag(int tag, int type, long count, long value) throws TiffException {
    try {
      ...
      String mess =
          MessageFormat.format(MessageConstants.TIFF_HUL_19.getMessage(), Integer.valueOf(tag));
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.TIFF_HUL_19.getId(), mess);
