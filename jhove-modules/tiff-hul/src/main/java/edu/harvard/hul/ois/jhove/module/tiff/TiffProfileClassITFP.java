 ...
 *
 * <p>The TIFF/IT spec states that "TIFF/IT-FP provides a mechanism for associating image files of
 * the different types that make up a final page." Note that Jhove profiles are applied to
 * individual IFD levels, so this profile does not check the relationships among IFDs which are part
 * of the FP specification.
 *
 * @author Gary McGath
 */
public final class TiffProfileClassITFP extends TiffProfileClassIT {
  ...
  /**
   * Returns true if the IFD satisfies the requirements of the profile. See the documentation for
   * details.
   */
  @Override
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    if (!(bps == null
        ...
        || (bps.length == 4 && bps[0] == 8 && bps[1] == 8 && bps[2] == 8 && bps[3] == 8))) {
      return false;
    }
    ...
    if (!satisfiesPhotometricInterpretation(tifd, new int[] {0, 1, 2, 5})) {
      return false;
    }

    // SamplesPerPixel=3 or 4 or undefined (consistent with
    // PhotometricInterpretation)
    if (!satisfiesSamplesPerPixel(tifd, new int[] {3, 4, NisoImageMetadata.NULL})) {
      return false;
    }
    ...
    return !(pint == 5 && (inkSet != 1 || nInks != 4 || !satisfiesDotRange(tifd, 0, 255)));
  }
}
