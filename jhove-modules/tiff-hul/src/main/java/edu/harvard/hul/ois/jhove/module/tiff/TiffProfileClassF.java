...
public final class TiffProfileClassF extends TiffProfile {
  ...
  /**
   * Returns true if the IFD satisfies the requirements of a Class F profile. See the Class F
  ...
  public boolean satisfiesThisProfile(IFD ifd) {
    ...
    if (cmpr == 3) {
      ...
      if (t4opt != 0 && t4opt != 1 && t4opt != 4 && t4opt != 5) {
        return false;
      }
    } else if (cmpr == 4) {
    ...
    if (!satisfiesImageWidth(
        tifd, new int[] {1728, 2048, 2432, 2592, 3072, 3648, 3456, 4096, 4864})) {
      return false;
    }
    ...
    if (!satisfiesXResolution(tifd, new int[] {204, 200, 300, 400, 408})) {
      return false;
    }

    if (!satisfiesYResolution(tifd, new int[] {98, 196, 100, 200, 300, 391, 400})) {
      return false;
    }
