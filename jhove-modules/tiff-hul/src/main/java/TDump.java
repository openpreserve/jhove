...
public class TDump extends Dump {
  ...
  public static void main(String[] args) {
    ...
    RandomAccessFile file = new RandomAccessFile(tiff, "r")) {
      ...
      _nIFDs = 0;
      while ((offset = readIFD(file, bigEndian, offset, nobyte, nosub)) > 0) {}
      file.close();
    } catch (Exception e) {
   ...
   * @param nobyte If true, only display the first 8 bytes of data of type BYTE
   * @param nosub If true, do not parse subIFDs
   */
  private static long readIFD(
      RandomAccessFile file, boolean bigEndian, long offset, boolean nobyte, boolean nosub)
      throws Exception {
    int nIFD = ++_nIFDs;
    ...
    _tags.put(leading(offset, 8) + offset, "IFD " + nIFD + " with " + nEntries + " entries");
    ...
    for (int i = 0; i < nEntries; i++) {
      ...
      StringBuffer buffer =
          new StringBuffer(
              tag + " (" + TiffTags.tagName(tag) + ") " + IFD.TYPE[type] + " " + count);
      if (type == IFD.ASCII) {
        if (count > 4) {
      ...
      } else if (type == IFD.BYTE || type == IFD.UNDEFINED) {
        if (count > 4) {
          long vo = ModuleBase.readUnsignedInt(file, bigEndian);
          file.seek(vo);
