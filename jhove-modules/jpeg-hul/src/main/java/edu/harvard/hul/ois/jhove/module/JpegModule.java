 ...
 * <p>General notes:
 *
 * <p>There is no such thing as a "JPEG file format." There are several commonly used file formats
 * which encapsulate JPEG data and conform to the JPEG stream format. There are also many formats
 * which can encapsulate JPEG data within some larger wrapper; this module does not attempt to
 * recognize them. Only JPEG file formats which are JPEG streams are treated here. A JPEG stream
 * which isn't one of the known file formats will be regarded as well-formed, but not valid. To be
 * valid, a file must conform to one of the following: JFIF, SPIFF, and JPEG/Exif. Other formats may
 * be added in the future.
 *
 * <p>This module uses the JPEG-L method of detecting a marker following a data stream, checking for
 * a 0 high bit rather than an entire 0 byte. So long at no JPEG markers are defined with a value
 * from 0 through 7F, this is valid for all JPEG files.
 *
 * <p>* @author Gary McGath
 */
public class JpegModule extends ModuleBase {
  /**
   * **************************************************************** DEBUGGING FIELDS. All
   * debugging fields should be set to false for release code.
   * ****************************************************************
   */
  ...
  public JpegModule() {
    ...
    Document doc =
        new Document(
            "Eric Hamilton, JPEG File Interchange Format, " + "Version 1.02, September 1, 1992",
            DocumentType.WEB);
    Agent agent =
        new Agent.Builder("C-Cube Microsystems", AgentType.COMMERCIAL)
            .address("1778 McCarthy Boulevard, Milipitas, CA 95035")
            .telephone("+1 (408) 944-6314")
            .fax("+1 (408) 944-6314")
            .build();
    doc.setPublisher(agent);
    doc.setDate("1992-09-01");
    doc.setIdentifier(
        new Identifier("http://www.w3.org/Graphics/JPEG/jfif3.pdf", IdentifierType.URL));
    ...
    doc =
        new Document(
            ...
            DocumentType.STANDARD);
    doc.setPublisher(isoAgent);
    doc.setIdentifier(new Identifier("ITU-T Rec. T.872 (06/12)", IdentifierType.CCITT));
    _specification.add(doc);
    ...
    Signature sig =
        new InternalSignature(sigByte, SignatureType.MAGIC, SignatureUseType.MANDATORY, 0, "");
    _signature.add(sig);
   ...
   *
   * <p>This function uses the JPEG-L method of detecting a marker following a data stream, checking
   * for a 0 high bit rather than an entire 0 byte. So long at no JPEG markers are defined with a
   * value from 0 through 7F, this is valid for all JPEG files.
   *
   * @param stream An InputStream, positioned at its beginning, which is generated from the object
   *     to be parsed
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    _metadata = new Property("JPEGMetadata", PropertyType.PROPERTY, PropertyArity.LIST, _propList);
    if (!readHeader(info)) {
      return 0;
    }
    _niso = new NisoImageMetadata();
    Property nisoProp = new Property(NISO_IMAGE_MD, PropertyType.NISOIMAGEMETADATA, _niso);
    ...
    try {
      ...
      for (; ; ) {
        ...
        if (!_seenJFIF
            ...
            && _numSegments >= 2
            && !_reportedJFIF) {
          info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_9, _nByte));
          info.setValid(false);
          _reportedJFIF = true;
        }
        if (dbyt >= 0xD0 && dbyt <= 0xD7) {
          ...
          switch (dbyt) {
            ...
            default:
              ...
              msg = new ErrorMessage(MessageConstants.JPEG_HUL_6, _nByte);
              info.setMessage(msg);
              info.setValid(false);
              break loop1;
          }
      ...
      msg = new ErrorMessage(MessageConstants.JPEG_HUL_2, _nByte);
      info.setMessage(msg);
      info.setWellFormed(false);
      return 0;
    }
    ...
    if (_units == 0) {
      ...
      _primaryImageList.add(
          new Property("PixelAspectRatio", PropertyType.PROPERTY, PropertyArity.LIST, list));
    }
    ...
    if (_restartInterval >= 0) {
      _primaryImageList.add(
          new Property("RestartInterval", PropertyType.INTEGER, _restartInterval));
    }
    ...
    if (!_quantTables.isEmpty()) {
      ...
      _primaryImageList.add(
          new Property("QuantizationTables", PropertyType.PROPERTY, PropertyArity.LIST, qpl));
    }
    ...
    if (!_arithCondTables.isEmpty()) {
      ...
      _primaryImageList.add(
          new Property("ArithmeticConditioning", PropertyType.PROPERTY, PropertyArity.LIST, qpl));
    }
    if (!_srsList.isEmpty()) {
      ...
      _primaryImageList.add(
          new Property("SelectivelyRefinedScans", PropertyType.PROPERTY, PropertyArity.LIST, srsl));
    }
    ...
    _imageList.add(
        new Property("Image", PropertyType.PROPERTY, PropertyArity.LIST, _primaryImageList));
     ...
     */
    List<Property> list = new ArrayList<Property>();
    list.add(new Property("Number", PropertyType.INTEGER, PropertyArity.SCALAR, _imageList.size()));
    Iterator<Property> iter = _imageList.iterator();
    while (iter.hasNext()) {
      Property prop = iter.next();
      list.add(prop);
    }
    _propList.add(new Property("Images", PropertyType.PROPERTY, PropertyArity.LIST, list));
    // _imageList));
    if (!_commentsList.isEmpty()) {
      _propList.add(
          new Property("Comments", PropertyType.STRING, PropertyArity.LIST, _commentsList));
    }
    if (!_jpegExtsList.isEmpty()) {
      _propList.add(
          new Property("Extensions", PropertyType.STRING, PropertyArity.LIST, _jpegExtsList));
    }
    if (!_appSegsList.isEmpty()) {
      _propList.add(
          new Property(
              "ApplicationSegments", PropertyType.STRING, PropertyArity.LIST, _appSegsList));
    }
    if (!_expList.isEmpty()) {
  ...
  /**
   * One-argument version of <code>readUnsignedShort</code>. JPEG is always big-endian, so
   * readUnsignedShort can unambiguously drop its endian argument.
   */
  ...
  /**
   * One-argument version of <code>readUnsignedInt</code>. JPEG is always big-endian, so
   * readUnsignedInt can unambiguously drop its endian argument.
   */
  ...
  protected boolean readHeader(RepInfo info) {
    ...
    if (!valid) {
      info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_4, 0));
      info.setWellFormed(false);
      return false;
    }
    return true;
    ...
    if (equalArray(ident, jfifByte)) {
      if (_numSegments > 1) {
        if (!_seenExif) {
          ...
          info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_5, _nByte));
          info.setValid(false);
      ...
      String vsn = Integer.toString(majorVersion) + "." + minorFmt.format(minorVersion);
      info.setVersion(vsn);
      ...
      if (xThumbPix > 0 && yThumbPix > 0) {
        ...
        Property thumbProp =
            new Property("ThumbImage", PropertyType.PROPERTY, PropertyArity.LIST, thumbPropList);
  ...
  protected void readAPP1(RepInfo info) throws IOException {
    ...
    if (equalArray(ident, exifByte)) {
      ...
      if (!JpegExif.isTiffAvailable()) {
        info.setMessage(new InfoMessage(MessageConstants.INF_EXIF_REPORT_REQUIRES_TIFF, _nByte));
        skipBytes(_dstream, length - 8, this);
        return;
      }
  ...
  protected void readAPP8(RepInfo info) throws IOException {
    ...
    if (equalArray(ident, spiffByte)) {
      if (_numSegments > 1) {
        info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_8, _nByte));
        info.setValid(false);
      }
  ...
  protected void readAPP2(RepInfo info) throws IOException {
    ...
    if (chunkNumber == numberOfChunks) {
      ...
      try {
        ...
        String desc = NisoImageMetadata.extractIccProfileDescription(_baosIccProfile.toByteArray());
        if (desc != null) {
          _niso.setProfileName(desc);
        }
      } catch (IllegalArgumentException ie) {
        info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_11, ie.getMessage(), _nByte));
      }
    }
  }
  ...
  protected void readVer(RepInfo info) throws IOException {
    ...
    String vsn = Integer.toString(majVersion) + "." + minorFmt.format(minVersion);
    info.setVersion(vsn);
  ...
  protected void readDTT(RepInfo info) throws IOException {
    readUnsignedShort(_dstream);
    if (_tiling == null) {
      info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_1, _nByte));
      info.setValid(false);
      return;
    }
  ...
  protected void readAPP14(RepInfo info) throws IOException {
    ...
    if (_transformFlag > 2) {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.JPEG_HUL_12.getId(), mess);
  ...
  protected void readSOF(int dbyt, RepInfo info) throws IOException {
    ...
    if (!_seenSOF) {
      ...
      _propList.add(
          new Property(
              "CompressionType", PropertyType.STRING, JpegStrings.COMPRESSION_TYPE[dbyt - 0xC0]));
      _seenSOF = true;
      ...
      switch (numComps) {
        ...
        default: // others ?!?
          ...
          JhoveMessage message =
              JhoveMessages.getMessageInstance(MessageConstants.JPEG_HUL_13.getId(), mess);
          info.setMessage(new ErrorMessage(message, _nByte));
          info.setValid(false);
          break;
      }
  ...
  protected Property buildCapProp(RepInfo info) {
    ...
    try {
      ...
      if (_je.getShowRawFlag()) {
        ...
        cap0Prop =
            new Property("Version0", PropertyType.STRING, JpegStrings.CAPABILITY_V0[_capability0]);
      }
      capList.add(cap0Prop);

      if (_capability1 >= 0) {
        if (_je.getShowRawFlag()) {
          Property cap1Prop = new Property("Version1", PropertyType.INTEGER, _capability1);
          ...
          Property cap1Prop =
              new Property("Version1", PropertyType.STRING, PropertyArity.ARRAY, cap1Str);
          capList.add(cap1Prop);
        }
      }
      return new Property(
          "CapabilityIndicator", PropertyType.PROPERTY, PropertyArity.LIST, capList);
    } catch (Exception e) {
  ...
  protected Property buildTilingProp(RepInfo info) {
    ...
    try {
      ...
      if (_je.getShowRawFlag()) {
        ...
        propArr[0] =
            new Property("TilingType", PropertyType.STRING, JpegStrings.TILING_TYPE[tilingType]);
      }
      ...
      info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_10));
      info.setValid(false);
      return null;
    }
  }
  ...
  protected Property buildExpandProp(RepInfo info) {
    ...
    Property prop =
        new Property("ExpansionSegments", PropertyType.PROPERTY, PropertyArity.LIST, plist);
  ...
  protected void extractExifNisoData(NisoImageMetadata exifData) {
    ...
    if (exifData.getDigitalCameraManufacturer() != null) {
      _niso.setDigitalCameraManufacturer(exifData.getDigitalCameraManufacturer());
    }
    ...
    if (exifData.getProcessingSoftwareName() != null) {
      _niso.setProcessingSoftwareName(exifData.getProcessingSoftwareName());
    }
    if (exifData.getProcessingSoftwareVersion() != null) {
      _niso.setProcessingSoftwareVersion(exifData.getProcessingSoftwareVersion());
    }
    ...
    if (_niso.getProfileName() == null && exifData.getProfileName() != null) {
