...
/**
 * Reader of Exif data embedded in a JPEG App1 block. This makes use of the TIFF module, since an
 * Exif stream is really an embedded TIFF file; but it is designed to fail cleanly if the TIFF
 * module is absent.
 *
 * @author Gary McGath
 */
public final class JpegExif {
  ...
  /**
   * Reads the Exif data from the current point at the data stream, puts it into a temporary file,
   * and makes a RepInfo object available. This should be called only if isTiffAvailable() has
  ...
  public RepInfo readExifData(DataInputStream dstream, JhoveBase je, int length) {
    ...
    try {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.JHOVE_1, e.getMessage()));
      return info;
    }
    try (FileOutputStream fos = new FileOutputStream(tiffFile)) {
      ...
      while (iter.hasNext()) {
        ...
        if (ifd instanceof TiffIFD && first) {
          ...
          info.setProperty(new Property("NisoImageMetadata", PropertyType.NISOIMAGEMETADATA, niso));
        ...
        if (ifd instanceof ExifIFD) {
          ...
          if (exifList != null) {
            info.setProperty(
                new Property("Exif", PropertyType.PROPERTY, PropertyArity.LIST, exifList));
          }
          ...
          if (!haveNisoMetadata) {
            ...
            info.setProperty(
                new Property("NisoImageMetadata", PropertyType.NISOIMAGEMETADATA, niso));
          } else {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.JPEG_HUL_3, e.getMessage()));
  ...
  /**
   * Returns <code>true</code> if the Exif IFD is present and satisfies the profile requirements.
   */
