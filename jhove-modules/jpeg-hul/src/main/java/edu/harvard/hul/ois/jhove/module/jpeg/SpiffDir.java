...
/**
 * This class represents a SPIFF directory and the tags defined under it. A SPIFF directory consists
 * of one or more APP8 segments, and may define ancillary images. It is always contained within the
...
public class SpiffDir {
  ...
  /**
   * Reads a directory entry, starting at the position after the APP8 marker and length. If the
   * entry is for a thumbnail, create a Property for that thumbnail and add it to the thumbnail
   * list. Other tags provide interesting information, some of which should go into properties, but
   * for the moment we just handle the thumbnail and ignore other tags.
   *
   * <p>An APP8 segment which is in a SPIFF file, and isn't the first APP8 segment (file header), is
   * presumed to be a directory entry. These directory entries are a little inconvenient, because
   * they can contain offsets to data in what we otherwise handle as a stream format. The offsets
   * can be either to data within the block, or to faraway indirect data blocks. For the present
   * version, we ignore offset data, which seems to be used only for the actual image bits (e.g.,
   * TNDATA).
   */
  ...
  private void readThumbnail(DataInputStream dstream, int length) throws IOException {
    ...
    Property imageProp =
        new Property("ThumbImage", PropertyType.PROPERTY, PropertyArity.LIST, propList);
