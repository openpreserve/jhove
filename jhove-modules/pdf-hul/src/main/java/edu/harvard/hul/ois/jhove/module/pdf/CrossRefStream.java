...
/**
 * This class implements the Cross-Reference Stream, an alternative to the Cross-Reference Table
 * starting in PDF 1.4.
 *
 * <p>A cross-reference stream is identified by a startxref keyword, as opposed to the xref keyword
 * which identifies the old-style cross-reference table.
 *
 * <p>JHOVE supports only FlateDecode as a filter for cross-reference streams. This is consistent
 * with the implementation limitation described in Appendix H of the PDF manual for Acrobat 6 and
 * earlier.
 *
 * @author Gary McGath
 */
public class CrossRefStream {
  ...
  /** Range elements of the _index array: Starting object and number of objects. */
   ...
   * Constructor.
   *
   * @param xstrm PdfStream object which contains a presumed cross-reference stream.
   */
  ...
  /**
   * Returns <code>true</code> if the PdfStream object meets the requirements of a cross-reference
   * stream. Also extracts information from the dictionary for subsequent processing.
   */
  ...
  /**
   * Prepares for reading the Stream. If the filter list includes one which we don't support, throws
   * a PdfException.
   */
  public void initRead(RandomAccessFile raf) throws IOException {
   ...
   *
   * <p>After calling <code>readObject</code>, it is possible to call accessors to get information
   * about the object. For the moment, we punt on the question of how to deal with Object Streams.
   *
   * <p>Free objects are skipped over while being counted. After <code>readNextObject</code> returns
   * <code>false</code>, the caller may call <code>getFreeCount</code> to determine the number of
   * free objects.
   *
   * @return <code>true</code> if there is an object, <code>false</code> if no more objects are
   *     available.
   */
  ...
  /**
   * Returns number of the last object read by <code>readNextObject ()</code>. Do not call if <code>
   * readNextObject ()</code> returns <code>false</code>.
   */
  ...
  /**
   * Returns <code>true</code> if the last object read by <code>readNextObject ()</code> is a
   * compressed object. Do not call if <code>readNextObject ()</code> returns <code>false</code>.
   */
  ...
  /**
   * Returns the number of free objects detected. This may be called after <code>readNextObject
   * </code> returns <code>false</code>, signifying that all the objects have been read and all the
   * free objects counted.
   */
  ...
  /**
   * Returns the total number of objects in the document's cross-reference table at the time this
   * stream was written.
   */
  ...
  /**
   * Returns the offset of the last object read. This is meaningful only if the last object read was
   * type 1 (uncompressed).
   */
  ...
  /**
   * Returns the object number of the content stream in which this object is stored. This is
   * meaningful only if the last object read was type 2 (compressed in content stream).
   */
  ...
  /**
   * Returns the content stream index of the last object read. This is meaningful only if the last
   * object read was type 2 (compressed in content stream).
   */
