...
/**
 * PDF profile checker for PDF/A-1 documents. See 19005-1:2005(E), "Document Imaging Applications
 * Application Issues".
 *
 * <p>Revised to reflect the final standard. With the new terminology, this profile is specific to
 * PDF/A-1; there may be additional standards in the PDF/A family later on. "PDF/A" means "PDF/A-1"
 * in the documentation of this code.
 *
 * <p>There are two levels of conformance, called Level A and Level B. We report these as two
 * different profiles. To accomplish this, we use AProfileLevelA, linked to an instance of this,
 * which simply checks if this profile established Level A compliance.
 */
public final class AProfile extends PdfProfile {
  ...
  /**
   * Returns <code>true</code> if the document satisfies the profile at Level B or better. Also sets
   * the level A flag to the appropriate value, so that <code>satisfiesLevelA</code> can
  ...
  public boolean satisfiesThisProfile() {
    ...
    // for Level A
    if (_taggedProfile != null && !_taggedProfile.isAlreadyOK()) {
      _levelA = false;
  ...
  /**
   * Returns true if the document was found to be Level A conformant. This returns a meaningful
   * result only after <code>satisfiesThisProfile</code> has been called, and is intended for use by
   * the Level A profiler.
   */
  ...
  private boolean catalogOK() {
    ...
    try {
      ...
      PdfStream metadata = (PdfStream) _module.resolveIndirectObject(cat.get("Metadata"));
      if (!metadataOK(metadata)) {
        return false;
      }

      // If it has an interactive form, it must meet certain criteria
      PdfDictionary form = (PdfDictionary) _module.resolveIndirectObject(cat.get("AcroForm"));
  ...
  private boolean fontOK(PdfDictionary font) {
    try {
      ...
      PdfDictionary desc =
          (PdfDictionary) _module.resolveIndirectObject(font.get("FontDescriptor"));
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        if (enc instanceof PdfDictionary) {
          ...
          PdfDictionary info =
              (PdfDictionary)
                  _module.resolveIndirectObject(((PdfDictionary) enc).get("CIDSystemInfo"));
        ...
        PdfArray descendants =
            (PdfArray) _module.resolveIndirectObject(font.get("DescendantFonts"));
        ...
        PdfDictionary subfont = (PdfDictionary) _module.resolveIndirectObject(objFont);
        PdfSimpleObject subtype = (PdfSimpleObject) subfont.get("Subtype");
        /*
        ...
        PdfDictionary info =
            (PdfDictionary) _module.resolveIndirectObject(subfont.get("CIDSystemInfo"));
        ob = (PdfSimpleObject) info.get("Registry");
        ...
        if ("CIDFontType2".equals(subtype.getStringValue())) {
          ...
          if (cgmap1 instanceof PdfIndirectObj) {
            cgmap = (PdfObject) _module.resolveIndirectObject(cgmap1);
          } else {
            cgmap = cgmap1;
          }
  ...
  private boolean ttFontEncodingOK(PdfDictionary font) {
    try {
      ...
      if ((flags & 4) == 0) {
        ...
        PdfSimpleObject encoding = (PdfSimpleObject) font.get("Encoding");
        String encStr = encoding.getStringValue();
        if (!"MacRomanEncoding".equals(encStr) && !"WinAnsiEncoding".equals(encStr)) {
          return false;
        }
      }
    } catch (Exception e) {
  ...
  */
    ...
    try {
      ...
      for (; ; ) {
        ...
        if (rsrc != null) {
          ...
          // Check color spaces.
          PdfDictionary cs = (PdfDictionary) _module.resolveIndirectObject(rsrc.get("ColorSpace"));
          if (!colorSpaceOK(cs)) {
            return false;
          }
        ...
        if (docNode instanceof PageObject) {
          ...
          if (streams != null) {
            ...
            while (iter.hasNext()) {
              ...
              PdfDictionary rs =
                  (PdfDictionary) _module.resolveIndirectObject(dict.get("Resources"));
              if (rs != null) {
                PdfDictionary cs =
                    (PdfDictionary) _module.resolveIndirectObject(rs.get("ColorSpace"));
                if (!colorSpaceOK(cs)) {
                  return false;
                }
          ...
          if (annots != null) {
            ...
            for (int i = 0; i < annVec.size(); i++) {
              PdfDictionary annDict =
                  (PdfDictionary) _module.resolveIndirectObject(annVec.elementAt(i));
      ...
      for (int i = 0; i < intVec.size(); i++) {
        ...
        if (outCond != null) {
          PdfStream outProfile =
              (PdfStream) _module.resolveIndirectObject(intent.get("DestOutputProfile"));
          if (outProfile != null) {
            ...
            if (subtype != null && "GTS_PDFA1".equals(subtype.getStringValue())) {
  ...
  private boolean outlinesOK() {
    ...
    try {
      PdfDictionary item = (PdfDictionary) _module.resolveIndirectObject(outlineDict.get("First"));
      while (item != null) {
        if (!checkOutlineItem(item)) {
          return false;
        }
        item = (PdfDictionary) _module.resolveIndirectObject(((PdfDictionary) item).get("Next"));
      }
    } catch (Exception e) {
      return false;
    }
  ...
  private boolean checkOutlineItem(PdfDictionary item) {
    ...
    try {
      PdfDictionary action = (PdfDictionary) _module.resolveIndirectObject(item.get("A"));
      if (action != null && !actionOK(action)) {
        return false;
      }
      PdfDictionary child = (PdfDictionary) _module.resolveIndirectObject(item.get("First"));
      PdfDictionary next;
      while (child != null) {
        if (!checkOutlineItem(child)) {
          return false;
        }
        next = (PdfDictionary) _module.resolveIndirectObject(child.get("Next"));
  ...
  private boolean extGStateOK(PdfDictionary gs) {
    ...
    try {
      ...
      if (blendMode != null) {
        ...
        if (!"Normal".equals(bmVal) && !"Compatible".equals(bmVal)) {
          return false;
        }
      }
