 ...
 *
 * <p>This is used in conjunction with the Parser, which assembles Tokens into higher-level
 * constructs.
 */
public abstract class Tokenizer {
  ...
  private static final int WHITESPACES[] = {NUL, HT, LF, FF, CR, ' '};
  ...
  /** If <code>true</code>, use the look-ahead character, rather than reading from the file. */
  ...
  /**
   * Parses out and returns a token from the input file. If it hits the end of the file, returns
   * null. Other parsing problems cause an exception to be thrown. When an exception is thrown, the
   * state is changed to WHITESPACE, so the parser can get back in sync more easily.
   */
      ...
      while (true) {
        ...
        if (!_lookAhead) {
          ...
          if (_ch < 0) {
            ...
            throw new PdfMalformedException(
                MessageConstants.PDF_HUL_64, // PDF-HUL-64
                _offset);
          }
          _offset++;
        } else {
        ...
        if (_state == (State.WHITESPACE)) {
          ...
          if (_ch == '.') {
            ...
            realValue = intValue;
            denom = 10;
          } else if (isDelimiter(_ch) || isWhitespace(_ch) || !isNumeral(_ch)) {
            if (negative) {
  ...
  /**
   * Returns the value of the pdfACompliant flag, which indicates that the tokenizer hasn't detected
   * non-compliance. A value of <code>true</code> is no guarantee that the file is compliant.
   */
  ...
  /**
   * Returns the value of the last white space string read by the tokenizer. Repositioning clears
  ...
  /**
   * Returns <code>true</code> if <code>ch</code> is a non-whitespace character which delimits a
   * token.
   */
   ...
   */
  protected abstract void initStream(Stream token) throws IOException, PdfException;

  /**
   * Sets the offset of a Stream to the current file position. Only the file-based tokenizer can do
   * this, which is why this overrides the Tokenizer method.
   */
