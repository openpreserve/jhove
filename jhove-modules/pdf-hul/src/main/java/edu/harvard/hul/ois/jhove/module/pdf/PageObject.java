...
public class PageObject extends DocNode {
   ...
   * @param parent The parent node in the document tree; may be null only for the root node
   * @param dict The dictionary object on which this node is based
   */
  ...
  /**
   * Find the content stream(s) for this page. This is called when the page tree content stream is
   * built by PageTreeNode. <code>getContentStreams</code> may subsequently be called to get the
   * content.
   */
  ...
  /**
   * Call this function when recursively walking through a document tree. This allows nextPageObject
   * () to be return this object exactly once.
   */
  @Override
  ...
  /**
   * Returns this object the first time it is called after startWalk is called, then null when
   * called again. This allows a recursive walk through a document tree to work properly.
   */
  @Override
  public PageObject nextPageObject() {
    if (_walkFinished) return null;
    _walkFinished = true;
    return this;
  }

  /**
   * Called to walk through all page tree nodes and page objects. Functionally identical with
   * nextPageObject.
   */
  @Override
  ...
  /**
   * Returns the ArtBox for the page, or null if none. Throws a PDFException if there is an ArtBox
   * but it is not a rectangle.
   */
  public PdfArray getArtBox() throws PdfException {
    return retrieveAndCheckRectangle(
        this._dict, "ArtBox", MessageConstants.PDF_HUL_23); // PDF-HUL-23
  }

  /**
   * Returns the TrimBox for the page, or null if none. Throws a PDFException if there is an TrimBox
   * but it is not a rectangle.
   */
  public PdfArray getTrimBox() throws PdfException {
    return retrieveAndCheckRectangle(
        this._dict, "TrimBox", MessageConstants.PDF_HUL_24); // PDF-HUL-24
  }

  /**
   * Returns the BleedBox for the page, or null if none. Throws a PDFException if there is an
   * BleedBox but it is not a rectangle.
   */
  public PdfArray getBleedBox() throws PdfException {
    return retrieveAndCheckRectangle(
        this._dict, "BleedBox", MessageConstants.PDF_HUL_25); // PDF-HUL-25
  }

  private static PdfArray retrieveAndCheckRectangle(
      final PdfDictionary dict, final String dictKey, final JhoveMessage invalidMessage)
  ...
  private void processContents(PdfModule module, final PdfObject contents)
  ...
  private void loadContentFromArray(PdfModule module, final PdfArray contents)
      throws PdfException, IOException {
