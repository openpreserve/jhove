...
/**
 * Class for nodes of a PDF name tree, e.g., ExternalFiles. Name trees are intended for large
 * amounts of data that won't have to all be brought into memory at once; so this implementation is
 * geared toward file-based lookup of a key rather than creating an in-memory structure. Descendant
 * nodes become available for garbage collection if they are not on the search path for a match.
 *
 * <p>Keys are collated according to raw bytes, not character encoding.
 */
public class NameTreeNode {
   ...
   */
  public NameTreeNode(PdfModule module, NameTreeNode parent, PdfDictionary dict)
      throws PdfException {
    _module = module;
    _parent = parent;
    _dict = dict;
  ...
  /**
   * See if a key is within the bounds of this node. All keys are within the bounds of the root
   * node.
   */
  public boolean inBounds(Vector key) {
    ...
    return !(compareKey(key, _lowerLimit) < 0 || compareKey(key, _upperLimit) > 0);
  }
    ...
    try {
      ...
      if (_names != null) {
        for (int i = 0; i < _names.size(); i += 2) {
          ...
          if (cmp == 0) {
            /* Match! */
            return _module.resolveIndirectObject((PdfObject) _names.elementAt(i + 1));
          } else if (cmp < 0) {
        ...
        for (int i = 0; i < _kids.size(); i++) {
          PdfDictionary kid =
              (PdfDictionary) _module.resolveIndirectObject((PdfObject) _kids.elementAt(i));
