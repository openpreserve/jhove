...
/**
 * PDF profile checker for PDF/X-1 documents. See ISO Standard 15930-1, "Complete exchange using
...
public final class X1Profile extends XProfileBase {
  ...
  /**
   * Returns <code>true</code> if the document satisfies the profile. X-1a compliance is a superset
   * of the requirements of X-1 compliance (i.e., X-1a compliant documents are a subset of X-1
   * compliant documents), so we test for X-1a compliance at the same time. The result can
   * subsequently be obtained by calling <code>isX1aCompliant</code>.
   */
  @Override
  ...
  /**
   * Returns the result of X-1a compliance testing which was performed in the course of <code>
   * satisfiesThisProfile</code>. If <code>satisfiesThisProfile</code> hasn't been called, returns
   * <code>false</code>.
   */
  ...
  */
            ...
            for (int i = 0; i < annVec.size(); i++) {
              ...
              if ("TrapNet".equals(subtypeObj.getStringValue())) {
                ...
                if (appDict != null) {
                  ...
                  if (normalDict != null) {
                    PdfSimpleObject pcm = (PdfSimpleObject) normalDict.get("PCM");
    ...
    while (iter.hasNext()) {
      ...
      if (res instanceof PdfArray) {
        ...
        if ("Separation".equals(sname) || "DeviceN".equals(sname)) {
          ...
          if (!("DeviceGray".equals(altSpace) || "DeviceCMYK".equals(altSpace))) {
            return false;
          }
        }
        if ("Indexed".equals(sname) || "Pattern".equals(sname)) {
          // Indexed and pattern color spaces must have a
          // base colorspace of DeviceCMYK, DeviceGray,
          // DeviceN, or Separation.
          PdfSimpleObject baseObj = (PdfSimpleObject) resv.elementAt(1);
  ...
  protected boolean xObjectOK(PdfDictionary xo) {
    ...
    try {
      ...
      PdfDictionary opi20 = (PdfDictionary) _module.resolveIndirectObject(opi.get("2.0"));
      if (opi20 == null) {
        return true;
      }
      ...
      PdfObject inks = _module.resolveIndirectObject(opi20.get("Inks"));
      if (inks == null) {
        return true;
      }
      if (inks instanceof PdfSimpleObject) {
        String inkname = ((PdfSimpleObject) inks).getStringValue();
        if (!("full_color".equals(inkname) || "registration".equals(inkname))) {
          return false;
        }
      } else if (inks instanceof PdfArray) {
        Vector<PdfObject> inkvec = ((PdfArray) inks).getContent();
        PdfSimpleObject inkobj = (PdfSimpleObject) inkvec.elementAt(0);
  ...
  private boolean filter1AOK(PdfObject filters) {
    return !hasFilters(filters, new String[] {"LZWDecode", "JBIG2Decode"});
  }
