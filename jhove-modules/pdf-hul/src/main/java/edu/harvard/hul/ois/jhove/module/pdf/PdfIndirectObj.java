...
public class PdfIndirectObj extends PdfObject {
  ...
  /**
   * Retrieves the object which is referenced. Uses the cached reference if there is one; caches the
   * reference if there wasn't one before.
   */
  public PdfObject getObject() {
    if (_cachedObject != null) {
      return _cachedObject;
    }
    long key = ((long) _objNumber << 32) + (_genNumber & 0XFFFFFFFFL);
