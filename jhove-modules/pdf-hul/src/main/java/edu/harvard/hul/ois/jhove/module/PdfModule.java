 ...
 * Validation Environment Copyright 2003-2007 by JSTOR and the President and Fellows of Harvard
 * College
 *
...
public class PdfModule extends ModuleBase {
  ...
  private static final String RIGHTS =
      "Copyright 2003-2007 by JSTOR and "
  ...
  private static final String[] compressionStrings = {
    FILTER_NAME_LZW, /* "FlateDecode", */ FILTER_NAME_RUN_LENGTH, FILTER_NAME_DCT, FILTER_NAME_CCITT
  };
  private static final int[] compressionValues = {5, /* 8, */ 32773, 6, 2};
  /*
  ...
  private static final String[] colorSpaceStrings = {
  ...
  public PdfModule() {
    ...
    Document doc =
        new Document(
            "PDF Reference: Adobe Portable " + "Document Format, Version 1.4", DocumentType.BOOK);
    ...
    doc =
        new Document(
            "PDF Reference: Adobe Portable " + "Document Format, Version 1.5", DocumentType.BOOK);
    doc.setPublisher(agent);
    doc.setDate("2003");
    ...
    doc =
        new Document(
            "PDF Reference: Adobe Portable " + "Document Format, Version 1.6", DocumentType.BOOK);
    doc.setPublisher(agent);
    doc.setDate("2004-11");
    ...
    _signature.add(new ExternalSignature(EXT, SignatureType.EXTENSION, SignatureUseType.OPTIONAL));
    _signature.add(
        new InternalSignature(
            PdfHeader.PDF_SIG_HEADER, SignatureType.MAGIC, SignatureUseType.MANDATORY, 0));
    ...
    doc.setIdentifier(
        new Identifier(
            "http://www.aiim.org/documents/standards/ISO_19005-1_(E).doc", IdentifierType.URL));
    _specification.add(doc);
   ...
   *
   * @param param The module parameter; under command-line Jhove, the -p parameter. If the parameter
   *     contains the indicated characters, then the specified information is omitted; otherwise, it
   *     is included. (This is the reverse of the behavior prior to beta 3.) These characters may be
   *     provided as separate parameters, or all in a single parameter.
   *     <ul>
   *       <li>a: annotations
   *       <li>f: fonts
   *       <li>o: outlines
  ...
  /**
   * Parses a file and stores descriptive information. A RandomAccessFile must be used to represent
   * the object.
   *
   * @param raf A PDF file
   * @param info A clean RepInfo object, which will be modified to hold the descriptive information
   */
  @Override
  public final void parse(RandomAccessFile raf, RepInfo info) throws IOException {
    ...
    _metadata =
        new Property(
            PROP_NAME_PDF_METADATA, PropertyType.PROPERTY, PropertyArity.LIST, metadataList);
    ...
    while (_startxref > 0) {
      ...
      if (_startxref == _prevxref) {
        info.setMessage(
            new ErrorMessage(
                MessageConstants.PDF_HUL_134, // PDF-HUL-134
                _parser.getOffset()));
    ...
    metadataList.add(
        new Property(PROP_NAME_OBJECTS, PropertyType.INTEGER, new Integer(_numObjects)));
    metadataList.add(
        new Property(PROP_NAME_FREE_OBJECTS, PropertyType.INTEGER, new Integer(_numFreeObjects)));
    metadataList.add(
        new Property(PROP_NAME_INC_UPDATES, PropertyType.INTEGER, new Integer(_numTrailers)));
    if (_docCatalogList != null) {
      metadataList.add(
          new Property(
              PROP_NAME_DOC_CATALOG, PropertyType.PROPERTY, PropertyArity.LIST, _docCatalogList));
    }
    if (_encryptList != null) {
      metadataList.add(
          new Property(
              PROP_NAME_ENCRYPTION, PropertyType.PROPERTY, PropertyArity.LIST, _encryptList));
    }
    if (_docInfoList != null) {
      metadataList.add(
          new Property(PROP_NAME_INFO, PropertyType.PROPERTY, PropertyArity.LIST, _docInfoList));
    }
    if (_idProperty != null) {
    ...
    if (_filtersList != null && !_filtersList.isEmpty()) {
      metadataList.add(
          new Property(PROP_NAME_FILTERS, PropertyType.PROPERTY, PropertyArity.LIST, _filtersList));
    }
    if (_imagesList != null && !_imagesList.isEmpty()) {
      metadataList.add(
          new Property(PROP_NAME_IMAGES, PropertyType.PROPERTY, PropertyArity.LIST, _imagesList));
    }
    if (_showFonts || _verbosity == Module.MAXIMUM_VERBOSITY) {
      try {
        ...
        info.setMessage(
            new ErrorMessage(MessageConstants.PDF_HUL_135, e.toString())); // PDF-HUL-135
      }
    }
    if (_nFonts > maxFonts) {
      info.setMessage(
          new InfoMessage(
              MessageConstants.PDF_HUL_136, // PDF-HUL-136
  ...
  /**
   * Returns true if the module hasn't detected any violations of PDF/A compliance. This must return
   * true, but is not sufficient by itself, to establish compliance. The <code>AProfile</code>
  ...
  /**
   * Initialize the module. This is called at the start of parse restore the module to its initial
   * state.
   */
  @Override
  ...
  protected boolean findLastTrailer(RepInfo info) throws IOException {
    ...
    if (_eof < 0L) {
      info.setWellFormed(false);
      info.setMessage(new ErrorMessage(MessageConstants.PDF_HUL_138, _raf.length())); // PDF-HUL-138
      return false;
    }
    ...
    if (_startxref < 0L) {
      ...
      info.setMessage(
          new ErrorMessage(
              MessageConstants.PDF_HUL_139, // PDF-HUL-139
              _parser.getOffset()));
      return false;
    }
    return true;
  ...
  protected boolean parseTrailer(RepInfo info, boolean prevOnly) throws IOException {
    ...
    try {
      ...
      if (xref instanceof Keyword) {
        _xrefIsStream = false;
        _parser.getNext(
            Numeric.class, // PDF-HUL-68
            MessageConstants.PDF_HUL_68); // first obj number

        _objCount =
            ((Numeric)
                    _parser.getNext(
                        Numeric.class, // PDF-HUL-69
                        MessageConstants.PDF_HUL_69))
                .getIntegerValue();
        ...
        if (!(pdfStreamObj instanceof PdfStream)) {
          throw new PdfInvalidException(MessageConstants.PDF_HUL_150, _parser.getOffset());
        }
        ...
        if (_docCatDictRef == null) {
          throw new PdfInvalidException(
              MessageConstants.PDF_HUL_70, // PDF-HUL-70
              _parser.getOffset());
        }
        /*
      ...
      if (trailer < 0L) {
        ...
        info.setMessage(
            new ErrorMessage(
                MessageConstants.PDF_HUL_71, // PDF-HUL-71
                _parser.getOffset()));
        return false;
      }
      ...
      if (obj != null) {
        if (obj instanceof PdfSimpleObject) {
          ...
          if (token instanceof Numeric) _prevxref = ((Numeric) token).getLongValue();
        }
        if (_prevxref < 0) {
          throw new PdfInvalidException(
              MessageConstants.PDF_HUL_72, // PDF-HUL-72
              _parser.getOffset());
        }
      }
      ...
      if (obj != null) {
        ...
        if (obj instanceof PdfSimpleObject) {
          token = ((PdfSimpleObject) obj).getToken();
          if (token instanceof Numeric) _numObjects = ((Numeric) token).getIntegerValue();
          _xref = new long[_numObjects];
        }
        if (_numObjects < 0) {
          throw new PdfInvalidException(
              MessageConstants.PDF_HUL_73, // PDF-HUL-73
              _parser.getOffset());
        }
        ...
        throw new PdfInvalidException(
            MessageConstants.PDF_HUL_74, // PDF-HUL-74
            _parser.getOffset());

      if (_docCatDictRef == null) {
        throw new PdfInvalidException(
            MessageConstants.PDF_HUL_75, // PDF-HUL-75
            _parser.getOffset());
      }
      _encryptDictRef =
          (PdfIndirectObj) _trailerDict.get(DICT_KEY_ENCRYPT); // This is at least v. 1.1
      ...
      if (infoObj != null && !(infoObj instanceof PdfIndirectObj)) {
        throw new PdfInvalidException(
            MessageConstants.PDF_HUL_76, // PDF-HUL-76
            _parser.getOffset());
      }
      ...
      if (obj != null) {
        if (obj instanceof PdfArray) {
          ...
          try {
            ...
            if (idVec.size() != 2) {
              throw new PdfInvalidException(MessageConstants.PDF_HUL_77); // PDF-HUL-77
            }
            ...
            throw new PdfInvalidException(MessageConstants.PDF_HUL_78); // PDF-HUL-78
          }
        } else {
          throw new PdfInvalidException(
              MessageConstants.PDF_HUL_79, _parser.getOffset()); // PDF-HUL-79
        }
      }
      ...
      else info.setMessage(new ErrorMessage(e.getMessage(), _parser.getOffset()));
      ...
      try {
        ...
        if (sObjNum < 0 || sObjNum >= xrefSize) {
          throw new PdfMalformedException(
              MessageConstants.PDF_HUL_80, // PDF-HUL-80
              _parser.getOffset());
        }
        ...
        try {
          while (xstream.readNextObject()) {
            ...
            if (xstream.isObjCompressed()) {
              ...
              _xref2[objNum] =
                  new int[] {xstream.getContentStreamObjNum(), xstream.getContentStreamIndex()};
            } else {
              if (_xref[objNum] == 0) {
          ...
          info.setMessage(
              new ErrorMessage(
                  MessageConstants.PDF_HUL_81, // PDF-HUL-81
                  _parser.getOffset()));
          return false;
        }
      } catch (PdfException e) {
      ...
      if (token instanceof Keyword) {
        while ((token = _parser.getNext()) != null) {
          ...
          for (int i = 0; i < _objCount; i++) {
            ...
            long offset =
                ((Numeric) _parser.getNext(Numeric.class, MessageConstants.PDF_HUL_82))
                    .getLongValue(); // PDF-HUL-82
            _parser.getNext(); // Generation number
            if (_parser.getWSString().length() > 1) {
              _pdfACompliant = false;
            }
            token = _parser.getNext(Keyword.class, MessageConstants.PDF_HUL_83); // PDF-HUL-83
            if (_parser.getWSString().length() > 1) {
              _pdfACompliant = false;
            }
  ...
  private boolean readDocCatalogDict(RepInfo info) throws IOException {
    ...
    try {
      ...
      p = new Property(DICT_KEY_PAGE_MODE, PropertyType.STRING, pModeText);
      _docCatalogList.add(p);
          ...
          if (ver != infoVer) {
            String mess =
                MessageFormat.format(
                    MessageConstants.PDF_HUL_87.getMessage(), infoVersString, versString);
            JhoveMessage message =
                JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_87.getId(), mess);
      ...
      try {
        ...
        if (namesDict != null) {
          PdfDictionary embeddedDict =
              (PdfDictionary) resolveIndirectObject(namesDict.get(DICT_KEY_EMBEDDED_FILES));
          if (embeddedDict != null) {
            _embeddedFiles = new NameTreeNode(this, null, embeddedDict);
          }

          PdfDictionary dDict =
              (PdfDictionary) resolveIndirectObject(namesDict.get(DICT_KEY_DESTS));
          if (dDict != null) {
  ...
  protected boolean readEncryptDict(RepInfo info) throws IOException {
    ...
    try {
        ...
        if (tok instanceof Numeric) {
          ...
          if (_je != null && _je.getShowRawFlag()) {
            ...
            try {
              p =
                  new Property(
                      PROP_NAME_ALGORITHM, PropertyType.STRING, PdfStrings.ALGORITHM[algValue]);
      ...
      if (length instanceof PdfSimpleObject) {
        ...
        if (_je != null) {
          p = new Property(PROP_NAME_KEY_LENGTH, PropertyType.INTEGER, new Integer(keyLen));
          _encryptList.add(p);
        }
      }
  ...
  protected boolean readDocInfoDict(RepInfo info) {
    ...
    try {
      ...
      info.setWellFormed(false);
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_94.getMessage(), e.getClass().getName());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_94.getId(), mess);
  ...
  protected boolean readDocumentTree(RepInfo info) {
    try {
      ...
      info.setMessage(
          new ErrorMessage(MessageConstants.PDF_HUL_96, _parser.getOffset())); // PDF-HUL-96
      info.setWellFormed(false);
      return false;
    } catch (Exception e) {
      // Catch any odd exceptions
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_98.getMessage(), e.getClass().getName());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_98.getId(), mess);
  ...
  protected boolean readPageLabelTree(RepInfo info) {
    ...
    try {
      ...
      info.setWellFormed(false);
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_99.getMessage(), e.getClass().getName());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_99.getId(), mess);
  ...
  protected boolean readXMPData(RepInfo info) {
    try {
      ...
      try {
        ...
        if (msg != null && msg.startsWith(ENCODING_PREFIX)) {
          ...
          try {
            ...
            _logger.log(
                Level.INFO, "Attempt to use explicit encoding to parse XMP metadata failed.", uee);
            throw new PdfInvalidException(MessageConstants.PDF_HUL_100); // PDF-HUL-100
          }
        }
      }
      ...
      info.setMessage(
          new ErrorMessage(
              MessageConstants.PDF_HUL_101, // PDF-HUL-101
              _parser.getOffset()));
      info.setValid(false);
      return false;
    }
  ...
  protected void findExternalStreams(RepInfo info) throws IOException {
    ...
    try {
      ...
      info.setWellFormed(false);
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_102.getMessage(), e.getClass().getName());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_102.getId(), mess);
  ...
  protected boolean findFilters(RepInfo info) throws IOException {
    ...
    // stop processing if there is no root for the document tree
    if (_docTreeRoot == null) return false;
    _docTreeRoot.startWalk();
    try {
      for (; ; ) {
        // Get all the page objects in the document sequentially
  ...
  /**
   * Finds the filters in a stream or array object which is the value of a stream's Filter key, and
   * put them in _filtersList if a duplicate isn't there already. If the name is "Crypt", appends a
   * colon and the name if available. Returns the filter string whether it's added or not, or null
  ...
  protected void findImages(RepInfo info) throws IOException {
    ...
    try {
            ...
            while (iter.hasNext()) {
              ...
              if (xobdict != null) {
                ...
                if (XOBJ_SUBTYPE_IMAGE.equals(subtype.getStringValue())) {
                  ...
                  Property prop =
                      new Property(
                          PROP_NAME_IMAGE, PropertyType.PROPERTY, PropertyArity.LIST, imgList);
                  NisoImageMetadata niso = new NisoImageMetadata();
                  imgList.add(
                      new Property(PROP_NAME_NISO_IMAGE_MD, PropertyType.NISOIMAGEMETADATA, niso));
                  ...
                  if (colorSpc != null) {
                    ...
                    if (colorName != null) {
                      ...
                      if (nisoSpace >= 0) {
                        ...
                        imgList.add(
                            new Property(PROP_NAME_COLOR_SPACE, PropertyType.STRING, colorName));
                      }
                    }
                  }
                  ...
                  if (intent != null) {
                    imgList.add(
                        new Property(
                            PROP_NAME_INTENT, PropertyType.STRING, intent.getStringValue()));
                  }
                  ...
                  if (imgmsk != null) {
                    ...
                    imgList.add(
                        new Property(
                            PROP_NAME_IMAGE_MASK, PropertyType.BOOLEAN, Boolean.valueOf(b)));
                  }
                  ...
                  if (dcd != null) {
                    ...
                    imgList.add(
                        new Property(
                            PROP_NAME_DECODE, PropertyType.INTEGER, PropertyArity.LIST, dcdlst));
                  }
                  ...
                  if (intrp != null) {
                    ...
                    imgList.add(
                        new Property(
                            PROP_NAME_INTERPOLATE, PropertyType.BOOLEAN, Boolean.valueOf(b)));
                  }

                  PdfSimpleObject nam = (PdfSimpleObject) xobdict.get(DICT_KEY_NAME);
                  if (nam != null) {
                    imgList.add(
                        new Property(PROP_NAME_NAME, PropertyType.STRING, nam.getStringValue()));
                  }

                  PdfSimpleObject id =
                      (PdfSimpleObject) resolveIndirectObject(xobdict.get(DICT_KEY_ID));
                  if (id != null) {
      ...
      info.setWellFormed(false);
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_103.getMessage(), e.getClass().getName());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_103.getId(), mess);
  ...
  protected void findFonts(RepInfo info) throws IOException {
    ...
    try {
        ...
        if (fonts != null) {
          ...
          while (fontIter.hasNext()) {
            ...
            if (!_skippedFontsReported && !_showFonts && _verbosity != Module.MAXIMUM_VERBOSITY) {
              info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_105)); // PDF-HUL-105
      ...
      _logger.log(Level.WARNING, MessageConstants.PDF_HUL_106.getMessage(), e);
      info.setWellFormed(false);
      info.setMessage(
          new ErrorMessage(
              MessageConstants.PDF_HUL_106, // PDF-HUL-106
              e.toString(),
              _parser.getOffset()));
      return;
    }
  ...
  /**
   * Add the font to the appropriate map, and return its subtype. If we've exceeded the maximum
   * number of fonts, then ignore it.
   */
  ...
  /**
   * If the argument is an indirect object reference, returns the object it resolves to, otherwise
   * returns the object itself. In particular, calling with null will return null.
   */
  ...
  /**
   * Returns an object of a given number. This may involve recursion into object streams, in which
   * case it calls itself.
   *
   * @param objIndex The object number to look up
   * @param recGuard The maximum permitted number of recursion levels; no particular value is
   *     required, but 30 or more should avoid false exceptions.
   */
  ...
  /**
   * Return a List of all the font maps. Together, these contain all the fonts and subfonts in the
   * document. Some of the maps may be null.
   */
  ...
  /**
   * Returns a NameTreeNode for the EmbeddedFiles entry of the Names dictionary. Returns null if
   * there isn't one.
   */
  ...
  /**
   * Add the various font lists as a fonts property. Note: only add the "Fonts" property if there
   * are, in fact, fonts defined.
   */
  protected void addFontsProperty(List<Property> metadataList) {
    ...
    if (fontTypesList.size() > 0) {
      metadataList.add(
          new Property(PROP_NAME_FONTS, PropertyType.PROPERTY, PropertyArity.LIST, fontTypesList));
    }
  }
    ...
    try {
      ...
      if (_pageLabelRoot != null) {
        if (!_pageLabelRoot.findNextKeyValue()) {
          throw new PdfMalformedException(MessageConstants.PDF_HUL_111); // PDF-HUL-111
        }
      ...
      if (_showPages || _verbosity == Module.MAXIMUM_VERBOSITY) {
        Property prop =
            new Property(PROP_NAME_PAGES, PropertyType.PROPERTY, PropertyArity.LIST, _pagesList);
        metadataList.add(prop);
      } else {
        if (!_skippedPagesReported) {
          info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_112)); // PDF-HUL-112
  ...
  protected Property buildPageProperty(PageObject page, int idx, RepInfo info) throws PdfException {
    ...
    try {
      ...
      if (plProp == null || nominalNum[0] != idx + 1) {
        ...
        pagePropList.add(
            new Property(PROP_NAME_SEQUENCE, PropertyType.INTEGER, new Integer(idx + 1)));
      }
    } catch (PdfException e) {
      throw e;
    } catch (Exception f) {
      ...
      if (annots != null) {
        ...
        for (int i = 0; i < contents.size(); i++) {
          ...
          if (annot instanceof PdfDictionary) {
            ...
            throw new PdfInvalidException(MessageConstants.PDF_HUL_114); // PDF-HUL-114
          }
        }
        if (!annotsList.isEmpty()) {
          if (_showAnnotations || _verbosity == Module.MAXIMUM_VERBOSITY) {
            Property annotProp =
                new Property(
                    PROP_NAME_ANNOTATIONS, PropertyType.PROPERTY, PropertyArity.LIST, annotsList);
            ...
            if (!_skippedAnnotationsReported) {
              info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_115)); // PDF-HUL-115
    ...
    try {
      ...
      PdfObject tempObj = page.get(DICT_KEY_ROTATE, true);
      ...
      if (tempObj != null && tempObj instanceof PdfSimpleObject) {
        ...
        rot = (PdfSimpleObject) ((PdfIndirectObj) tempObj).getObject();
      }
      if (rot != null && rot.getIntValue() != 0) {
        pagePropList.add(
            new Property(PROP_NAME_ROTATE, PropertyType.INTEGER, new Integer(rot.getIntValue())));
      }
      ...
      if (vp != null) {
        ...
        while (iter.hasNext()) {
          ...
          vpPropList.add(makeRectProperty((PdfArray) resolveIndirectObject(vpdbb), DICT_KEY_BBOX));
          ...
          vplist.add(
              new Property(
                  PROP_NAME_VIEWPORT, PropertyType.PROPERTY, PropertyArity.LIST, vpPropList));
        }
        pagePropList.add(
            new Property(PROP_NAME_VIEWPORTS, PropertyType.PROPERTY, PropertyArity.LIST, vplist));
      }
      // Thumbnail -- we just report if it's there. It's a
      // non-inheritable property
      PdfObject thumb = page.get(DICT_KEY_THUMB, false);
      if (thumb != null) {
        pagePropList.add(new Property(PROP_NAME_THUMB, PropertyType.BOOLEAN, Boolean.TRUE));
      }
      return new Property(PROP_NAME_PAGE, PropertyType.PROPERTY, PropertyArity.LIST, pagePropList);
    } catch (PdfException e) {
      throw e;
    } catch (Exception f) {
  ...
  protected Property buildPageLabelProperty(PageObject page, int pageIndex, int[] nomNumRef)
    ...
    try {
      ...
      PdfDictionary pageLabelDict =
          (PdfDictionary) resolveIndirectObject(_pageLabelRoot.getPrevValue());
      StringBuffer labelText = new StringBuffer();
      PdfSimpleObject prefixObj = (PdfSimpleObject) pageLabelDict.get(DICT_KEY_P);
      if (prefixObj != null) {
      ...
      return new Property(PROP_NAME_LABEL, PropertyType.STRING, labelText.toString());
    } catch (Exception e) {
  ...
  protected Property buildMeasureProperty(PdfDictionary meas) {
    ...
    if (itemObj instanceof PdfSimpleObject) {
      plist.add(
          new Property(
              PROP_NAME_RATIO, PropertyType.STRING, ((PdfSimpleObject) itemObj).getStringValue()));
    }
    ...
    if (itemObj instanceof PdfArray) {
      ...
      plist.add(new Property(PROP_NAME_DISTANCE, PropertyType.DOUBLE, PropertyArity.ARRAY, x));
    }
    itemObj = meas.get("A");
    if (itemObj instanceof PdfArray) {
      Vector<PdfObject> v = ((PdfArray) itemObj).getContent();
      ...
      plist.add(new Property(PROP_NAME_AREA, PropertyType.DOUBLE, PropertyArity.ARRAY, x));
    }
    return new Property(PROP_NAME_MEASURE, PropertyType.PROPERTY, PropertyArity.LIST, plist);
  }
  ...
  protected Property buildAnnotProperty(PdfDictionary annot, RepInfo info) throws PdfException {
    ...
    try {
      ...
      propList.add(makeRectProperty((PdfArray) resolveIndirectObject(itemObj), PROP_NAME_RECT));
      ...
      if (itemObj != null) {
        propList.add(
            new Property(
                DICT_KEY_NAME, PropertyType.STRING, ((PdfSimpleObject) itemObj).getStringValue()));
      }
      ...
      if (itemObj != null) {
        ...
        Property dateProp;
        dateProp = new Property(PROP_NAME_LAST_MOD, PropertyType.STRING, lastModLit.getValue());
      ...
      if (itemObj != null) {
        ...
        Property flagProp =
            (buildBitmaskProperty(
                flagValue, PROP_NAME_FLAGS, PdfStrings.ANNOTATIONFLAGS, PROP_VAL_NO_FLAGS_SET));
        if (flagProp != null) {
      ...
      if (itemObj != null) {
        propList.add(new Property(PROP_NAME_APP_DICT, PropertyType.BOOLEAN, Boolean.TRUE));
      }
      ...
      if (itemObj != null) {
        ...
        PdfSimpleObject actionSubtype = (PdfSimpleObject) ((PdfDictionary) itemObj).get("S");
        if (actionSubtype == null) {
          throw new PdfMalformedException(MessageConstants.PDF_HUL_120); // PDF-HUL-120
        }
        if (ACTION_VAL_GOTO.equals(actionSubtype.getStringValue())) {
          PdfObject destObj = ((PdfDictionary) itemObj).get("D");
          if (destObj != null) {
            addDestination(destObj, PROP_NAME_ACTION_DEST, propList, info);
          }
        }
      }
      ...
      if (itemObj instanceof PdfSimpleObject) {
        ...
        propList.add(new Property(PROP_NAME_REPLY_TYPE, PropertyType.STRING, type));
      }
      ...
      if (itemObj instanceof PdfSimpleObject) {
        String type = ((PdfSimpleObject) itemObj).getStringValue();
        propList.add(new Property(PROP_NAME_INTENT, PropertyType.STRING, type));
      }
      ...
      if (itemObj instanceof PdfArray) {
        ...
        propList.add(
            new Property(PROP_NAME_CALLOUT_LINE, PropertyType.DOUBLE, PropertyArity.LIST, clList));
      }

      return new Property(
          PROP_NAME_ANNOTATION, PropertyType.PROPERTY, PropertyArity.LIST, propList);
  ...
  protected void addDestination(
      PdfObject itemObj, String propName, List<Property> propList, RepInfo info) {
    try {
      Destination dest = new Destination(itemObj, this, false);
      if (dest.isIndirect()) {
        // Encryption messes up name trees
        if (!_encrypted) {
          int pageObjNum = resolveIndirectDest(dest.getIndirectDest(), info);
          if (pageObjNum == -1) {
            // The scope of the reference is outside this
            // file, so we just report it as such.
            propList.add(new Property(propName, PropertyType.STRING, PROP_VAL_EXTERNAL));
          } else {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_122.getId(), msg);
  ...
  protected Property buildFontProperty(String name, Map map, int fontType) {
    ...
    while (fontIter.hasNext()) {
      ...
      Property fProp =
          new Property(PROP_NAME_FONT, PropertyType.PROPERTY, PropertyArity.LIST, fontPropList);
      fontList.add(fProp);
    }
  ...
  protected List<Property> oneFontPropList(PdfDictionary dict, int fontType) {
    ...
    if (fontType != F_TYPE3) {
      ...
      if (baseFontObj != null) {
        ...
        prop = new Property(PROP_NAME_BASE_FONT, PropertyType.STRING, baseStr);
        fontPropList.add(prop);
      }
    }
    ...
    if (fontType == F_TYPE1 || fontType == F_TT || fontType == F_MM1) {
      if (isFontSubset(baseStr)) {
        prop = new Property(PROP_NAME_FONT_SUBSET, PropertyType.BOOLEAN, Boolean.TRUE);
        fontPropList.add(prop);
      }
    }
    ...
    if (fontType == F_TYPE1 || fontType == F_TT || fontType == F_MM1 || fontType == F_TYPE3) {
      ...
      try {
        ...
        prop = new Property(PROP_NAME_FIRST_CHAR, PropertyType.INTEGER, new Integer(firstChar));
        fontPropList.add(prop);
      } catch (Exception e) {
      }
      ...
      try {
        ...
        prop = new Property(PROP_NAME_LAST_CHAR, PropertyType.INTEGER, new Integer(lastChar));
        fontPropList.add(prop);
      } catch (Exception e) {
      }
    }
    ...
    if (fontType == F_TYPE3) {
      ...
      try {
        if (bboxObj instanceof PdfArray) {
          fontPropList.add(makeRectProperty((PdfArray) bboxObj, PROP_VAL_FONT_BBOX));
        }
      } catch (Exception e) {
      }
    ...
    if (fontType == F_TYPE3) {
      ...
      if (rsrc != null) {
        prop = new Property(PROP_NAME_RESOURCES, PropertyType.BOOLEAN, Boolean.TRUE);
        fontPropList.add(prop);
      }
    }
    ...
    if (fontType == F_TYPE0
      ...
      if (toUniObj != null) {
        prop = new Property(PROP_NAME_TO_UNICODE, PropertyType.BOOLEAN, Boolean.TRUE);
        fontPropList.add(prop);
      }
    }
  ...
  protected Property buildCMapDictProperty(PdfStream encoding) {
    ...
    Property prop =
        new Property(PROP_NAME_CMAP_DICT, PropertyType.PROPERTY, PropertyArity.LIST, propList);
    Property subprop;
    ...
    if (!cidList.isEmpty()) {
      subprop =
          new Property(PROP_NAME_CID_INFOS, PropertyType.PROPERTY, PropertyArity.LIST, cidList);
  ...
  protected Property buildCIDInfoProperty(PdfDictionary dict) {
    ...
    Property prop =
        new Property(PROP_NAME_CID_INFO, PropertyType.PROPERTY, PropertyArity.LIST, propList);
    Property subprop;
    ...
    if (reg instanceof PdfSimpleObject) {
      try {
        ...
        subprop =
            new Property(PROP_NAME_REGISTRY, PropertyType.STRING, _encrypted ? ENCRYPTED : regText);
        propList.add(subprop);
      } catch (Exception e) {
      }
    }
    ...
    if (reg instanceof PdfSimpleObject) {
      try {
        ...
        subprop = new Property(PROP_NAME_REGISTRY, PropertyType.STRING, ordText);
        propList.add(subprop);
      } catch (Exception e) {
      }
    }
    ...
    if (supp instanceof PdfSimpleObject) {
      try {
        ...
        subprop = new Property(PROP_NAME_SUPPLEMENT, PropertyType.INTEGER, new Integer(suppvalue));
        propList.add(subprop);
      } catch (Exception e) {
      }
    }
  ...
  protected Property buildEncodingDictProperty(PdfDictionary encodingDict) {
    ...
    Property prop =
        new Property(
            PROP_NAME_ENCODING_DICTIONARY, PropertyType.PROPERTY, PropertyArity.LIST, propList);
    ...
    if (baseEnc instanceof PdfSimpleObject) {
      ...
      if (baseEncString != null) {
        Property baseEncProp =
            new Property(PROP_NAME_BASE_ENCODING, PropertyType.STRING, baseEncString);
    ...
    Property diffsProp =
        new Property(PROP_NAME_DIFFERENCES, PropertyType.BOOLEAN, Boolean.valueOf(diffs != null));
  ...
  protected Property buildFontDescriptorProperty(PdfDictionary encodingDict) {
    ...
    Property prop =
        new Property(PROP_NAME_FONT_DESC, PropertyType.PROPERTY, PropertyArity.LIST, propList);
    Property subprop;
    try {
      PdfSimpleObject fName = (PdfSimpleObject) encodingDict.get(DICT_KEY_FONT_NAME);
      String fNameStr = fName.getStringValue();
      subprop = new Property(PROP_NAME_FONT_NAME, PropertyType.STRING, fNameStr);
      propList.add(subprop);
    } catch (Exception e) {
    }

    try {
      PdfSimpleObject flags = (PdfSimpleObject) encodingDict.get(DICT_KEY_FLAGS);
      int flagValue = flags.getIntValue();
      subprop =
          buildBitmaskProperty(
              flagValue, PROP_NAME_FLAGS, PdfStrings.FONTDESCFLAGS, PROP_VAL_NO_FLAGS_SET);
      if (subprop != null) {
        propList.add(subprop);
      }
    } catch (Exception e) {
    ...
    try {
      ...
      if (bbox != null) {
        ...
        subprop =
            new Property(PROP_NAME_FONT_BBOX, PropertyType.INTEGER, PropertyArity.ARRAY, ibbox);
        propList.add(subprop);
      }
    } catch (Exception e) {
    }
    ...
    if (fontFile != null) {
      ...
      subprop = new Property(PROP_NAME_FONT_FILE, PropertyType.BOOLEAN, Boolean.TRUE);
      propList.add(subprop);
    }
    fontFile = encodingDict.get(DICT_KEY_FONT_FILE_2);
    if (fontFile != null) {
      subprop = new Property(PROP_NAME_FONT_FILE_2, PropertyType.BOOLEAN, Boolean.TRUE);
      propList.add(subprop);
    }
    fontFile = encodingDict.get(DICT_KEY_FONT_FILE_3);
    if (fontFile != null) {
      subprop = new Property(PROP_NAME_FONT_FILE_3, PropertyType.BOOLEAN, Boolean.TRUE);
      propList.add(subprop);
    }
    return prop;
  }
  ...
  protected Property buildViewPrefProperty(PdfDictionary prefDict) {
    ...
    Property prop =
        new Property(DICT_KEY_VIEWER_PREFS, PropertyType.PROPERTY, PropertyArity.LIST, propList);
  ...
  protected Property buildOutlinesProperty(PdfDictionary dict, RepInfo info) throws PdfException {
    ...
    Property prop =
        new Property(PROP_NAME_OUTLINES, PropertyType.PROPERTY, PropertyArity.LIST, itemList);
    try {
      ...
      while (item != null) {
        ...
        if (item.getObjNumber() == onum.intValue()) {
          if (!_recursionWarned) {
            info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_123)); // PDF-HUL-123
  ...
  protected Property buildOutlineItemProperty(PdfDictionary dict, RepInfo info)
    ...
    try {
      Property prop =
          new Property(PROP_NAME_ITEM, PropertyType.PROPERTY, PropertyArity.LIST, itemList);
      PdfSimpleObject title = (PdfSimpleObject) resolveIndirectObject(dict.get(DICT_KEY_TITLE));
      if (title == null) {
      ...
      if (destObj != null) {
        ...
        if (dest.isIndirect()) {
          itemList.add(
              new Property(
                  PROP_NAME_DESTINATION,
                  PropertyType.STRING,
                  dest.getIndirectDest().getStringValue()));
        } else {
          int pageObjNum = dest.getPageDestObjNumber();
          Integer destPg = _pageSeqMap.get(new Integer(pageObjNum));
          if (destPg != null) {
            itemList.add(new Property(PROP_NAME_DESTINATION, PropertyType.INTEGER, destPg));
          }
        }
      }

      PdfDictionary child = (PdfDictionary) resolveIndirectObject(dict.get(DICT_KEY_FIRST));
      if (child != null) {
        List<Property> childList = new LinkedList<Property>();
        Property childProp =
            new Property(PROP_NAME_CHILDREN, PropertyType.PROPERTY, PropertyArity.LIST, childList);
        ...
        while (child != null) {
          ...
          if (_visitedOutlineNodes.contains(onum)) {
            ...
            if (!_recursionWarned) {
              // Warn of recursion
              info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_128)); // PDF-HUL-128
              _recursionWarned = true;
            }
          } else {
          ...
          child = (PdfDictionary) resolveIndirectObject(child.get(DICT_KEY_NEXT));
          if (child == null) {
            break;
          }
          // Check if this object is its own sibling. (It really does
          ...
          if (child.getObjNumber() == onum.intValue()) {
            if (!_recursionWarned) {
              info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_129)); // PDF-HUL-129
              _recursionWarned = true;
            }
            break;
          }
          if (++listCount > 2000) break; // safety check
        }
    ...
    if (_outlineDict != null) {
      try {
        ...
        if (_showOutlines || _verbosity == Module.MAXIMUM_VERBOSITY) {
          ...
          info.setMessage(new InfoMessage(MessageConstants.PDF_HUL_132)); // PDF-HUL-132
  ...
  protected int resolveIndirectDest(PdfSimpleObject key, RepInfo info) throws PdfException {
    ...
    _logger.finest("Looking for indirectly referenced Dest: " + key.getStringValue());
    ...
    if (destObj == null) {
      ...
      String mess =
          MessageFormat.format(MessageConstants.PDF_HUL_149.getMessage(), key.getStringValue());
      JhoveMessage message =
          JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_149.getId(), mess);
  ...
  /**
   * Add a string property, based on a dictionary entry with a string value, to a specified List.
   */
  protected void addStringProperty(
      PdfDictionary dict, List<Property> propList, String key, String propName) {
  ...
  /** Add a date property, based on a dictionary entry with a string value, to a specified List. */
  protected void addDateProperty(
      PdfDictionary dict, List<Property> propList, String key, String propName)
      throws PdfException {
    if (_encrypted) {
  ...
  protected Property buildBitmaskProperty(
      int val, String name, String[] valueNames, String defaultStr) {
    if (_je != null && _je.getShowRawFlag()) {
  ...
  private static boolean checkTypeKey(
      final PdfDictionary dict,
      final RepInfo info,
      final String expctVal,
    ...
    if (typeObj != null && typeObj instanceof PdfSimpleObject) {
      ...
      // the wrong type
      JhoveMessage message = (typeObj == null) ? typeNotFoundMess : typeNotSimpleMess;
  ...
  private PdfObject getObjectFromStream(final int objIndex, final int recGuard)
    ...
    try {
      ...
      if (objStreamIndex == _cachedStreamIndex) {
        ...
        streamObj = resolveIndirectObject(getObject(objStreamIndex, recGuard - 1));
        if (streamObj instanceof PdfStream) {
          ...
          if (ostrm.isValid()) {
            ...
            throw new PdfMalformedException(MessageConstants.PDF_HUL_108); // PDF-HUL-108
          }
        }
      }
