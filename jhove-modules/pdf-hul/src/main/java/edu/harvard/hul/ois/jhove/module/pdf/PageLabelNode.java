...
public class PageLabelNode {
  ...
  /**
   * Build the subtree of descendants of this node, using the Kids entry in the dictionary.Leaf
   * nodes are recognized by not having a Kids entry.
   *
    ...
    try {
      ...
      if (kids != null) {
        ...
        for (int i = 0; i < kidsVec.size(); i++) {
          PdfDictionary kid = (PdfDictionary) _module.resolveIndirectObject(kidsVec.elementAt(i));
          PageLabelNode nodeObj = new PageLabelNode(_module, this, kid);
  ...
  /**
   * Get the next leaf object which is under this node. This function is designed such that calling
   * startWalk() and then repeatedly calling nextLeafObject() will return all the leaf objects in
   * the tree under this node, and finally will return null when there are no more. A leaf object is
   * one which has no Kids; it is required to have a Nums entry.
   *
  ...
  /**
   * Obtain the next key-value pair from the tree.This returns true if a pair is available, false if
   * not. After this is called, getCurrentKey and getCurrentValue may be called to retrieve the key
   * and value thus found. Each time this is called, currentKey and currentValue get copied into
   * prevKey and prevValue.
   *
   * @return boolean: true if a next key-value pair is available, false if no
   * @throws edu.harvard.hul.ois.jhove.module.pdf.PdfException
   */
  public boolean findNextKeyValue() throws PdfException {
    try {
      if (_currentLeaf == null || _currentNumsIndex >= _currentNumsLength) {
        ...
        _currentNumsIndex = 0;
        PdfArray pairArray =
            (PdfArray) _module.resolveIndirectObject(_currentLeaf._dict.get("Nums"));
        if (pairArray == null) {
      ...
      PdfSimpleObject keyObj = (PdfSimpleObject) _currentNumsVec.elementAt(_currentNumsIndex);
  ...
  /**
   * A convenience method to turn integers into "letter" page numbers as defined for PDF. The first
   * 26 pages are A-Z, the next 26 AA-ZZ, etc.
   *
