...
/**
 * Abstract base class for PDF profilers of the PDF/X family. See ISO Standard 15930-1, "Complete
...
public abstract class XProfileBase extends PdfProfile {
  ...
  public static int PDFX1 = 1, PDFX1A = 2, PDFX2 = 3, PDFX3 = 4;
  ...
  /**
   * Checks if the entries which are required in the document information dictionary by PDF-X/1 and
   * X/3 are there. These entries are optional under the PDF specification, so they must be checked
   * here.
   */
  ...
  /**
   * Checks if the OutputIntents of this document conform to profile requirements. There must be
   * exactly one entry in the dictionary which has a subtype of GTS_PDFX.
   */
  protected boolean outputIntentsOK(PdfArray intents) {
    ...
    try {
      for (int i = 0; i < intVec.size(); i++) {
        PdfDictionary intent = (PdfDictionary) _module.resolveIndirectObject(intVec.elementAt(i));
        PdfSimpleObject sval = (PdfSimpleObject) _module.resolveIndirectObject(intent.get("S"));
        if (sval != null) {
          ...
          if ("GTS_PDFX".equals(subtype)) {
            ...
            PdfSimpleObject outcond =
                (PdfSimpleObject)
                    _module.resolveIndirectObject(intent.get("OutputConditionIdentifier"));
            if (outcond == null) return false;

            // X1 and 1a only: There must be either a RegistryName entry
            // or a DestOutputProfile
            // entry.  Having them both is OK.
            if (_xType == PDFX1) {
              PdfSimpleObject regName =
                  (PdfSimpleObject) _module.resolveIndirectObject(intent.get("RegistryName"));
              PdfStream dop =
  ...
  /**
   * Checks if the ExtGState resource meets profile requirements. It may not have TR, TR2, or HTP
   * entries.
   */
  ...
  /**
   * Checks a single XObject for xObjectsOK. Calls imageObjectOK and formObjectOK for
  ...
  /**
   * Checks the conformance of a form XObject. Does nothing; must be overridden if there are
  ...
  /**
   * Checks all the page objects for bounding boxes. If requireMediaBox is true, each page has to
   * include or inherit a MediaBox. Each page must include one but not both of a TrimBox and an
   * ArtBox.
   */
  ...
  /**
   * Checks ViewerPreferences dictionary against MediaBox and BleedBox. In PDF-X1/a and X2, if a
   * BleedBox is present and if the ViewerPreferences dictionary contains the ViewClip, PrintArea or
   * PrintClip keys, each of those keys present shall have the value MediaBox or BleedBox. This must
   * be called after bboxOK has checked if any BleedBoxes are found.
   */
    ...
    try {
      ...
      for (int i = 0; i < 3; i++) {
        if (areas[i] != null) {
          ...
          if (!"MediaBox".equals(s) && !"BleedBox".equals(s)) {
            return false;
          }
        }
      }
