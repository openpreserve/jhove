...
/**
 * Class for Tokens which represent PDF numbers. Numeric values are stored as <code>double</code> if
 * they have a real value, or as int if they have an integer value. The implementation notes state
 * that the maximum value of an integer on a 32-bit machine is 2 ^ 31 - 1. However, they also say
 * that byte offsets have a limit of 10 decimal digits, which is a larger value. So we store integer
 * values as long.
 */
