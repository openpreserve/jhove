...
/**
 * Class to encapsulate a stream token. The content of the stream is not saved, only its length and
 * starting offset.
 */
public class Stream extends Token {
  ...
  /**
   * Returns the length of the stream. This is 0, unless the Stream's setLength method has been
   * called.
   */
  ...
  /**
   * Sets the length field. This should be the length of the stream proper (not counting its
   * dictionary) before filtering, in other words, the number of bytes stored in the file.
   */
  ...
  /**
   * Returns the current offset in the stream. This is -1, unless the Stream's setOffset method has
   * been called.
   */
   ...
   * a PdfException. This supports the abbreviated filter names in Appendix H of the PDF spec.
   */
  ...
  /**
   * Reads an ASCII string, which may be preceded by white space. Will eat the first white space
   * character after the ASCII string.
   */
  public int readAsciiInt() throws IOException, PdfException {
    ...
    for (; ; ) {
      ...
      if (Character.isDigit(c)) {
        ...
        throw new PdfMalformedException(MessageConstants.PDF_HUL_46); // PDF-HUL-46
      }
    }
    return val;
  }
  ...
  /**
   * Advances to a specified offset in the stream. The offset is defined as the number of
   * decompressed bytes which precede the position in the stream. Returns <code>true</code> if the
   * advance is successful, <code>false</code> if the point has already been passed or some other
   * failure occurs.
   */
