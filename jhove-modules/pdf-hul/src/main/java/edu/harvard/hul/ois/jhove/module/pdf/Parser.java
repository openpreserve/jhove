...
/**
 * The Parser class implements some limited syntactic analysis for PDF. It isn't by any means
 * intended to be a full parser. Its main job is to track nesting of syntactic elements such as
 * dictionary and array beginnings and ends.
 */
public class Parser {
  ...
  /**
   * Clears the state of the parser so that it can start reading at a different place in the file.
   * Clears the stack and the dictionary and array depth counters.
   */
  public void reset() {
    _dictDepth = 0;
    _arrayDepth = 0;
  ...
  /**
   * Clears the state of the parser so that it can start reading at a different place in the file
   * and ignore any nesting errors. Sets the stack and the dictionary and array depth counters to a
   * large number so that nesting exceptions won't be thrown.
   */
  ...
  /**
   * Gets a token. Uses Tokenizer.getNext, and keeps track of the depth of dictionary and array
   * nesting.
   */
  ...
  /**
   * A class-sensitive version of <code>getNext</code>. The token which is obtained must be of the
   * specified class (or a subclass thereof), or a <code>PdfInvalidException</code> with message
   * <code>errMsg</code> will be thrown.
   */
  public Token getNext(Class<?> clas, JhoveMessage errMsg) throws IOException, PdfException {
    Token tok = getNext();
  ...
  /**
   * Returns false if either the parser or the tokenizer has detected non-compliance with PDF/A
   * restrictions. A value of <code>true</code> is no guarantee that the file is compliant.
   */
  ...
  /**
   * Sets the value of the pdfACompliant flag. This may be used to clear previous detection of
   * noncompliance. If the parameter has a value of <code>true</code>, the tokenizer's pdfACompliant
  ...
  /**
   * Reads an object definition, from wherever we are in the stream to the completion of one full
   * object after the obj keyword.
   */
  public PdfObject readObjectDef() throws IOException, PdfException {
    Numeric objNumTok = (Numeric) getNext(Numeric.class, MessageConstants.PDF_HUL_35); // PDF-HUL-35
   ...
   *
   * <ul>
   *   <li>It doesn't retain the contents of streams
   *   <li>It doesn't recognize a stream when it's pointing at the stream's dictionary; it will just
   *       read the dictionary
   * </ul>
   *
   * Functions which it uses may call it recursively to build up structures. If it encounters a
   * token inappropriate for an object start, it throws a <code>PdfException</code> on which <code>
   * getToken</code> may be called to retrieve that token.
   */
  public PdfObject readObject(boolean allowPseudo) throws IOException, PdfException {
    ...
    // For the end of a dictionary or array, retu
      ...
      throw new PdfMalformedException(MessageConstants.PDF_HUL_39, getOffset(), tok); // PDF-HUL-39
    }
  }

  /**
   * Reads a PDF array. When this is called we have already read the ArrayStart token and arrayDepth
  ...
  public PdfArray readArray() throws IOException, PdfException {
    ...
    for (; ; ) {
      ...
      if (!(obj instanceof PdfPseudoObject)) {
        ...
        throw new PdfMalformedException(MessageConstants.PDF_HUL_40, getOffset()); // PDF-HUL-40
      }
    }
  }

  /**
   * Reads a PDF dictionary. When this is called, we have already read the DictionaryStart token,
   * and dictDepth has been incremented to reflect this. Only for use in this special case, where
   * we're picking up a dictionary in midstream.
   */
  public PdfDictionary readDictionary() throws IOException, PdfException {
    ...
    for (; ; ) {
      ...
      if (obj instanceof PdfSimpleObject && ((PdfSimpleObject) obj).getToken() instanceof Comment) {
        continue;
      }
      if (!(obj instanceof PdfPseudoObject)) {
        vec.add(obj);
          ...
          if ((vecSize % 2) != 0) {
            String mess =
                MessageFormat.format(
                    MessageConstants.PDF_HUL_41.getMessage(), Integer.valueOf(vecSize));
            JhoveMessage message =
                JhoveMessages.getMessageInstance(MessageConstants.PDF_HUL_41.getId(), mess);
          ...
          for (int i = 0; i < vecSize; i += 2) {
            try {
              ...
              throw new PdfMalformedException(
                  MessageConstants.PDF_HUL_42, getOffset()); // PDF-HUL-42
            }
          }
        ...
        throw new PdfMalformedException(MessageConstants.PDF_HUL_43, getOffset()); // PDF-HUL-43
      }
    }
  }
      ...
      if (obj instanceof PdfSimpleObject) {
        ...
        if (tok instanceof Keyword && "R".equals(((Keyword) tok).getValue())) {
          ...
          try {
            ...
            throw new PdfMalformedException(MessageConstants.PDF_HUL_44); // PDF-HUL-44
          }
        }
      }
    }
  ...
  /**
   * If <code>true</code>, do not attempt to parse non-whitespace delimited tokens, e.g., literal
