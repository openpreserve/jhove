 ...
 *
 * <p>We need to make two different kinds of distinctions: between destinations that make an
 * indirect and a direct reference to a page; and between destinations that have been reached by a
 * direct and an indirect reference. The PDF spec allows only one level of indirection, so each of
 * these forms has options not available to the other.
 *
 * <p>We call a destination which has been reached directly an unnamed destination, and one which
 * has been reached indirectly a named destination. We call a destination which has an indirect
 * target an indirect destination, and one which has a page object as a target a direct destination.
 * Applying the PDF documentation, we find that a destination can never be both named and indirect.
 * In other words, there are really two cases, involving three kinds of destinations:
 *
 * <UL>
 *   <LI>An unnamed, direct destination, which refers to the page object.
 *   <LI>An unnamed, indirect destination, which refers to a named, direct destination, which refers
 *       to the page object.
 * </UL>
 */
public final class Destination {
  ...
  /**
   * Constructor. If this is a named destination, the destObj may be a PdfArray or a PdfDictionary;
   * if this is not a named destination, the destObj may be a PdfSimpleObject (encapsulating a
   * Literal or Name) or a PdfDictionary.
   *
   * @param destObj The destination object
   * @param module The invoking PdfModule
   * @param named Flag indicating whether this object came from a named destination.
   */
    ...
    try {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              String.format(
                  MessageConstants.PDF_HUL_3.getMessage(), // PDF-HUL-3
  ...
  /**
   * Returns the string naming the indirect destination. Returns null if the destination is not
   * indirect.
   */
  ...
  /**
   * Returns the page object dictionary if the destination is direct. Returns null if the
   * destination is not direct.
   */
  ...
  /**
   * Returns the object number of the page object dictionary if the destination is direct. Throws a
