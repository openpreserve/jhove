...
/**
 * Exception subclass used internally by the PDF module. A PdfInvalidException is thrown when a
 * condition indicates that the document is invalid but not necessarily ill-formed.
 */
public final class PdfInvalidException extends PdfException {
  ...
  public PdfInvalidException(final JhoveMessage message, final long offset, final Token token) {
    super(message, offset, token);
  }

  /**
   * Performs the appropriate disparagement act on a RepInfo object. For a PdfInvalidException, this
   * is to call <code>setValid (false)</code>.
   */
  @Override
