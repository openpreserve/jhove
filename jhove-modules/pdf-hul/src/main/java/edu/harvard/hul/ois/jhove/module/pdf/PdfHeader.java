...
/**
 * Simple class that is the a prototype of a proper header parser class. The aim was to introduce a
 * simple version check for the PDF/A minor version number, see {@link PdfHeader#isVersionValid()},
 * while not changing anything else through over ambition.
 *
...
public final class PdfHeader {
  ...
  /** */
  private PdfHeader(final String versionString, final boolean isPdfaCompliant) {
  ...
  /**
   * Performs a very simple version number validity check. Given version number is a String of form
   * 1.x, x is the minor version number. This method parses the minor version number from the
   * version String and tests whether it is less than or equal to {@link
   * PdfHeader#MAX_VALID_MAJOR_VERSION}.
   *
   * @return true if an integer minor version number can be parsed from the version string AND it is
   *     less than or equal to {@link PdfHeader#MAX_VALID_MAJOR_VERSION}. Otherwise false.
   */
   ...
   *
   * @param versionString the version number from the PDF Header, should be of form <code>1.x</code>
   *     where x should be of the range 0-7.
   * @param isPdfaCompliant boolean flag indicating if the PDF/A is compliant or non compliant with
   *     JHOVE's PDF/A profile.
   * @return a {@link PdfHeader} instance initialised using <code>versionString</code> and <code>
   *     isPdfaCompliant</code>.
   * @throws NullPointerException when parameter <code>versionString</code> is null.
   */
  static PdfHeader fromValues(final String versionString, final boolean isPdfaCompliant) {
  ...
  /**
   * Factory method for {@link PdfHeader} that parses a new instance using the supplied {@link
   * Parser} instance.
   *
   * @param parser the {@link Parser} instance that will be used to parse header details
   * @return a new {@link PdfHeader} instance derived using the supplied {@link Parser} or <code>
   *     null</code> when no header could be found and parsed.
   */
