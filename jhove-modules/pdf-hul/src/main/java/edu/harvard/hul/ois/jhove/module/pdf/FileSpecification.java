...
/**
 * A class which encapsulates a file specification in PDF. A file specification may be given as
 * either a string or a dictionary. The specification is converted to a string according to the
 * following rules: If a PDF string object is the file specifier, that string is used, without
 * attempting to convert file separators to the local file system. If a PDF dictionary is used, one
 * of the following is used, in decreasing order of preference:
 *
 * <UL>
...
public enum FileSpecification {
   ...
   * Constructor.
   *
   * @param obj A PdfDictionary with the file specification under the key "F", "Unix", "DOS", or
   *     "Mac"; or a PdfSimpleObject whose string value is the file specification. If <code>obj
   *     </code> is a dictionary and more than one key is specified, then the first of the keys F,
   *     Unix, DOS, and Mac to be found is used.
   */
