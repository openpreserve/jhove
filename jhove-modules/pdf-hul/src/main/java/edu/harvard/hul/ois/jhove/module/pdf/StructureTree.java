...
/**
 * Class for PDF document structure tree. See section 9.6, "Logical Structure," of PDF Reference,
...
public class StructureTree {
  ...
  /**
   * Constructor. If there is a document structure tree, this fills in the appropriate information.
   * If there isn't, it does nothing. Call {@code isPresent} to determine whether there is a
   * document structure tree. A {@code PdfInvalidException} may be thrown if there is a structure
   * tree but it is invalid.
   *
  ...
  /**
   * Constructor with transient flag. Calling this can save a lot of memory if the tree is being
   * validated as it's built (which happens to be the only case, but I don't want to throw out the
   * more general code).
   */
  public StructureTree(PdfModule module, boolean tranzhent) {
    ...
    try {
      ...
      _rootDict = null;
      try {
        _rootDict = (PdfDictionary) _module.resolveIndirectObject(docCatDict.get("StructTreeRoot"));
      } catch (IOException e) {
      }
      if (_rootDict == null) {
        _present = false;
  ...
  /** Returns <code>true</code> if and only if the document structure exists. */
  public boolean isPresent() {
    return _present;
  }

  /** Returns <code>true</code> if and only if no errors were detected. */
  ...
  /**
   * Dereference a name in the role map. If there is no role map, or if the parameter is not mapped
   * by the role map, the original parameter will be returned. The string will be looked up through
   * multiple levels in the role map. The maximum number of levels is limited to 50, in case of
   * circular mappings. The value returned will be null if the role map contains invalid data or the
  ...
  public String dereferenceStructType(String st) {
    ...
    for (int i = 0; i < 50; i++) {
      try {
        PdfSimpleObject mapped = (PdfSimpleObject) _roleMap.get(st);
        if (mapped == null) {
          return st;
        }
  ...
  private void validateRoot() throws PdfException {
    try {
      PdfSimpleObject typ = (PdfSimpleObject) _rootDict.get("Type");
  ...
  /**
   * Replaces a string with a string to which the role map maps it. This may involve multiple levels
   * of lookup.
   */
  ...
  private void checkChildren() throws PdfException {
    ...
    PdfObject kids = null;
    try {
      kids = _module.resolveIndirectObject(_rootDict.get("K"));
    } catch (IOException e) {
    }
    if (kids == null) {
      return;
    ...
    if (kids instanceof PdfDictionary) {
      ...
      for (int i = 0; i < kidsVec.size(); i++) {
        PdfObject kid;
        try {
          kid = _module.resolveIndirectObject(kidsVec.elementAt(i));
        } catch (IOException e) {
  ...
  private void checkRoleMap() throws PdfException {
    try {
      _roleMap = (PdfDictionary) _module.resolveIndirectObject(_rootDict.get("RoleMap"));
    } catch (Exception e) {
