...
public class Literal
    ...
    /**
     * Process the incoming characters into a string literal.This is used for
     * literals delimited
     ...
     */
    public Date parseDate() throws PdfInvalidException {
    ...
    static final Date parseDate(String dateString) throws PdfInvalidException {
                    ...
                    case 9: // expecting time zone minute -- in single quotes
                        ...
                        if (timezonechar == '+' || timezonechar == '-') {
                            if (str.charAt(charidx) == '\'') {
                                timezoneminute = Integer.parseInt(str.substring(charidx,
                                        charidx + 2));
                            }
                            if (timezonechar == '-') {
                            ...
                            if ((str.length() > 22) && (!str.substring(charidx + 2, charidx + 3).equals("'"))) {
                                throw new PdfInvalidException(MessageConstants.PDF_HUL_133, 0); // PDF-HUL-133
                            }
                        }
                        break wloop;
        ...
        return createTzDate(year, month, day, hour, minute, second, timezonehour, timezoneminute, timezonechar);
    }

    static Date createTzDate(final int year, final int month, final int day, final int hour, final int minute,
            final int second, final int timezonehour, final int timezoneminute, final char timezonechar) {
        Calendar cal;
