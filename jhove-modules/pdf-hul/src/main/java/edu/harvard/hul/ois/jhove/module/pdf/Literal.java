...
/**
 * Class for Tokens which represent PDF strings. The class maintains a field for determining whether
 * the string is encoded as PDF encoding or UTF-16. This is determined in the course of analyzing
...
public class Literal extends StringValuedToken {
  ...
  /** Indicates if a character for the first half of a hex byte has already been buffered */
  ...
  /**
   * The state of the tokenization. Only the subset of States which pertain to Literals are used
   * here.
   */
  private State _state;

  /**
   * True if no discrepancies with PDF/A requirements have been found, false if there is a
   * discrepancy in this literal.
   */
   ...
   * parentheses, as opposed to hex strings.
   *
   * @param tok The tokenizer, passed to give access to its getChar function.
   * @return <code>true</code> if the character was processed normally, <code>false</code> if a
   *     terminating parenthesis was reached.
   * @throws IOException
   */
  ...
  /**
   * Convert the raw hex data.Two buffers are saved: _rawBytes for the untranslated hex-encoded
   * data, and _value for the PDF or UTF encoded string.
   *
   * @throws edu.harvard.hul.ois.jhove.module.pdf.PdfException
   */
  public void convertHex() throws PdfException {
    if (_rawBytes != null) {
      ...
      if (_rawBytes.size() >= 2 && rawByte(0) == 0XFE && rawByte(1) == 0XFF) {
        utf = true;
      }
      if (utf) {
  ...
  /**
   * Returns <code>true</code> if this string is in PDFDocEncoding, false if UTF-16.
   *
  ...
  /**
   * Returns <code>true</code> if the string value is a parsable date. Conforms to the ASN.1 date
   * format: D:YYYYMMDDHHmmSSOHH'mm' where everything before and after YYYY is optional. If we take
   * this literally, the format is frighteningly ambiguous (imagine, for instance, leaving out hours
   * but not minutes and seconds), so the checking is a bit loose.
   *
  ...
  /**
   * Parse the string value to a date. PDF dates conform to the ASN.1 date format. This consists of
   * D:YYYYMMDDHHmmSSOHH'mm' where everything before and after YYYY is optional. Adobe doesn't
   * actually say so, but I'm assuming that if a field is included, everything to its left must be
   * included, e.g., you can't have seconds but leave out minutes.
   *
  ...
  /**
   * Returns <code>true</code> if this token doesn't violate any PDF/A rules, <code>false</code> if
   * it does.
   *
  ...
  */
  ...
  /**
   * After a backslash, read characters into an escape sequence. If we don't find a valid escape
   * sequence, return 0.
   */
  ...
  /**
   * We have just read an ESC in a UTF string. Save all character up to and exclusive of the next
   * ESC as a language code.
   */
  ...
  /**
   * If we're in the backslash substate (backslashFlag = true), then call this to process
   * characters. It will accumulate octal digits into octalBuf and process other escaped characters.
   * If the accumulation produces a character, it will return that character code, otherwise it will
   * return 0 to indicate no character is available yet.
   *
   * <p>Althought the backslash itself is a byte, even in a 16-bit string, the characters which
   * follow it are 16-bit characters, not bytes. So we call this only after applying UTF-16 encoding
   * where applicable.
   */
