...
public abstract class DocNode {
  ...
  /**
   * Get the next DocNode which is under this node. All PageTreeNodes and PageObjects are eventually
   * returned by walking through a structure with nextNode.
   */
  ...
  /**
   * Get the Resources dictionary. Either a PageTreeNode or a PageObject can have a Resources
   * dictionary. Returns null if there is no Resources dictionary. The object may be referenced
   * indirectly.
   */
   ...
   * Otherwise returns null. The dictionary will most often have indirect object references as
   * values. What is returned is not a Font dictionary, but rather a dictionary of Font
   * dictionaries.
   */
  public PdfDictionary getFontResources() throws PdfException {
    ...
    try {
      ...
      throw new PdfMalformedException(MessageConstants.PDF_HUL_6); // PDF-HUL-6
    }
  }

  /**
   * Get the MediaBox of this node. MediaBox is an inheritable property, so it walks up the chain of
   * ancestors if it doesn't contain one. Returns null if none. Throws a PdfInvalidException if an
   * invalid MediaBox is found.
   */
  ...
  /**
   * Get an named property. If this object doesn't have the specified property and <code>inheritable
   * </code> is true, walks up the chain of ancestors to try to find one. If no ancestor has the
   * property or inheritable is false, returns null.
   */
