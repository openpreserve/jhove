...
/**
 * Class encapsulating a PDF page tree node. The page tree is built such that callers can walk
 * through it by calling startWalk and then calling nextDocNode (for all nodes) or nextPageObject
 * (for pages only) repeatedly.
 */
public class PageTreeNode extends DocNode {
    ...
    try {
      ...
      if (toplevel && kids == null) {
        ...
        for (int i = 0; i < kidsVec.size(); i++) {
          ...
          /**
           * ************************************************ To avoid a simple case of infinite
           * recursion, check that this kid is not the same page object as its parent.
           * ************************************************
           */
          /**
           * ************************************************ int kidObjNumber = kidRef.getObjNumber
           * (); int kidGenNumber = kidRef.getGenNumber (); if (objNumber >= 0 && genNumber >= 0 &&
           * objNumber == kidObjNumber && genNumber == kidGenNumber) { break; }
           * ************************************************
           */
  ...
  /**
   * Get the next PageObject which is under this node. This function is designed such that calling
   * startWalk() and then repeatedly calling nextPageObject() will return all the PageObjects in the
   * tree under this node, and finally will return null when there are no more.
   */
  @Override
  ...
  /**
   * Get the next DocNode which is under this node. This function is designed such that calling
   * startWalk() and then repeatedly calling nextPageObject() will return first this node, then all
   * the DocNodes in the tree under this node. It finally will return null when there are no more.
   */
  @Override
