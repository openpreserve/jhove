...
public enum ControlChar {
  ...
  DEL("DEL", 0x7F, "DELETE"),
  /** Unicode Extensions 0x80 - 0x9F Currently not used */
  ...
  public static final EnumSet<ControlChar> ASCII = EnumSet.range(NUL, DEL);
  /** Set of Control Chars that are only Unicode */
  public static final EnumSet<ControlChar> UNICODE_EXTENSIONS = EnumSet.complementOf(ASCII);
  /** Set of Unicode Control Chars, {@code ASCII} + {@code UNICODE_EXTENSIONS} */
  public static final EnumSet<ControlChar> UNICODE = EnumSet.allOf(ControlChar.class);
   ...
   *
   * @param toTest the ControlChar to test
   * @return true if toTest is a CR or LF character.
   */
  public static final boolean isLineEndChar(final ControlChar toTest) {
   ...
   *
   * @param charVal the int value of a possible ASCII character
   * @return the control character with matching ASCII value or null if the value is out of the
   *     ASCII control character range.
   */
  public static final ControlChar asciiFromInt(final int charVal) {
    if (charVal > US.value) {
  ...
  public static final ControlChar fromMnemonic(final String mnemonic) {
    for (ControlChar ctrlChar : ControlChar.values()) {
