...
public class AsciiModule extends ModuleBase {
  public static final int MAX_CHAR = 0x7f;
  ...
  private static final String WELLFORMED =
      "An ASCII object is well-formed " + "if each byte is between 0x00 and 0x7F";
  private static final String VALIDITY = null;
  private static final String REPINFO =
      "Additional representation information includes: line ending and control characters";
  private static final String NOTE = null;
  private static final String RIGHTS =
      "Copyright 2003-2015 by JSTOR and "
  ...
  public AsciiModule() {
    ...
    doc =
        new Document(
            "Information Systems -- Coded Character Sets "
                + "7-Bit American National Standard Code for "
                + "Information Interchange (7-Bit ASCII)",
            DocumentType.STANDARD);
    Builder builder =
        new Agent.Builder("ANSI", AgentType.STANDARD)
    ...
    doc.setDate("1991-12");
    builder =
        new Agent.Builder("ECMA", AgentType.STANDARD)
  ...
  public final int parse(InputStream stream, RepInfo info, int parseIndex) throws IOException {
    ...
    while (!eof) {
      try {
        ...
        if (ch > MAX_CHAR) {
          ErrorMessage error =
              new ErrorMessage(
                  MessageConstants.ASCII_HUL_1,
                  String.format(
                      MessageConstants.ASCII_HUL_1_SUB.getMessage(),
    ...
    if (!metadataList.isEmpty()) {
      info.setProperty(
          new Property("ASCIIMetadata", PropertyType.PROPERTY, PropertyArity.LIST, metadataList));
    }
  ...
  /**
   * Check if the digital object conforms to this Module's internal signature information. An ASCII
   * file has no "signature," so in cases like this we just check the beginning of the file as a
   * plausible guess. This really proves nothing, since a text file could have a single accented
