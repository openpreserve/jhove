...
/**
 * This class defines the permitted behavior of a particular HTML tag. The full descriptive power of
 * a DTD description isn't implemented here, but content types and sequences of content are
 * implemented.
 *
 * @author Gary McGath
 */
public class HtmlTagDesc {
   ...
   * @param content List of permitted tags. But what do I do when a particular order is required?
   *     Null if element is defined at EMPTY.
   * @param attributes List of HtmlAttributeDesc elements enumerating the permitted attributes. May
   *     be null, in which case _attributes will be stored as an empty list.
   */
  public HtmlTagDesc(
      String name,
   ...
   * @param openTagRequired <code>true</code> if an opening tag is required
   * @param closeTagRequired <code>true</code> if a closing tag is required
   * @param sequence Array indicating the sequencing of elements in content. Must have the same
   *     length as <code>sequence</code>.
   * @param attributes List of HtmlAttributeDesc elements enumerating the permitted attributes. May
   *     be null, in which case _attributes will be stored as an empty list.
   */
  public HtmlTagDesc(
  ...
  /**
   * Specifies tags which may not be included in this element or in any element nested at any depth
   * within it. Corresponds to the -(content) feature of the DTD.
   */
  ...
  /**
   * Returns <code>true</code> if a given tag is excluded within this element. It is necessary to
   * call this method for each element on the stack to determine if it is excluded.
   */
  ...
  /**
   * Alternative way of setting the attribute names. This can be used where all the attributes are
   * unrestricted. This will replace any previously set attributes.
   */
  ...
  /**
   * Provides the object with an array of element tags in which this element can be nested. Only
   * tags for elements with optional opening tags may be listed here.
   */
  ...
  /**
   * Reports whether this is a temporary tag descriptor. Returns <code>false</code> unless
   * overridden.
   */
  ...
  /**
   * Reports whether this element can be implicitly nested in an element with a given tag. There may
   * be more than one implicit container for a tag; if the DTD is unambiguous, there should be only
   * one which is permissible in any given context.
   */
  ...
  /**
   * Reports whether additional elements can be matched at the specified content index. The index is
   * assumed to be legal.
   */
  ...
  /**
   * Reports whether it's legal to advance to the next content index. The index is assumed to be
   * legal, but the one to which it's trying to advance may not be.
   */
  ...
  /**
   * Reports whether this element allows a given tag name in its content. This version should be
   * used only with element descriptors that aren't associated with tags, for determining if a
   * hypothetical implied element could contain the given tag.
   */
  ...
  /**
   * Returns the attribute with a given name, or null if no such attribute is defined for the
   * element
   */
  ...
  /**
   * Accepts a list of attribute names, and returns a List of required attribute names which are not
   * present in the parameter list. Returns an empty List if all required attributes are present.
   */
