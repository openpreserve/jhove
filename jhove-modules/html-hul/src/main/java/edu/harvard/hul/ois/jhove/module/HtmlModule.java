 ...
 *
 * <p>HTML is different from most of the other documents in that sloppy construction is practically
 * assumed in the specification. This module attempt to report as many errors as possible and
 * recover reasonably from errors. To do this, there is more heuristic behavior built into this
 * module than into the more straightforward ones.
 *
 * <p>XHTML is recognized by this module, but is handed off to the XML module for processing. If the
 * XML module is missing (which it shouldn't be if you've installed the JHOVE application without
 * modifications), this won't be able to deal with XHTML files.
 *
 * <p>HTML should be placed ahead of XML in the module order. If the XML module sees an XHTML file
 * first, it will recognize it as XHTML, but won't be able to report the complete properties.
 *
 * <p>The HTML module uses code created with the JavaCC parser generator and lexical analyzer
 * generator. There is apparently a bug in JavaCC which causes blank lines not to be counted in
 * certain cases, causing lexical errors to be reported with incorrect line numbers.
 *
 * @author Gary McGath
 */
public class HtmlModule extends ModuleBase {
  ...
  private static final String VALIDITY =
      "An HTML file is valid if it is " + "well-formed and has a valid DOCTYPE declaration.";
  private static final String REPINFO =
      "Languages, title, META tags, "
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) {
    if (parseIndex != 0) {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.JHOVE_1));
    ...
    try {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.JHOVE_2, e.getMessage()));
    ...
    try {
      ...
      if (elements.isEmpty()) {
        ...
        info.setWellFormed(false);
        info.setMessage(new ErrorMessage(MessageConstants.JHOVE_3));
        return 0;
      }
      type = checkDoctype(elements);
      if (type < 0) {
        info.setWellFormed(false);
        info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_15));
        return 0;
      }
      /*
      ...
      if (!hasElements) {
        info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_17));
        info.setWellFormed(false);
        return 0;
      }
      ...
      if (lineEnd == null) {
        info.setMessage(new InfoMessage(MessageConstants.INF_EOL_TYPE_UNDET));
      ...
      if (type == 0) {
        ...
        switch (seemsToBeXHTML(elements)) {
          ...
          case 1: // XML but not HTML
            info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_14));
            info.setWellFormed(false);
            return 0;
        ...
        info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_16));
        info.setValid(false);
        // But keep going
      }
      ...
      if (docDesc == null) {
        info.setMessage(new InfoMessage(MessageConstants.INF_HTML_VER_UNSPPRTD));
      ...
      info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_19, f.getLocalizedMessage()));
      info.setWellFormed(false);
    }
