...
/**
 * This is an abstract class for processing an HTML document that has been parsed into a List of
 * HtmlElements. It defines common behavior for all supported versions of HTML except XHTML.
 * Subclasses modify this base as needed.
 *
 * @author Gary McGath
 */
public abstract class HtmlDocDesc {
  ...
  /**
   * Generic list of supported tags. For efficiency, this is generated only once. Subclasses will
   * need to get a copy of this list and make additions or deletions as necessary. They must not
   * modify any of the existing members of the list.
   */
  protected static HashMap commonTags;

  /**
   * List of supported tags for this version of HTML. The subclass is responsible for generating
   * this, typically using commonTags as a starting point.
   */
   ...
   * @param info The RepInfo object which will be populated with properties
   */
  public boolean validate(List elements, RepInfo info) {
    ...
    if (!elementStack.isHeadSeen()) {
      info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_11));
      info.setValid(false);
    }
    return true;
  }
  ...
  private void doOpenTag(JHOpenTag tag, RepInfo info) {
    ...
    if (msg != null) {
      info.setMessage(
          new ErrorMessage(
              msg,
              "Name = " + name + ", Line = " + tag.getLine() + ", Column = " + tag.getColumn()));
  ...
  private void doPCData(JHPCData elem, RepInfo info, HtmlMetadata metadata) {
    ...
    if (elementStack.isEmpty()) {
      ...
      info.setMessage(new ErrorMessage(MessageConstants.HTML_HUL_12));
      info.setValid(false);
      return;
    }
  ...
  private boolean checkElementContext(JHOpenTag elem, RepInfo info) {
    ...
    if (topElem == htmlElement) {
      ...
      if (!elementStack.isBodySeen() && bodyElement != null && bodyElement.allowsTag(name, this)) {
   ...
   * This kind of attribute is normally represented in HTML without an explicit value; in fact, some
   * (most?) readers won't permit an explicit value.
   */
  protected static void addSelfAttribute(List atts, String name) {
    atts.add(new HtmlAttributeDesc(name, new String[] {name}, HtmlAttributeDesc.IMPLIED));
  }
