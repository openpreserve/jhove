...
/**
 * A LinkedList dressed up as a stack for processing HTML objects. It knows about certain elements
 * and their history on the stack.
 *
 * @author Gary McGath
 */
public class HtmlStack extends LinkedList {
  ...
  /**
   * Pops top element from element stack. If we ever decide to go to a different stack
   * implementation, it's necessary only to change these methods. Also, they add some type checking.
   *
   * <p>Name changed from "pop" to "popp" to avoid a conflict in Java 1.6 with the List class.
   */
  protected void popp() {
    removeLast();
  }
  ...
  /** Returns <code>true</code> if a HEAD element has been pushed on the stack. */
  protected boolean isHeadSeen() {
    return headSeen;
  }

  /** Returns <code>true</code> if a BODY element has been pushed on the stack. */
  protected boolean isBodySeen() {
    return bodySeen;
  }

  /** Returns <code>true</code> if any element on the stack prohibits the specified tag. */
