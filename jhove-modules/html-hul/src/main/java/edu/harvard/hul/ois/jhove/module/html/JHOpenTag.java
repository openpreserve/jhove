...
/**
 * Representation of a parsed HTML open tag, including its attributes. This arguable would better be
 * called an element, but JHElement is the name of the abstract superclass.
 *
 * @author Gary McGath
 */
public class JHOpenTag extends JHElement {
  ...
  public String _name;
  /**
   * List of element attributes. Each attributes is an array of two strings, the name and the value.
   * If no explicit value was given, attribute[1] is null. If the attribute was in quotes, the
   ...
   * Constructor.
   *
   * @param elements The list of parsed elements, to which this gets added. May be null for a stub
   *     element not generated by the parser.
   * @param name The name of the tag
   * @param attrs A List of attributes, representing the parsed attributes of the tag. Each
   *     attributes is an array of two strings, the name and the value. If no explicit value was
   *     given, attribute[1] is null. If the attribute was in quotes, the quotes are still there.
  ...
  /**
   * Constructor with error message. This is used to allow constructs which are erroneous but common
   ...
   *     attributes is an array of two strings, the name and the value. If no explicit value was
   *     given, attribute[1] is null. If the attribute was in quotes, the quotes are still there.
   * @param message An error message indicating that this element isn't well-formed, but we'll take
   *     it anyway.
   */
  ...
  /**
   * Constructor for a stub attribute. This shouldn't ever be used by the parser, but only by the
   * module for generating implied elements.
   */
  ...
  /**
   * Associates an the tag with an element definition. This is done by the HTML module, not by the
   * parser.
   */
   ...
   *
   * @return The attributes as a List. Each attributes is an array of two strings, the name and the
   *     value. If no explicit value was given, attribute[1] is null. If the attribute was in
   *     quotes, the quotes are still there.
   */
  ...
  /** Returns <code>true</code> if the tag given in the parameter is allowable in our context. */
  ...
  /**
   * Returns the error message associated with this element. If it returns a non-null value, the tag
   * is not well-formed, and the error should be reported.
   */
  ...
  /**
   * Processes metadata from an A element. Only elements with an HREF attribute are of interest
   * here. We ignore links to anchors.
   */
