...
public class WaveModuleTest {
  ...
  public void testSample3() throws IOException {
    ...
    assertEquals("Should not be well formed. ", info.getWellFormed(), RepInfo.FALSE);

    // Check that there is a message:
    assertTrue("There should be at least one message. ", info.getMessage().size() > 0);
    ...
    for (Message m : info.getMessage()) {
      ...
      if ("WAVE-HUL-3".equals(m.getId()) && m.getOffset() == 96) {
    ...
    assertTrue(
        "The message of id=WAVE-HUL-3 ('Unexpected end of file')@96 was not found. ",
        foundEofMessage);
  }
}
