 ...
 * Validation Environment Copyright 2004-2005 by the President and Fellows of Harvard College
 *
...
public class WDump extends Dump {
    ...
    try {
      ...
      while (!endOfFile) {
        ...
        try {
          ...
          System.out.print(leading(os, 8) + os + ": " + ckID + " " + ckSize);
          long alreadyRead = 0;
          if ("fact".equals(ckID)) {
            long sampleLength = ModuleBase.readUnsignedInt(stream, ENDIAN);
            System.out.print(": " + sampleLength);
            alreadyRead = 4;
          } else if ("fmt ".equals(ckID)) {
            int formatTag = ModuleBase.readUnsignedShort(stream, ENDIAN);
            int channels = ModuleBase.readUnsignedShort(stream, ENDIAN);
            long samplesPerSec = ModuleBase.readUnsignedInt(stream, ENDIAN);
            long avgBytesPerSec = ModuleBase.readUnsignedInt(stream, ENDIAN);
            int blockAlign = ModuleBase.readUnsignedShort(stream, ENDIAN);
            ...
            alreadyRead = 14;
            if (ckSize > 14) {
              int bitsPerSample = ModuleBase.readUnsignedShort(stream, ENDIAN);
              System.out.print(" " + bitsPerSample);
              alreadyRead = 16;
              if (ckSize > 16) {
                int size = ModuleBase.readUnsignedShort(stream, ENDIAN);
                System.out.print(" " + size);
                alreadyRead = 18;
                if (size == 22) {
                  int validBitsPerSample = ModuleBase.readUnsignedShort(stream, ENDIAN);
                  long channelMask = ModuleBase.readUnsignedInt(stream, ENDIAN);
                  hex = Long.toHexString(channelMask);
                  System.out.print(
                      " " + validBitsPerSample + " 0x" + leading(hex, 8) + hex + " 0x");
                  for (int i = 0; i < 4; i++) {
                    long guid = ModuleBase.readUnsignedInt(stream, ENDIAN);
                    hex = Long.toHexString(guid);
                    System.out.print(leading(hex, 8) + hex);
                  }
                  alreadyRead = 40;
                }
              }
  ...
  hold nested chunks. */
  private static void readNestedChunks(String ckID, DataInputStream stream, long ckSize, long os)
      throws IOException {
    ...
    while (ckSize > 0) {
      ...
      System.out.println(leading(os, 8) + os + ": " + ckID + "/" + subCkID + " " + subCkSize);
