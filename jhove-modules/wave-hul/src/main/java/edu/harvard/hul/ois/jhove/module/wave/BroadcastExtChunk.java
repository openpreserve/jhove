...
public class BroadcastExtChunk extends Chunk {
  ...
  private static final int UNUSED_LOUDNESS_FIELD = 0x7FFF;
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    if (version >= 2) {
      ...
      maxMomentaryLoudness = formatLoudness(module.readSignedShort(_dstream));
      maxShortTermLoudness = formatLoudness(module.readSignedShort(_dstream));
    }
    ...
    if (version == 0) {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              String.format(MessageConstants.WAVE_HUL_27.getMessage(), Integer.valueOf(version)));
    ...
    if (!originatorRef.isEmpty()) {
      plist.add(new Property("OriginatorReference", PropertyType.STRING, originatorRef));
    }
    ...
    plist.add(new Property("TimeReference", PropertyType.LONG, Long.valueOf(timeReference)));
    plist.add(new Property("Version", PropertyType.INTEGER, Integer.valueOf(version)));
    ...
    if (!loudnessValue.isEmpty()) {
      plist.add(new Property("LoudnessValue", PropertyType.STRING, loudnessValue));
    }
    if (!loudnessRange.isEmpty()) {
      plist.add(new Property("LoudnessRange", PropertyType.STRING, loudnessRange));
    }
    if (!maxTruePeakLevel.isEmpty()) {
      plist.add(new Property("MaxTruePeakLevel", PropertyType.STRING, maxTruePeakLevel));
    }
    if (!maxMomentaryLoudness.isEmpty()) {
      plist.add(new Property("MaxMomentaryLoudness", PropertyType.STRING, maxMomentaryLoudness));
    }
    if (!maxShortTermLoudness.isEmpty()) {
      plist.add(new Property("MaxShortTermLoudness", PropertyType.STRING, maxShortTermLoudness));
    }
   ...
   *
   * @return an empty String if no UMID is found; a basic UMID if the extension space is empty; and
   *     an extended UMID if it isn't
   */
   ...
   *
   * <p>Loudness fields store integer values accurate to two decimal places by multiplying the
   * original value by 100, and rounding away any remainder. To recover the original value, we do
   * the reverse.
   *
   * <p>Unused fields should contain the value 0x7FFF.
   */
