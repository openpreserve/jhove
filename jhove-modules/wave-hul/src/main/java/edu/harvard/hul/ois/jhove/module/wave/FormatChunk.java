...
public class FormatChunk extends Chunk {
  ...
  private static final int[] losslessCodecs = {
    0x0163, // WMA lossless
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    if (bytesLeft > 0) {
      ...
      if (waveCodec == WAVE_FORMAT_PCM && (((bitsPerSample + 7) / 8) * numChannels) == blockAlign) {
    ...
    if (waveCodec != WAVE_FORMAT_EXTENSIBLE) {
      ...
      String extendedMIMEType = _module.getMimeType()[0] + "; codec=" + codecID;
    ...
    module.addWaveProperty(
        new Property("AverageBytesPerSecond", PropertyType.LONG, Long.valueOf(bytesPerSecond)));
    module.addWaveProperty(
        new Property("BlockAlign", PropertyType.INTEGER, Integer.valueOf(blockAlign)));
    if (extraBytes != null) {
      module.addWaveProperty(
          new Property("ExtraFormatBytes", PropertyType.BYTE, PropertyArity.ARRAY, extraBytes));
    }
    if (validBitsPerSample != -1) {
      ...
      module.addWaveProperty(
          new Property(
              "ValidBitsPerSample", PropertyType.INTEGER, Integer.valueOf(validBitsPerSample)));
    }
    if (channelMask != -1) {
      module.addWaveProperty(
          new Property("ChannelMask", PropertyType.LONG, Long.valueOf(channelMask)));
    }
    if (subformat != null) {
      module.addWaveProperty(new Property("Subformat", PropertyType.STRING, subformat));
    }
  ...
  /**
   * Set default channel assignments. This is fairly simple, but it's helpful to keep the same
   * structure as the equivalent CommonChunk.setChannelLocations function.
   */
