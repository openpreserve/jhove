...
/**
 * Implementation of the WAVE Link Chunk, as specified in <cite>Specification of the Broadcast Wave
 * Format: A format for audio data files in broadcasting; Supplement 4: &lt;link&gt; Chunk</cite>
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    try {
      ...
      if (!fileNames.isEmpty()) {
        ...
        plist.add(new Property("FileNames", PropertyType.STRING, PropertyArity.LIST, fileNames));
        if (id != null) {
          plist.add(new Property("ID", PropertyType.STRING, id));
        }

        module.addWaveProperty(
            new Property("Link", PropertyType.PROPERTY, PropertyArity.LIST, plist));
      }

    } catch (SAXException se) {
      info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_13));
      info.setValid(false);
      return true;
    } catch (ParserConfigurationException pe) {
      info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_14));
      info.setValid(false);
      return true;
    }
    return true;
