...
/**
 * This implements any of the subchunks of the ListInfoChunk (a LIST chunk with a list type of
 * INFO). All such chunks are identical in format, consisting of a null-terminated string. About 17
 * chunk ID's are recognized; others will be ignored.
 *
 * @author Gary McGath
 */
public class ListInfoTextChunk extends Chunk {
   ...
   * @param listInfoProps A List of the Properties associated with the ListInfoChunk
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    if (propName != null) {
      _listInfoProps.add(new Property(propName, PropertyType.STRING, txt));
    } else {
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              MessageConstants.WAVE_HUL_19.getId(),
              String.format(MessageConstants.WAVE_HUL_19.getMessage(), _chunkID));
