 ...
 *
 * <p>Two chunk types, 'exif' and 'INFO', are supported; other list types will be reported as
 * unknown and treated as an error.
 *
 * @author Gary McGath
 */
public class ListInfoChunk extends Superchunk {
    ...
    if ("INFO".equals(typeID)) {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              MessageConstants.WAVE_HUL_15.getId(),
              String.format(MessageConstants.WAVE_HUL_15.getMessage(), typeID));
  ...
  private boolean readInfoChunk(RepInfo info) throws IOException {
    ...
    for (; ; ) {
      ...
      chunk = new ListInfoTextChunk(_module, chunkh, _dstream, listInfoProps, this);
  ...
  /** The Exif chunk, unlike the Info chunk, has subchunks which aren't homogeneous. */
  private boolean readExifChunk(RepInfo info) throws IOException {
    ...
    for (; ; ) {
      ...
      if (chunk == null) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(
                MessageConstants.WAVE_HUL_17.getId(),
                String.format(MessageConstants.WAVE_HUL_17.getMessage(), id));
   ...
   *
   * @return <code>false</code> if the chunk or a nested chunk is structurally invalid, otherwise
   *     <code>true</code>
   */
  public boolean readAdtlChunk(RepInfo info) throws IOException {
    for (; ; ) {
      ...
      if (chunk == null) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(
                MessageConstants.WAVE_HUL_18.getId(),
                String.format(MessageConstants.WAVE_HUL_18.getMessage(), id));
        info.setMessage(new InfoMessage(message));
      } else {
        if (!chunk.readChunk(info)) {
          return false;
