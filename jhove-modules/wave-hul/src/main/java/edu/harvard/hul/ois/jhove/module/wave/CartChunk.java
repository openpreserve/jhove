...
public class CartChunk extends Chunk {
  ...
  /** Number of timer tags. This is a fixed value specified by the chunk definition. */
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    for (int i = 0; i < N_TIMER_TAGS; i++) {
      ...
      if (timerTagUsage.length() > 0) {
        ...
        ttprop[0] = new Property("Usage", PropertyType.STRING, timerTagUsage);
        ttprop[1] = new Property("Value", PropertyType.LONG, new Long(timerTagValue));

        timerTags.add(
            new Property("PostTimer", PropertyType.PROPERTY, PropertyArity.ARRAY, ttprop));
      }
    }
    ...
    if (classification.length() > 0) {
      plist.add(new Property("Classification", PropertyType.STRING, classification));
    }
    if (outCue.length() > 0) {
    ...
    if (producerAppID.length() > 0) {
      plist.add(new Property("ProducerAppID", PropertyType.STRING, producerAppID));
    }
    if (producerAppVersion.length() > 0) {
      plist.add(new Property("ProducerAppVersion", PropertyType.STRING, producerAppVersion));
    }
    if (userDef.length() > 0) {
      plist.add(new Property("UserDef", PropertyType.STRING, userDef));
    }
    plist.add(new Property("LevelReference", PropertyType.INTEGER, new Integer(levelReference)));
    if (timerTags.size() > 0) {
