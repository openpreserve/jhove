...
/**
 * The associated data list ('list') chunk, which is different from the RIFF 'LIST' chunk,
 * ListInfoTextChunk. It can contain several different types of informational chunks.
 *
 * @author Gary McGath
 */
public class AssocDataListChunk extends Superchunk {
  ...
  public AssocDataListChunk(
      ModuleBase module, ChunkHeader hdr, DataInputStream dstrm, RepInfo info) {
  ...
  public boolean readChunk(RepInfo info) throws IOException {
    ...
    if (!"adtl".equals(typeID)) {
      info.setMessage(
          new ErrorMessage(
              MessageConstants.WAVE_HUL_9,
              String.format(MessageConstants.WAVE_HUL_9_SUB.getMessage(), typeID),
      ...
      if (chunk == null) {
        ...
        JhoveMessage message =
            JhoveMessages.getMessageInstance(
                MessageConstants.WAVE_HUL_10.getId(),
                String.format(MessageConstants.WAVE_HUL_10.getMessage(), id));
