 ...
 *
 * <p>There is no published specification for WAVE files; this module is based on several Internet
 * sources.
 *
 * <p>WAVE format is a type of RIFF format. RIFF, in turn, is a variant on EA IFF 85.
 *
 * @author Gary McGath
 */
public class WaveModule extends ModuleBase {
  ...
  private static final String COVERAGE =
      "WAVE (PCMWAVEFORMAT, WAVEFORMATEX, WAVEFORMATEXTENSIBLE); "
          + "Broadcast Wave Format (BWF) version 0, 1 and 2; RF64";
  private static final String[] MIMETYPES = {
    "audio/vnd.wave", "audio/wav", "audio/wave", "audio/x-wav", "audio/x-wave"
  };
  ...
  /**
   * Map of 64-bit chunk sizes found in the Data Size 64 chunk. <code>Long</code> size values should
  ...
  /**
   * Number of samples in the file. Obtained from the Data chunk for uncompressed files, and the
   * Fact chunk for compressed ones. Value should be treated as unsigned.
   */
   ...
   * Class constructor.
   *
   * <p>Instantiates a <code>WaveModule</code> object.
   */
  public WaveModule() {
    super(
        NAME,
    ...
    Agent msAgent =
        new Agent.Builder("Microsoft Corporation", AgentType.COMMERCIAL)
    ...
    Agent ebuAgent =
        new Agent.Builder("European Broadcasting Union", AgentType.COMMERCIAL)
            .address(
    ...
    doc.setIdentifier(
        new Identifier("https://tech.ebu.ch/docs/tech/tech3285.pdf", IdentifierType.URL));
    ...
    doc.setIdentifier(
        new Identifier("https://tech.ebu.ch/docs/tech/tech3306-2009.pdf", IdentifierType.URL));
    doc.setPublisher(ebuAgent);
    doc.setDate("2009-07");
    _specification.add(doc);

    Agent ietfAgent =
        new Agent.Builder("IETF", AgentType.STANDARD).web("https://www.ietf.org").build();
    ...
    sig =
        new ExternalSignature(
            ".rf64", SignatureType.EXTENSION, SignatureUseType.OPTIONAL, "For RF64 profile");
    _signature.add(sig);

    sig =
        new InternalSignature(
            "RIFF", SignatureType.MAGIC, SignatureUseType.MANDATORY_IF_APPLICABLE, 0);
    _signature.add(sig);

    sig =
        new InternalSignature(
            "RF64", SignatureType.MAGIC, SignatureUseType.MANDATORY_IF_APPLICABLE, 0);
    _signature.add(sig);
  ...
  public int parse(InputStream stream, RepInfo info, int parseIndex) {
    ...
    try {
      ...
      if (firstFourChars.equals(RF64_SIGNATURE)) {
        ...
        info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_1, 0));
      ...
      if (!"WAVE".equals(formType)) {
        info.setMessage(
            new ErrorMessage(MessageConstants.WAVE_HUL_2, _nByte - RIFF_FORM_TYPE_LENGTH));
        info.setWellFormed(false);
        return 0;
      }
        ...
        if (readChunk(info) && dataSize64ChunkSeen) {
          if (riffSize == LOOKUP_EXTENDED_DATA_SIZE) {
            ...
            if (Long.compareUnsigned(extendedRiffSize, Long.MAX_VALUE) > 0) {
              info.setMessage(new InfoMessage(MessageConstants.WAVE_HUL_22));
          ...
          info.setMessage(
              new ErrorMessage(
                  MessageConstants.WAVE_HUL_23, Chunk.HEADER_LENGTH + RIFF_FORM_TYPE_LENGTH));
          info.setWellFormed(false);
          return 0;
        }
      }
      ...
      if (getBytesRemaining() > 0) {
        ...
        remainingDataInfo(_dstream, info, getBytesRemaining(), firstFourChars);
      }

    } catch (EOFException eofe) {
      info.setWellFormed(false);
      String subMessage =
          String.format(MessageConstants.WAVE_HUL_3_SUB.getMessage(), getBytesRemaining());
      if (eofe.getMessage() != null) {
        subMessage += "; " + eofe.getMessage();
      }
      info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_3, subMessage, _nByte));
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              String.format(
                  MessageConstants.WAVE_HUL_4.getMessage(),
   ...
   *
   * <p>If no ExifInfo object has been set, returns <code>null</code>.
   */
  ...
  /**
   * One-argument version of <code>readSignedLong</code>. WAVE is always little-endian, so we can
  ...
  /**
   * One-argument version of <code>readUnsignedInt</code>. WAVE is always little-endian, so we can
   * unambiguously drop its endian argument.
   */
  ...
  /**
   * One-argument version of <code>readSignedInt</code>. WAVE is always little-endian, so we can
   * unambiguously drop its endian argument.
   */
  ...
  /**
   * One-argument version of <code>readUnsignedShort</code>. WAVE is always little-endian, so we can
   * unambiguously drop its endian argument.
   */
  ...
  /**
   * One-argument version of <code>readSignedShort</code>. WAVE is always little-endian, so we can
   * unambiguously drop its endian argument.
   */
  ...
  /**
   * Adds to the number of data bytes. This may be called multiple times to give a cumulative total.
   */
  ...
  protected boolean readChunk(RepInfo info) throws IOException {
    ...
    if (Long.compareUnsigned(getBytesRemaining(), chunkSize) < 0) {
      info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_6, _nByte - Chunk.SIZE_LENGTH));
      info.setWellFormed(false);
      return false;
    }
    ...
    if ("fmt ".equals(chunkId)) {
      ...
      if (!formatChunkSeen) {
        info.setMessage(new ErrorMessage(MessageConstants.WAVE_HUL_25, chunkh.getOffset()));
        info.setValid(false);
      }
      if (dataChunkSeen) {
      ...
      JhoveMessage message =
          JhoveMessages.getMessageInstance(
              ...
              String.format(MessageConstants.WAVE_HUL_7.getMessage(), chunkId));
  ...
  /**
   * Reports and passes over any data still remaining following an attempt at reading a chunk. This
   * ensures we align with the start of any subsequent chunk.
   */
  private void remainingDataInfo(
      DataInputStream stream, RepInfo info, long bytesToProcess, String chunkId)
      throws IOException {
    ...
    if (stream.available() > 0) {
      ...
      bytesProcessed += skipBytes(stream, bytesToProcess - bytesProcessed, this);

      info.setMessage(
          new InfoMessage(
              MessageConstants.WAVE_HUL_26,
              String.format(
                  MessageConstants.WAVE_HUL_26_SUB.getMessage(), chunkId, bytesProcessed, nullData),
              _nByte - bytesProcessed));

    } else {
      throw new EOFException(
          String.format(MessageConstants.WAVE_HUL_3_SUB_2.getMessage(), chunkId));
    }
  }
  ...
  protected void dupChunkError(RepInfo info, String chunkName) {
    JhoveMessage message =
        JhoveMessages.getMessageInstance(
            MessageConstants.WAVE_HUL_8.getId(),
            String.format(MessageConstants.WAVE_HUL_8.getMessage(), chunkName));
    info.setMessage(new ErrorMessage(message, _nByte - Chunk.HEADER_LENGTH));
    info.setValid(false);
  }
  ...
  /**
   * Returns whether or not the module has parsed the chunks required to provide extended data
   * sizes, namely RF64's Data Size 64 chunk.
   */
  ...
  /**
   * Adds a chunk's extended chunk size to the map of extended sizes. If a chunk has previously been
   * mapped, its chunk size will be replaced.
   */
