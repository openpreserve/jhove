...
/**
 * Encapsulation of Exif information for a Wave file. Exif sound information doesn't closely follow
 * Exif image information, so we don't particularly try to force property names to match.
 *
...
public class ExifInfo {
  ...
  public Property buildProperty() {
    ...
    if (_relatedImageFile != null) {
      entries.add(new Property("RelatedImageFile", PropertyType.STRING, _relatedImageFile));
    }
    if (_timeCreated != null) {
      entries.add(new Property("TimeCreated", PropertyType.STRING, _timeCreated));
    }
    if (_manufacturer != null) {
      entries.add(new Property("Manufacturer", PropertyType.STRING, _manufacturer));
    }
    if (_model != null) {
      entries.add(new Property("Model", PropertyType.STRING, _model));
    }
    if (_makerNote != null) {
      entries.add(new Property("MakerNote", PropertyType.BYTE, PropertyArity.ARRAY, _makerNote));
    }
    if (_userComment != null) {
      entries.add(new Property("UserComment", PropertyType.STRING, _userComment));
    }

    return new Property("Exif", PropertyType.PROPERTY, PropertyArity.LIST, entries);
  }
