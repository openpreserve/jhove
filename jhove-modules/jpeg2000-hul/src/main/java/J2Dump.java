...
public class J2Dump extends Dump {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (!endOfFile) {
        ...
        System.out.println(
            leading(os, 8) + os + ": " + stackPrefix(boxStack) + box.type + " " + box.length);
        if (box.isSuperbox()) {
          boxStack.push(box);
        } else {
          os += box.bytesLeft;
  ...
  class Box {
    ...
    /**
     * Returns true if this box contains other boxes. At present, we don't deal with the insides of
     * boxes that contain both data and boxes (e.g., cref).
     */
    public boolean isSuperbox() {
      ...
      String[] supertypes = {
        "asoc", "cgrp", "comp", "drep", "ftbl", "jp2h", "jpch", "jplh", "res ", "uuid"
      };
