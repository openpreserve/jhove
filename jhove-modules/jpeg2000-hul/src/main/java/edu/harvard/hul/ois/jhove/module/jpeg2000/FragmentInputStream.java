...
/**
 * A FragmentInputStream provides an interface by which the scattered fragments of a Fragment List
 * Box can be read as a single stream. Only fragments within the originating file are supported, not
 * fragments in external files.
 *
 * @author Gary McGath
 */
public class FragmentInputStream extends InputStream {
  ...
  /**
   * @param fragments List of fragment entries. Each fragment entry is an array of two longs, with
   *     fragment[0] being the length and fragment[1] the offset.
   */
  ...
  /**
   * Returns the next byte from the stream, buffering each fragment in turn until the last fragment
   * is exhausted.
   *
   * @return The next byte of the stream, or -1 to indicate no more bytes are available.
   */
  @Override
