...
public class ROIBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    for (int i = 0; i < nroi; i++) {
      ...
      if (incs > 1) {
        _repInfo.setMessage(
            new ErrorMessage(MessageConstants.JPEG2000_HUL_51, _module.getFilePos()));
        _repInfo.setValid(false);
      }
      roiPropList.add(_module.addIntegerProperty("InCodestream", incs, JP2Strings.inCodestreamStr));

      int rtyp = ModuleBase.readUnsignedByte(_dstrm, _module);
      if (rtyp > 1) {
        _repInfo.setMessage(
            new ErrorMessage(MessageConstants.JPEG2000_HUL_52, _module.getFilePos()));
        _repInfo.setValid(false);
      }
      roiPropList.add(_module.addIntegerProperty("RegionType", rtyp, JP2Strings.roiTypeStr));
    ...
    roiProp = new Property("ROIs", PropertyType.PROPERTY, PropertyArity.LIST, propList);
   ...
   * returned by selfDescProperty.
   */
  @Override
  protected Property getSelfPropDesc() {
    if (roiProp != null) {
      return new Property(DESCRIPTION_NAME, PropertyType.PROPERTY, roiProp);
    }
    return null;
  }
}
