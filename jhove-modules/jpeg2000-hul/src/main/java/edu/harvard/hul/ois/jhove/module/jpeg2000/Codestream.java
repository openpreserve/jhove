...
/**
 * The information on a codestream, extracted from a Contiguous Codestream or Fragment Table, and
 * the corresponding Codestream Header if any.
 *
 * @author Gary McGath
 */
public class Codestream extends MainOrTile {
  ...
  public Property makeProperty() {
    ...
    if (tileLengthList != null && !tileLengthList.isEmpty()) {
      propList.add(
          new Property("TileLengths", PropertyType.PROPERTY, PropertyArity.LIST, tileLengthList));
    }
    ...
    if (_ppmLengthList != null && !_ppmLengthList.isEmpty()) {
      propList.add(
          new Property(
              "PackedPacketHeaderLengths", PropertyType.LONG, PropertyArity.LIST, _ppmLengthList));
    }
    if (_crgProperty != null) {
      propList.add(_crgProperty);
    }

    propList.add(new Property("NisoImageMetadata", PropertyType.NISOIMAGEMETADATA, _niso));
  ...
  /**
   * Set the initial Niso values from a default Niso object. This doesn't attempt to be complete,
   * but sets the values which we know could have been set from the JP2 header.
   */
  ...
  /**
   * Set a property indexed by component. If a property for that component doesn't already exist, it
   * is created. <code>prop</code> is then added to the property list of that property.
   */
  @Override
  public void setCompProperty(int idx, Property prop) {
    if (_components != null && _components.length > idx) {
      if (_components[idx] == null) {
        ...
        _components[idx] =
            new Property("Component", PropertyType.PROPERTY, PropertyArity.LIST, new LinkedList());
      }
