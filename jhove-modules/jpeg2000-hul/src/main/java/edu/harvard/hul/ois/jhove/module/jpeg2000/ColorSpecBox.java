...
/**
 * Color specification box. See I.5.3.3 in ISO/IEC 15444-1:2000 and ISO/IEC FCD15444-2: 2000,
 * L.9.4.2
 *
 * @author Gary McGath
 */
public class ColorSpecBox extends JP2Box {
   ...
   *
   * @param parent parent superbox of this box (must be JP2HeaderBox or ColorGroupBox)
   */
  ...
  public boolean readBox() throws IOException {
    if (!(_parentBox instanceof JP2HeaderBox || _parentBox instanceof ColorGroupBox)) {
      wrongBoxContext();
      return false;
    }
    initBytesRead();
    ...
    subProps.add(_module.addIntegerProperty("Method", meth, JP2Strings.methodStr));
    ...
    if (approx == 0) {
      ...
      subProps.add(
          _module.addIntegerProperty("Approx", approx, JP2Strings.approxStr, JP2Strings.approxIdx));
    }
    ...
    if (meth == 1) {
      ...
      Property p;
      p = _module.addIntegerProperty("EnumCS", (int) enumCS, JP2Strings.enumCSStr);
      subProps.add(p);
    } else if (meth == 2) {
      ...
      if (tagSignatureSet.contains("desc")
        ...
        subProps.add(
            new Property("RestrictedICCProfile", PropertyType.STRING, "Monochrome Input Profile"));
      }
      ...
      // Not a valid method 2 box
        ...
        // _module.setJP2Compliant (false);
        _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_10, filePos));
    ...
    if (_boxHeader.getLength() != 0) {
      _module.skipBytes(_dstrm, (int) (len - (_module.getFilePos() - startBytesRead)), _module);
    }
    finalizeBytesRead();
