 ...
 * Composition Box (JPX).
 *
 * <p>See ISO/IEC FCD15444-2: 2000, L.9.10
 *
 * @author Gary McGath
 */
public class CompositionBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    if (!(box instanceof CompOptionsBox)) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_11, _module.getFilePos()));
    ...
    propList.add(new Property("Width", PropertyType.LONG, new Long(_width)));
    propList.add(new Property("Height", PropertyType.LONG, new Long(_height)));
    propList.add(new Property("Loop", PropertyType.INTEGER, new Integer(_loop)));
    if (!instSets.isEmpty()) {
      propList.add(
          new Property("InstructionSets", PropertyType.PROPERTY, PropertyArity.LIST, instSets));
    }
    _module.addProperty(
        new Property("Composition", PropertyType.PROPERTY, PropertyArity.LIST, propList));
    return true;
  }
