 ...
 * Validation Environment Copyright 2004-2007 by JSTOR and the President and Fellows of Harvard
 * College
 *
 ...
 *
 * <p>Code is included for JPX, but is almost entirely untested due to the lack of available sample
 * files that use more than a tiny fraction of the features. The current version of the module is
 * based on the typo-laden and inconsisent "JPEG 2000 Part II Final Committee Draft" (7 December
 * 2000). The final standard (5 May 2004) has just reached our hands, and this code will be reviewed
 * against it and revised accordingly in the near future. (All opinions expressed in this paragraph
 * are those of the programmer.)
 *
 * <p>JPEG 2000 format is not JPEG format, and isn't compatible with it. As with JPEG, JPEG 2000 is
 * not in itself a file format. It can be encapsulated in JP2 or JPX format, which are recognized
 * here.
 *
 * @author Gary McGath
 */
public class Jpeg2000Module extends ModuleBase {
  ...
  private static final String COVERAGE =
      "JP2 (ISO/IEC 15444-1:2000/" + "ITU-T Rec. T.800 (200)), JPX (ISO/IEC 15444-2:2004)";
  ...
  public Jpeg2000Module() {
    ...
    doc =
        new Document(
            "MIME Type Registrations for JPEG 2000 " + "(ISO/IEC 15444) RFC 3745",
            DocumentType.RFC);
    Agent ietfAgent =
    ...
    doc =
        new Document(
            "ITU-T Rec. T.800 (2002), Information "
                + "technology -- JPEG 2000 image coding system: "
                + "Core coding system",
            DocumentType.STANDARD);
    Builder ituBuilder =
        new Agent.Builder("ITU", AgentType.STANDARD)
            .address("ITU, Place des Nations, " + "CH-1211 Geneva 20 Switzerland")
            .telephone("+41 22 730 51 11")
            .fax("+41 22 730 6500")
            .email("itumail@itu.int")
            .web("http://www.itu.int/home/");
   ...
   *
   * <p>This module is based on a RandomAccessFile because of the requirements of the (so far)
   * rarely used fragmented codestream feature. Since just about everything else can be done with an
   * InputStream, we use a RAFInputStream except on the occasions when random access is needed. We
   * pass the module as the <code>counted</code> argment to all read calls, so that we can compute
   * relative positions in the stream based on _nByte.
   *
  ...
  public final void parse(RandomAccessFile raf, RepInfo info) throws IOException {
    initParse();
    _rafStream = new RAFInputStream(raf, _je != null ? _je.getBufferSize() : 0);
    ...
    Property metadata =
        new Property("JPEG2000Metadata", PropertyType.PROPERTY, PropertyArity.LIST, _propList);
    ...
    if (!colorSpecs.isEmpty()) {
      _propList.add(
          new Property("ColorSpecs", PropertyType.PROPERTY, PropertyArity.LIST, colorSpecs));
    }
    if (!binaryFilterProps.isEmpty()) {
      _propList.add(
          new Property(
              "BinaryFilters", PropertyType.PROPERTY, PropertyArity.LIST, binaryFilterProps));
    }
    if (!associationProps.isEmpty()) {
      _propList.add(
          new Property(
              "Associations", PropertyType.PROPERTY, PropertyArity.LIST, associationProps));
    }

    if (!digitalSigProps.isEmpty()) {
      _propList.add(
          new Property(
              "DigitalSignatures", PropertyType.PROPERTY, PropertyArity.LIST, digitalSigProps));
    }
    if (!uuids.isEmpty()) {
      _propList.add(new Property("UUIDs", PropertyType.PROPERTY, PropertyArity.LIST, uuids));
    }
    if (!composLayers.isEmpty()) {
      _propList.add(
          new Property(
              "CompositingLayers", PropertyType.PROPERTY, PropertyArity.LIST, composLayers));
    }
    if (!uuidInfos.isEmpty()) {
      _propList.add(
          new Property("UUIDInfoBoxes", PropertyType.PROPERTY, PropertyArity.LIST, uuidInfos));
    }
    if (!codestreams.isEmpty()) {
      ...
      _propList.add(
          new Property("Codestreams", PropertyType.PROPERTY, PropertyArity.LIST, csProps));
    }
    if (!xmlList.isEmpty()) {
      _propList.add(new Property("XML", PropertyType.STRING, PropertyArity.LIST, xmlList));
    }
  ...
  /**
   * Return the current position in the module. This is somewhat optimistically named; it should be
   * trusted only for relative positions when no seek operations occur in between calls to
   * getFilePos.
   */
  ...
  /**
   * Returns the current NisoImageMetadata object. If curCodestream is null, returns the default
   * NisoImageMetadata, otherwise returns the NisoImageMetadata of curCodestream.
   */
  ...
  /**
   * Returns the nth <code>Codestream</code>, creating it if necessary, and make it the current one.
   * The value of <code>nCodestreams</code> or <code>nCodestreamHeaders</code> is not affected, even
   * if a new <code>Codestream</code> is created.
   */
  ...
  /**
   * Returns the codestream count. Because items may be added to the codestreams list when only the
   * header is seen, this may be less than the size of the codestreams list. It will never be more.
   */
  ...
  /**
   * Sets the codestream count. This affects only the variable, not the size of the codestreams
   * list. The codestream count should never be set to a value larger than the codestreams list; it
   * signifies the number of elements of the list for which codestream boxes have actually been
   * seen.
   */
  ...
  /**
   * Sets a flag indicating JP2 compliance. If the flag is set to <code>true</code>, and the JPX
   * compliance flag is also true, set the MIME type to "image/jpx".
   */
  ...
  /**
   * Adds a Compositing Layer property to the list of Compositing Layer properties. Called from the
   * ComposLayerHdrBox.
   */
  ...
  /**
   * One-argument version of <code>readUnsignedShort</code>. JPEG2000 is always big-endian, so
  ...
  /**
   * One-argument version of <code>readUnsignedInt</code>. JPEG2000 is always big-endian, so
  ...
  /**
   * One-argument version of <code>readSignedLong</code>. JPEG2000 is always big-endian, so
   * readSignedLong can unambiguously drop its endian argument.
   */
  ...
  protected boolean readFileTypeBox(RepInfo info) throws IOException {
    ...
    if (!"ftyp".equals(hdr.getType())) {
      info.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_22, hdr.getType(), _nByte));
      info.setWellFormed(false);
      return false;
    }
   ...
   */
  protected boolean skipOverBox(BoxHeader hdr, RepInfo info, DataInputStream dstrm)
      throws IOException {
    if (hdr.getLength() != 0) {
