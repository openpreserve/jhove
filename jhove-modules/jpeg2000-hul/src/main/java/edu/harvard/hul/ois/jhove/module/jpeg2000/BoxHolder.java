...
public class BoxHolder implements Iterator<Object> {
  ...
  /**
   * Checks if any more subboxes are available. This class doesn't fully conform to the Iterator
   * interface, as there are some cases where the lack of more boxes won't be detected till an EOF
   * is encounterd. So callers should call <code>hasNext</code> to avoid reading overruns, and then
  ...
  public Object next() {
    ...
    try {
      ...
      if ("bfil".equals(hType)) {
        binaryFilterBox =
            new BinaryFilterBox(_raf, (this instanceof JP2Box) ? (JP2Box) this : null);
      ...
      if ("cref".equals(hType)) {
        ...
        CrossRefBox xrefBox =
            new CrossRefBox(_raf, (this instanceof JP2Box) ? (JP2Box) this : null);
        if (!xrefBox.readBox()) {
          return null;
        }
        BoxHeader xrefhdr = new BoxHeader(_module, xrefBox.getCrossRefStream());
        xrefhdr.readHeader();
        nextBox =
  ...
  /**
   * Utility error reporting function for a subbox overrunning its superbox. Sets the RepInfo's
   * wellFormed flag to <code>false</code>.
   */
  protected void superboxOverrun() {
    _repInfo.setMessage(
        new ErrorMessage(
            MessageConstants.JPEG2000_HUL_53, getSelfPropName(), _module.getFilePos()));
    _repInfo.setWellFormed(false);
  }

  /**
   * Utility error reporting function for a subbox underrunning its superbox. Sets the RepInfo's
   * wellFormed flag to <code>false</code>.
   */
  protected void superboxUnderrun() {
    _repInfo.setMessage(
        new ErrorMessage(
            MessageConstants.JPEG2000_HUL_54, getSelfPropName(), _module.getFilePos()));
    _repInfo.setWellFormed(false);
  }
