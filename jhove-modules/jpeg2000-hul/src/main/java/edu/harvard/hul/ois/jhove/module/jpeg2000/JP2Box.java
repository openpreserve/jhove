...
public abstract class JP2Box extends BoxHolder {
  ...
  /**
   * Constructor. Has no arguments, so that invoking lots of different subclasses is relatively
   * simple. setModule, setBoxHeader, setRepInfo, and setDataInputStream should be called
  ...
  /**
   * Static factory method for generating an object of the appropriate subclass of MarkerSegment,
   * based on the box type. This is for use in top-level reading of boxes, not subboxes. Provision
   * is made for calling this with a parent box, but the set of boxes dispatched on is the set used
   * at top level.
   *
   * <p>Certain box types have magical characteristics and have to be checked by the BoxHolder.
   * These include BinaryFilterBox and CrossReferenceBox.
   *
  ...
  /**
   * Reads the box, putting appropriate information in the RepInfo object. setModule, setBoxHeader,
   * setRepInfo and setDataInputStream must be called before <code>readBox</code> is called. Thus,
   * the header of the box must already have been read. <code>readBox</code> must completely consume
   * the box, so that the next byte to be read by the DataInputStream is the <code>FF</code> byte of
   * the next Box. The number of bytes read must be placed in _bytesRead.
   */
  ...
  public void skipBox() throws IOException {
    ...
    if (_boxHeader.getLength() != 0) {
      _module.skipBytes(_dstrm, (int) _boxHeader.getDataLength(), _module);
    }
    finalizeBytesRead();
  }
  ...
  /**
   * Utility error reporting function for incorrect box length. Sets the RepInfo's wellFormed flag
   * to <code>false</code>.
   */
  protected void wrongBoxSize() {
    JhoveMessage message =
        JhoveMessages.getMessageInstance(
            MessageConstants.JPEG2000_HUL_5.getId(),
            String.format(MessageConstants.JPEG2000_HUL_5.getMessage(), getSelfPropName()));
    _repInfo.setMessage(new ErrorMessage(message, _module.getFilePos()));
    _repInfo.setWellFormed(false);
  }

  /**
   * Utility error reporting function for box in a context (superbox or lack thereof) which is not
   * permitted. Sets the RepInfo's wellFormed flag to <code>false</code>.
   */
  protected void wrongBoxContext() {
    _repInfo.setMessage(
        new ErrorMessage(MessageConstants.JPEG2000_HUL_3, getSelfPropName(), _module.getFilePos()));
    _repInfo.setWellFormed(false);
  }
  ...
  protected void emptyBox() {
    _repInfo.setMessage(
        new ErrorMessage(
            MessageConstants.JPEG2000_HUL_4,
            "Box type = " + getSelfPropName(),
            _module.getFilePos()));
    _repInfo.setWellFormed(false);
  }
  ...
  protected Property makeAssocProperty() {
    ...
    return new Property("Associations", PropertyType.PROPERTY, PropertyArity.LIST, associations);
  }

  /**
   * Returns a Property which describes the Box, for use by Association boxes and perhaps others.
   * Most subclasses will only have to override <code>getSelfPropName</code> and <code>
   * getSelfPropDesc</code>. A subclass that shouldn't be added to the Association box's property
   * can override this to return <code>null</code>.
   */
  protected Property selfDescProperty() {
    ...
    subprops.add(new Property("Name", PropertyType.STRING, name));
  ...
  /**
   * Returns a Property which describes the box. This is used as a subproperty of the Property
   * returned by selfDescProperty. Properties that we don't care to describe don't have to override
   * this. This class should return either <code>null</code> or a property with <code>
   * DESCRIPTION_NAME</code> for its name.
   */
