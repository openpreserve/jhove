...
public class ChannelDefBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    if (!(_parentBox instanceof JP2HeaderBox || _parentBox instanceof CodestreamHeaderBox)) {
      wrongBoxContext();
      return false;
    }
    initBytesRead();
    ...
    for (int i = 0; i < nchan; i++) {
      ...
      cprop[0] = new Property("ChannelIndex", PropertyType.INTEGER, new Integer(cidx));
      int typ = _module.readUnsignedShort(_dstrm);
      cprop[1] =
          _module.addIntegerProperty("ChannelType", typ, JP2Strings.ctypStr, JP2Strings.ctypIdx);
      ...
      // an integer.
      cprop[2] = new Property("ChannelAssociation", PropertyType.INTEGER, new Integer(assoc));
      chans[i] = new Property("Channel", PropertyType.PROPERTY, PropertyArity.ARRAY, cprop);
    }

    _module.skipBytes(_dstrm, len, _module);

    Property prop =
        new Property("ChannelDefinition", PropertyType.PROPERTY, PropertyArity.ARRAY, chans);
