...
public class CRGMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    if (_ccs.getCurTile() != null) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_14));
      return false;
    }
    int ncomps = _cs.getNumComponents();
    if (ncomps * 4 != bytesToEat) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_15));
    }
    ...
    props[0] =
        new Property("HorizontalOffsets", PropertyType.INTEGER, PropertyArity.ARRAY, horOffsets);
    props[1] =
        new Property("VerticalOffsets", PropertyType.INTEGER, PropertyArity.ARRAY, vertOffsets);
    _cs.setCRGProperty(
        new Property("ComponentRegistration", PropertyType.PROPERTY, PropertyArity.ARRAY, props));
