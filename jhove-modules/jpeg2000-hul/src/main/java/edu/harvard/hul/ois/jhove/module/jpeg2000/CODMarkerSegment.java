...
/**
 * Class for the COD (coding style default) marker segment. This comes either in the main header or
 * after an SOT and describes the functions used to code the entire tile.
 *
 * @author Gary McGath
 */
public class CODMarkerSegment extends MarkerSegment {
  ...
  /**
   * Processes the marker segment. The DataInputStream will be at the point of having read the
   * marker code. The <code>process</code> method must consume exactly the number of bytes remaining
   * in the marker segment.
   *
  ...
  protected boolean process(int bytesToEat) throws IOException {
    ...
    propList.add(
        new Property(
            "MultipleComponentTransformation", PropertyType.INTEGER, new Integer(mcTrans)));
    propList.add(
        new Property("NumberDecompositionLevels", PropertyType.INTEGER, new Integer(nDecomp)));
    ...
    if (precSize != null) {
      propList.add(
          new Property("PrecinctSize", PropertyType.INTEGER, PropertyArity.ARRAY, precSize));
    }
    cs.setCODProperty(
        new Property("CodingStyleDefault", PropertyType.PROPERTY, PropertyArity.LIST, propList));
