...
/**
 * Class for the RGN (region of interest) marker segment. This comes either in the main header or
 * after an SOT.
 *
 * @author Gary McGath
 */
public class RGNMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    ...
    if (compIdxBytes == 0) {
      ...
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_50));
      return false;
    }
    int compIdx;
    ...
    propList.add(new Property("ROIStyle", PropertyType.INTEGER, new Integer(srgn)));
    propList.add(new Property("ROIParameter", PropertyType.INTEGER, new Integer(sprgn)));
    cs.setCompProperty(
        compIdx,
        new Property("RegionOfInterest", PropertyType.PROPERTY, PropertyArity.LIST, propList));
