...
public abstract class MarkerSegment {
  ...
  protected static final int SOC = 0X4F, // start of codestream
  ...
  /**
   * Constructor. After an instance of a MarkerSegment is created, the setter methods <code>
   * setContCodestream</code>, <code>setCodestream</code>, <code>setModule</code>, and <code>
   * setDataInputStream</code> must all be called as part of the setup before <code>process</code>
   * is called.
   */
  ...
  /**
   * Returns <code>true</code> if this segment is a Marker. Will return <code>false</code> unless
   * overridden.
   */
  ...
  /**
   * Reads and returns the length field of the marker segment. The setter methods <code>setModule
   * </code> and <code>setDataInputStream</code> must be called as part of the setup before <code>
   * readMarkLen</code> is called.
   */
  ...
  /**
   * Determines size of fields indexed by number of components. Some marker segments have fields
   * which are 1 byte long if the number of components is 1-255, and 2 bytes long if the number of
   * components is 256-65535.
   *
  ...
  /**
   * Returns the MainOrTile object which is currently applicable in the Contiguous Codestream. If
   * the Contiguous Codestream has a current Tile, that is returned; otherwise the Codestream object
   * established by setCodestream is returned.
   */
  ...
  /**
   * Process the marker or marker segment. The DataInputStream will be at the point of having read
   * the marker code. The <code>process</code> method must consume exactly the number of bytes
   * remaining in the marker segment; for a marker, this number will always be 0.
   *
   * @param bytesToEat The number of bytes that must be consumed. For a Marker, this number will
   *     always be 0. If it is 0 for a MarkerSegment, the number of bytes to consume is unknown.
   * @return <code>true</code> if segment is well-formed, <code>false</code> otherwise.
   */
