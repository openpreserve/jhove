...
/**
 * Class for the QCC (Quantization component) marker segment. May occur in the main or the tile part
 * header. In the main header it overrides the QCD for the specified component. In the tile part
 * header it overrides the QCD for the component in the tile part.
 *
 * @author Gary McGath
 */
public class QCCMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    ...
    if (compIdxBytes == 0) {
      ...
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_46));
      return false;
    }
    int compIdx;
    int bytesEaten;
    ...
    switch (sqccLow) {
      ...
      case 2:
        ...
        break;
      default:
        _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_47));
        return false; // reserved value
    }

    MainOrTile cs = getMainOrTile();
    List propList = new ArrayList(2);
    propList.add(new Property("QuantizationStyle", PropertyType.INTEGER, new Integer(sqcc)));
    propList.add(new Property("StepValue", PropertyType.INTEGER, PropertyArity.ARRAY, spqcc));
    cs.setCompProperty(
        compIdx,
        new Property("QuantizationComponent", PropertyType.PROPERTY, PropertyArity.LIST, propList));
    return true;
  }
}
