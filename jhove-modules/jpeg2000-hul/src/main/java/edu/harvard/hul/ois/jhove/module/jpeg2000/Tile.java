...
public class Tile extends MainOrTile {
  ...
  public Property makeProperty() {
    ...
    if (_components != null) {
      ...
      for (int i = 0; i < _components.length; i++) {
        if (_components[i] == null) {
          _components[i] =
              new Property(
                  "Component", PropertyType.PROPERTY, PropertyArity.LIST, new LinkedList());
        }
      }
      propList.add(
          new Property("Components", PropertyType.PROPERTY, PropertyArity.ARRAY, _components));
    }
    ...
    if (_packetLengthList != null && !_packetLengthList.isEmpty()) {
      propList.add(
          new Property("PacketLengths", PropertyType.LONG, PropertyArity.LIST, _packetLengthList));
    }
    if (_pptLengthList != null && _pptLengthList.isEmpty()) {
      propList.add(
          new Property(
              "PackedPacketHeaderLengths", PropertyType.LONG, PropertyArity.LIST, _pptLengthList));
    }
    if (!_comments.isEmpty()) {
      propList.add(new Property("Comments", PropertyType.PROPERTY, PropertyArity.LIST, _comments));
    }
