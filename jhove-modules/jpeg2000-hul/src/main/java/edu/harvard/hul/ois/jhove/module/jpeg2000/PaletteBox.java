...
public class PaletteBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    if (!(_parentBox instanceof JP2HeaderBox)) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_26, _module.getFilePos()));
      return false;
    }
    initBytesRead();
    ...
    if (ne < 1 || ne > 1024) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_37, _module.getFilePos()));
      _repInfo.setValid(false); // But keep going anyway
    }
    Property[] subProp = new Property[4];
    subProp[0] = new Property("Entries", PropertyType.INTEGER, new Integer(ne));

    int nc = ModuleBase.readUnsignedByte(_dstrm, _module);
    subProp[1] = new Property("Components", PropertyType.INTEGER, new Integer(nc));
    ...
    subProp[2] = new Property("BitDepth", PropertyType.INTEGER, PropertyArity.ARRAY, bpc);

    // Now the actual component value arrays.  Skip this if
    // ne is out of bounds.
    if (ne > 1024 || ne < 1) {
      subProp[3] = new Property("Values", PropertyType.STRING, "Invalid");
    } else {
      ...
      for (int i = 0; i < nc; i++) {
        ...
        cprop[i] = new Property("Component", PropertyType.INTEGER, PropertyArity.ARRAY, c);
      }
      subProp[3] = new Property("Values", PropertyType.PROPERTY, PropertyArity.ARRAY, cprop);
    }
    Property palProp = new Property("Palette", PropertyType.PROPERTY, PropertyArity.ARRAY, subProp);
    ...
    _module.skipBytes(_dstrm, (int) (len - (_module.getFilePos() - startNByte)), _module);
