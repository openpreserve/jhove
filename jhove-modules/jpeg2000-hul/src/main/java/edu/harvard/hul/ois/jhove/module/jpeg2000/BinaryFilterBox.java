...
/**
 * Binary Filter box (JPX). See ISO/IEC FCD15444-2: 2000, L.9.14
 *
 * <p>A Binary Filter Box can subsume any number of other boxes, which will look to the module as if
 * they simply replace this box. BoxHolder makes a special case of BinaryFilterBoxes, calling the
 * getBoxStream method to extract the subsumed boxes.
 *
 * <p>Only Deflate coding, not DES, is supported.
 *
 * <p>It is assumed that a BinaryFilterBox is never encoded inside another BinaryFilterBox.
 *
 * <p>This is untested code, due to lack of sample files; please report any bugs found to HUL/OIS.
 *
 * @author Gary McGath
 */
public class BinaryFilterBox extends JP2Box {

  private static final int[] gzipuuid = {
   ...
   *
   * @param parent parent superbox of this box or TopLevelBoxHolder
   */
   ...
   * setRepInfo and setDataInputStream must be called before <code>readBox</code> is called. <code>
   * readBox</code> must completely consume the box, so that the next byte to be read by the
  ...
  public boolean readBox() throws IOException {
    ...
    _module.addBinaryFilterProp(
        new Property("BinaryFilter", PropertyType.BYTE, PropertyArity.ARRAY, uuidbuf));
    if (isGzip) {
      // report that we've left information unprocessed
      _repInfo.setMessage(
          new InfoMessage(MessageConstants.INF_BINARY_FILTER_BOX_NOT_GZIP, _module.getFilePos()));
    } else {
      ...
      boxStream =
          new DataInputStream(new InflaterInputStream(new CountedInputStream(_dstrm, streamLimit)));
    }
