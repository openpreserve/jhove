...
public class ComposLayerHdrBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    while (hasNext()) {
      ...
      if (box instanceof ColorGroupBox
        ...
        if (box instanceof LabelBox) {
          label = new Property("Label", PropertyType.STRING, ((LabelBox) box).getLabel());
        }
      } else {
        box.skipBox();
      }
    ...
    if (hasOpacity && hasChannelDef) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_12, _module.getFilePos()));
    ...
    _module.addComposLayer(
        new Property("CompositeLayerHeader", PropertyType.PROPERTY, PropertyArity.LIST, propList));
    return true;
  }
