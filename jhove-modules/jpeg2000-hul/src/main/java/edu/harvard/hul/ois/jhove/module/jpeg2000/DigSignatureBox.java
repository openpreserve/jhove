...
/**
 * Digital Signature Box (JPX). See ISO/IEC FCD15444-2: 2000, L.9.17
 *
 * <p>Only the MD5 and SHA-1 algorithms are supported.
 *
 * @author Gary McGath
 */
public class DigSignatureBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    if (styp > 5) {
      ...
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_19, _module.getFilePos()));
    ...
    if (ptyp > 1) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_18, _module.getFilePos()));
      _repInfo.setValid(false);
    }
    ...
    if (ptyp == 1) {
      ...
      propList.add(new Property("Offset", PropertyType.LONG, off));
      propList.add(new Property("Length", PropertyType.LONG, len));
      sizeLeft -= 8;
    }
    ...
    if (styp == 0 || styp == 1) {
      try {
        ...
        propList.add(new Property("Valid", PropertyType.BOOLEAN, isSigValid(styp, off, len, data)));
    ...
    _module.addDigitalSignatureProp(
        new Property("DigitalSignature", PropertyType.PROPERTY, PropertyArity.LIST, propList));
    finalizeBytesRead();
    return true;
  }
  ...
  private boolean isSigValid(int styp, long off, long len, byte[] data)
