...
/**
 * Class for the POC (Progression order change) marker segment. May occur in the main or the tile
 * part header.
 *
 * @author Gary McGath
 */
public class POCMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    ...
    if (compIdxBytes == 0) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_41));
    ...
    if (changeSize * nChanges != bytesToEat) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_42));
      return false;
    }
    ...
    for (int i = 0; i < nChanges; i++) {
      ...
      propArr[0] =
          new Property("StartResolutionLevelIndex", PropertyType.INTEGER, new Integer(rspoc));
      propArr[1] = new Property("ComponentIndex", PropertyType.INTEGER, new Integer(cspoc));
      propArr[2] = new Property("LayerIndex", PropertyType.INTEGER, new Integer(lyepoc));
      propArr[3] =
          new Property("EndResolutionLevelIndex", PropertyType.INTEGER, new Integer(cepoc));
      propArr[4] = new Property("ProgressionOrder", PropertyType.INTEGER, new Integer(ppoc));
      changes[i] = new Property("Change", PropertyType.PROPERTY, PropertyArity.ARRAY, propArr);
    }
    MainOrTile cs = getMainOrTile();
    cs.setPOCProperty(
        new Property(
            "ProgressionOrderChange", PropertyType.PROPERTY, PropertyArity.ARRAY, changes));
