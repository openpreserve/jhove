...
public class JP2HeaderBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    if (_module.isJP2HdrSeen()) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_32, _module.getFilePos()));
    ...
    while (hasNext()) {
      ...
      if (state == 0 && box instanceof LabelBox) {
        ...
        _module.addProperty(
            new Property("JP2HeaderLabel", PropertyType.STRING, ((LabelBox) box).getLabel()));
      ...
      // is the image header.
        ...
        if (box instanceof ImageHeaderBox) {
          ...
          _repInfo.setMessage(
              new ErrorMessage(MessageConstants.JPEG2000_HUL_33, _module.getFilePos()));
    ...
    if (hasCMap && !hasPalette) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_31, _module.getFilePos()));
      _repInfo.setValid(false);
    }
    if (!hasCMap && hasPalette) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_30, _module.getFilePos()));
      _repInfo.setValid(false);
    }
