...
public class InstructionSetBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    if ((ityp & 0X2B) == 0) {
      if (sizeLeft != 0) {
        _repInfo.setMessage(
            new ErrorMessage(MessageConstants.JPEG2000_HUL_29, _module.getFilePos()));
      ...
      while (sizeLeft >= 0) {
        if (hasXO_YO) {
          ...
          instProps.add(new Property("HorizontalOffset", PropertyType.LONG, xo));
          long yo = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("VerticalOffset", PropertyType.LONG, yo));
          sizeLeft -= 8;
        }
        if (hasWid_Ht) {
          long width = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("Width", PropertyType.LONG, width));
          long height = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("Height", PropertyType.LONG, height));
          sizeLeft -= 8;
        }
        if (hasAnimation) {
          ...
          instProps.add(new Property("Persist", PropertyType.BOOLEAN, persist));
          life &= 0X7FFFFFFF;
          instProps.add(new Property("Life", PropertyType.LONG, life));

          // Sloppy documentation: I'm assuming that N
          // and NEXT-USE are the same thing.
          long nextuse = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("NextUse", PropertyType.LONG, nextuse));
          sizeLeft -= 8;
        }
        if (hasCrop) {
          long xc = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("HorizontalCropOffset", PropertyType.LONG, xc));
          long yc = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("VerticalCropOffset", PropertyType.LONG, yc));
          long wc = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("CroppedWidth", PropertyType.LONG, wc));
          long hc = _module.readUnsignedInt(_dstrm);
          instProps.add(new Property("CroppedHeight", PropertyType.LONG, hc));
          sizeLeft -= 16;
        }
        if (sizeLeft < 0) {
          _repInfo.setMessage(
              new ErrorMessage(MessageConstants.JPEG2000_HUL_28, _module.getFilePos()));
          _repInfo.setWellFormed(false);
          return false;
        }
      }
