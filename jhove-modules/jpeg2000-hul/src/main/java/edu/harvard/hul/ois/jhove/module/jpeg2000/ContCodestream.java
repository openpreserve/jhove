 ...
 *
 * <p>This is based on the information in Appendix A of ISO/IEC 15444-1:2000(E). That standard "does
 * not include a definition of compliance or conformance."
 *
 * @author Gary McGath
 */
public class ContCodestream {
  ...
  private static final int SOC = 0X4F, // start of codestream
   ...
   * Constructor.
   *
   * @param length Length of the codestream, exclusive of the box header. If the codestream box has
   *     a length field of 0, pass 0 for this parameter.
   */
  ...
  /**
   * Reading a codestream generates various bits of information about the image. These are made
   * available after reading through accessor functions.
   *
   * @param cs The image which this codestream defines. Must have a non-null <code>codestream</code>
   *     field.
   * @param info The RepInfo object which accumulates information about the document. Used for
   *     reporting errors.
   * @return True if no fatal errors detected, false if error prevents safe continuation
   */
    ...
    try {
      while (lengthLeft > 0) {
        ...
        if (!ms.process(markLen == 0 ? 0 : markLen - 2)) {
          info.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_9));
          info.setWellFormed(false);
          return false;
        }
  ...
  /**
   * Returns the value of the ppmSeen flag, signifying that a PPM marker segment has been
   * encountered.
   */
