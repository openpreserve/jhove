...
/**
 * File Type Box. See I.5.2 in ISO/IEC 15444-1:2000
 *
 * <p>A File Type box can occur only as the first thing after the Signature Box, so this will be
 * invoked only directly from the Module.
 *
 * @author Gary McGath
 */
public class FileTypeBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    _module.addProperty(new Property("Brand", PropertyType.STRING, brand));
    ...
    _module.addProperty(new Property("MinorVersion", PropertyType.LONG, new Long(minv)));
    ...
    if (ncomp < 1) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_21, _module.getFilePos()));
      _repInfo.setWellFormed(false);
      return false;
    }
    List<String> compList = new ArrayList<>(ncomp);
    boolean eflag = false;
      ...
      for (int j = 0; j < cbytes.length; j++) {
        ...
        if (ch == 0 || ch >= 0X7F) {
          ...
          if (!eflag) {
            ...
            _repInfo.setMessage(
                new ErrorMessage(MessageConstants.JPEG2000_HUL_20, _module.getFilePos()));
          }
        }
      }
      if (!binflag) {
    ...
    _module.addProperty(
        new Property("Compatibility", PropertyType.STRING, PropertyArity.LIST, compList));
