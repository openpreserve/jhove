...
/**
 * Class for the PPT (Packed packet headers, tile-part header) marker segment. Similar to the PPM
 * marker segment, but applicable to tile parts rather than the main header.
 *
 * @author Gary McGath
 */
public class PPTMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    if (_ccs.isPPMSeen()) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_45));
      return false;
    }
    Tile tile = _ccs.getCurTile();
    if (tile == null) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_44));
      return false;
    }
    ...
    while (bytesToEat > 0) {
      ...
      bytesToEat -= 4;
      if (nppt > bytesToEat) {
        _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_43));
        return false;
      }
