...
/**
 * Class for the COC (Coding style component) marker segment. May occur in the main or the tile part
 * header. In the main header it overrides the COD for the specified component. In the tile part
 * header it overrides the COD for the component in the tile part.
 *
 * @author Gary McGath
 */
public class COCMarkerSegment extends MarkerSegment {
  ...
  protected boolean process(int bytesToEat) throws IOException {
    ...
    if (compIdxBytes == 0) {
      ...
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_6));
      return false;
    }
    int compIdx;
    ...
    cs.setCompProperty(
        compIdx, new Property("COC", PropertyType.PROPERTY, PropertyArity.LIST, propList));
