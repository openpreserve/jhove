...
public class OpacityBox extends JP2Box {
  ...
  public boolean readBox() throws IOException {
    ...
    propList.add(_module.addIntegerProperty("Type", otyp, JP2Strings.opacityTypeStr));
    if (otyp > 2) {
      _repInfo.setMessage(new ErrorMessage(MessageConstants.JPEG2000_HUL_36, _module.getFilePos()));
    ...
    if (bytesLeft > 0) {
      ...
      for (int i = 0; i < nch; i++) {
        ...
        for (int j = 0; j < keysize; j++) {
          chkey = (chkey << 8) + ModuleBase.readUnsignedByte(_dstrm, _module);
        }
        keys[i] = chkey;
      }
      propList.add(
          new Property("ChromaKeyValues", PropertyType.INTEGER, PropertyArity.ARRAY, keys));
    }
    ((ComposLayerHdrBox) _parentBox)
        .addOpacity(new Property("Opacity", PropertyType.PROPERTY, PropertyArity.LIST, propList));
